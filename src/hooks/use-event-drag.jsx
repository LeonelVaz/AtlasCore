...
export function useEventDrag({
  ...
  const removeHighlights = () => {
    if (dragInfo.current.highlightedCell) {
      dragInfo.current.highlightedCell.classList.remove(
        "drag-target-active",
        "exceed-limit-slot"
      );
    ...
    document
      .querySelectorAll(
        ".drag-target-active, .exceed-limit-slot, .exceed-limit"
      )
      .forEach((cell) => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (movedSignificantly) {
      ...
      if (targetSlot) {
        if (
          targetSlot.classList.contains("calendar-time-slot") ||
          targetSlot.classList.contains("day-view-hour-slot")
        ) {
          const eventsCount = parseInt(
            targetSlot.getAttribute("data-events-count") || "0"
          );
          dragInfo.current.grid.targetEventsCount = eventsCount;
          dragInfo.current.wouldExceedLimit =
            eventsCount >= maxSimultaneousEvents;
  ...
  const handleMouseUp = (e) => {
    ...
    if (wasActuallyDragged) {
      try {
        ...
        if (
          dragInfo.current.grid.inWeekView &&
          dragInfo.current.grid.dayWidth > 0
        ) {
          daysDelta = Math.round(
            dragInfo.current.deltaX / dragInfo.current.grid.dayWidth
          );
        }

        const deltaY =
          dragInfo.current.virtualDeltaY !== undefined
        ...
        if (snapValue === 0) {
          ...
          let newMinutes =
            ...
            Math.round((hourDelta - Math.floor(hourDelta)) * 60);
          ...
          const closestMinute = validPositions.reduce(
            (prev, curr) =>
              Math.abs(curr - newMinutes) < Math.abs(prev - newMinutes)
                ? curr
                : prev,
            validPositions[0]
          );
          ...
          const newEndDate = new Date(
            startDate.getTime() + dragInfo.current.originalDuration * 60 * 1000
          );
          endDate.setTime(newEndDate.getTime());
        } else {
          const minutesDelta = calculatePreciseTimeChange(
            deltaY,
            false,
            gridSize,
            snapValue
      ...
      document.addEventListener(
        "click",
        function blockClickAfterDrag(evt) {
