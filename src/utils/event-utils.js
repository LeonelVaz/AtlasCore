...
export function initializeGridInfo(eventRef, gridSize, event) {
  try {
    ...
    const isInDayView =
      eventRef.current.closest(".day-view-container") !== null;

    const currentSlot =
      eventRef.current.closest(".calendar-time-slot") ||
    ...
    if (isWeekView) {
      ...
      timeSlots = container
        ? Array.from(container.querySelectorAll(".calendar-time-slot"))
        : [];
    } else {
      container = eventRef.current.closest(".day-view-container");
      timeSlots = container
        ? Array.from(container.querySelectorAll(".day-view-hour-slot"))
        : [];
    }
...
export function findTargetSlot(clientX, clientY, dragInfo) {
  ...
  for (const element of elementsAtPoint) {
    if (
      element.classList &&
      (element.classList.contains("calendar-time-slot") ||
    ...
    if (draggingEvent) {
      ...
      if (elementUnderEvent) {
        ...
        while (current && !targetSlot) {
          if (
            current.classList &&
            (current.classList.contains("calendar-time-slot") ||
  ...
  if (targetSlot) {
    ...
    const minutes = parseInt(
      targetSlot.getAttribute("data-minutes") || "0",
      10
    );
    ...
    if (targetSlot.querySelectorAll) {
      const visibleEvents = targetSlot.querySelectorAll(
        ".calendar-event:not(.dragging)"
      );
 ...
 */
export function calculatePreciseTimeChange(
  deltaY,
  isResize = false,
  gridSize = 60,
  snapValue = 0
