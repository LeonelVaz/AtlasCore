...
const TimeScaleConfig = () => {
  const { currentTimeScale, availableTimeScales, changeTimeScale, createCustomTimeScale } = useTimeScale();
  ...
  return (
    <div className="time-scale-config">
      ...
      <p className="time-scale-config-description">
        Configura la densidad visual de la rejilla temporal para mostrar más o menos información.
      </p>
      ...
      <div className="time-scale-options">
        ...
        <div className="time-scale-options-grid">
          {availableTimeScales.map(scale => (
            <div 
              key={scale.id}
              className={`time-scale-option ${currentTimeScale.id === scale.id ? 'selected' : ''}`}
          ...
          <div 
            className={`time-scale-option ${currentTimeScale.id === 'custom' ? 'selected' : ''}`}
          ...
          <div className="time-scale-custom-form">
            <form onSubmit={handleCustomHeightSubmit}>
              <div className="time-scale-custom-input">
                <label htmlFor="custom-height">Altura (en píxeles por hora):</label>
                <input 
                  id="custom-height"
                  type="number" 
                  min="20" 
              ...
              <div className="time-scale-custom-buttons">
                <button type="submit" className="time-scale-custom-apply">Aplicar</button>
                <button 
                  type="button" 
      ...
      <div className="time-scale-info">
        <h4>Información</h4>
        <p>
          La escala de tiempo determina cuánto espacio vertical ocupa cada hora en el calendario. 
          Una escala más compacta permite ver más horas a la vez, mientras que una escala más espaciosa 
          proporciona más detalle y facilita la interacción con los eventos.
        </p>
      </div>
    </div>
  );
