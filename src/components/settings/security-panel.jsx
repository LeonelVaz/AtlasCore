...
const SecurityPanel = () => {
  ...
  const [securityLevel, setSecurityLevel] = useState(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
  ...
  useEffect(() => {
    const loadSecurityData = async () => {
      try {
        ...
        setSecurityLevel(stats.securityLevel || PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
  ...
  const handleToggleSecurityCheck = async (checkName) => {
    try {
      ...
      const success = pluginSecurityManager.toggleSecurityCheck(checkName, !isActive);
  ...
  const handleApprovePermissions = async (pluginId, permissions) => {
    try {
      setLoading(true);
      const success = await pluginManager.approvePluginPermissions(pluginId, permissions);
  ...
  const handleRejectPermissions = async (pluginId, permissions) => {
    try {
      setLoading(true);
      const success = await pluginManager.rejectPluginPermissions(pluginId, permissions);
  ...
  const handleBlacklistPlugin = async (pluginId, reason = 'Razones de seguridad') => {
    try {
      setLoading(true);
      ...
      <div className="security-config-section">
        ...
        <div className="security-level-selector">
          ...
          <div className="security-level-options">
            <label className={`security-level-option ${securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW ? 'active' : ''}`}>
              <input 
                type="radio" 
                name="securityLevel" 
                value={PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW}
                checked={securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW}
                onChange={() => handleSecurityLevelChange(PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW)}
                disabled={loading}
              />
              <span className="level-name">Bajo</span>
              <span className="level-description">Para desarrollo. Menos restricciones.</span>
            </label>
            
            <label className={`security-level-option ${securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL ? 'active' : ''}`}>
              <input 
                type="radio" 
                name="securityLevel" 
                value={PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL}
                checked={securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL}
                onChange={() => handleSecurityLevelChange(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL)}
                disabled={loading}
              />
              <span className="level-name">Normal</span>
              <span className="level-description">Equilibrio entre seguridad y funcionalidad.</span>
            </label>
            
            <label className={`security-level-option ${securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH ? 'active' : ''}`}>
              <input 
                type="radio" 
                name="securityLevel" 
                value={PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH}
                checked={securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH}
                onChange={() => handleSecurityLevelChange(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH)}
                disabled={loading}
              />
              <span className="level-name">Alto</span>
              <span className="level-description">Máxima seguridad. Para entornos críticos.</span>
            </label>
          </div>
        </div>
        ...
        <div className="security-checks-section">
          ...
          <div className="security-checks-list">
            <label className="security-check-item">
              ...
              <span className="check-description">Controla uso de memoria, CPU y otras operaciones</span>
            </label>
            ...
            <label className="security-check-item">
              ...
              <span className="check-description">Verifica permisos para acceder a APIs del sistema</span>
            </label>
            ...
            <label className="security-check-item">
              ...
              <span className="check-description">Controla uso y límites de almacenamiento persistente</span>
            </label>
            ...
            <label className="security-check-item">
              ...
              <span className="check-description">Detecta manipulaciones sospechosas del DOM</span>
            </label>
            ...
            <label className="security-check-item">
              <input 
                ...
                onChange={() => handleToggleSecurityCheck('externalCommunication')}
                disabled={loading}
              />
              <span>Control de Comunicación Externa</span>
              <span className="check-description">Monitorea y restringe peticiones a servicios externos</span>
            </label>
            ...
            {pendingPermissions.map((request, index) => (
              <div key={index} className="permission-request-item">
                <div className="request-header">
                  ...
                  <span className="request-timestamp">{new Date(request.timestamp).toLocaleString()}</span>
                </div>
                    ...
                    {request.permissions.map((permission, permIndex) => (
                      <li key={permIndex} className="permission-item">
                        ...
                        <span className="permission-description">
                          {PLUGIN_CONSTANTS.SECURITY.PERMISSION_TYPES[permission.toUpperCase()] || 
                ...
                <div className="request-actions">
                  <Button 
                    variant="primary" 
                    onClick={() => handleApprovePermissions(request.pluginId, request.permissions)}
                  >
                    Aprobar
                  </Button>
                  ...
                  <Button 
                    variant="danger" 
                    onClick={() => handleRejectPermissions(request.pluginId, request.permissions)}
                  >
                    Rechazar
                  </Button>
                </div>
                ...
                <div className="threat-actions">
                  ...
                  {threat.actionTaken !== 'blacklisted' && (
                    <Button 
                      variant="danger" 
                      size="small"
                      onClick={() => handleBlacklistPlugin(threat.pluginId, `Amenaza detectada: ${threat.type}`)}
                    >
                      Lista Negra
                    </Button>
                  )}
      ...
      >
        <div className="plugin-security-info">
            ...
            <div className="score-display">
              <div className="score-circle" style={{ 
                background: `conic-gradient(
                  ${securityInfo.securityScore > 70 ? '#4CAF50' : 
                  securityInfo.securityScore > 40 ? '#FFB300' : '#E53935'} 
                  ${securityInfo.securityScore * 3.6}deg, 
                  #e0e0e0 ${securityInfo.securityScore * 3.6}deg 360deg
                )`
              }}>
                <span className="score-number">{securityInfo.securityScore}</span>
              </div>
              <div className="score-label">
                {securityInfo.securityScore > 70 ? 'Bueno' : 
                 securityInfo.securityScore > 40 ? 'Medio' : 'Malo'}
              </div>
            </div>
          </div>
          ...
          <div className="security-status-section">
            ...
            <div className="status-item">
              <strong>Advertencias:</strong> {securityInfo.warnings?.length || 0}
            </div>
            
            <div className="status-item">
              <strong>Errores de Sandbox:</strong> {securityInfo.sandboxErrors?.length || 0}
            </div>
          </div>
            ...
            {securityInfo.permissions?.approved?.length > 0 ? (
              <div className="approved-permissions">
                ...
                <ul className="permissions-list">
                  {securityInfo.permissions.approved.map((permission, index) => (
                    ...
                    <li key={index} className="permission-item">
                      ...
                      <div className="permission-actions">
                        <Button 
                          variant="primary" 
                          size="small"
                          onClick={() => handleApprovePermissions(selectedPlugin.id, [permission])}
                        >
                          Aprobar
                        </Button>
                        <Button 
                          variant="danger" 
                          size="small"
                          onClick={() => handleRejectPermissions(selectedPlugin.id, [permission])}
                        >
                          Rechazar
                        </Button>
                      </div>
              ...
              <div className="resource-metrics">
                <div className="resource-metric">
                  ...
                  <span className="metric-value">
                    {securityInfo.resourceUsage.operationCounts?.cpuTime.toFixed(2) || 0} ms
                  </span>
                </div>
                ...
                <div className="resource-metric">
                  ...
                  <span className="metric-value">
                    {securityInfo.resourceUsage.operationCounts?.totalOperations || 0}
                  </span>
                </div>
                ...
                <div className="resource-metric">
                  ...
                  <span className="metric-value">
                    {formatBytes(securityInfo.resourceUsage.resources?.storage || 0)}
                  </span>
                </div>
              ...
              {securityInfo.resourceUsage.violations?.length > 0 && (
                <div className="resource-violations">
                  ...
                  <ul className="violations-list">
                    {securityInfo.resourceUsage.violations.slice(0, 5).map((violation, index) => (
          ...
          {securityInfo.auditHistory && securityInfo.auditHistory.length > 0 && (
            <div className="audit-history-section">
              <h4>Historial de Auditoría</h4>
              <div className="audit-entries">
                {securityInfo.auditHistory.slice(0, 10).map((entry, index) => (
            ...
            <div className="audit-filters">
              <div className="filter-item">
                ...
                <input 
                  ...
                  onChange={(e) => handleAuditFilterChange('pluginId', e.target.value)}
              ...
              <div className="filter-item">
                ...
                <select 
                  ...
                  onChange={(e) => handleAuditFilterChange('auditType', e.target.value)}
                >
              ...
              <div className="filter-item">
                ...
                <select 
                  ...
                  onChange={(e) => handleAuditFilterChange('limit', e.target.value)}
                >
                ...
                {auditLogs.map((log, index) => (
                  <div key={index} className="audit-log-item">
                    <div className="log-header">
                      ...
                      <span className="log-plugin">{log.pluginId || 'Sistema'}</span>
