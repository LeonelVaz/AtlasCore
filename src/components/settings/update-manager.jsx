...
const UpdateManager = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeUpdateAvailable = eventBus.subscribe('pluginSystem.updateAvailable', ({ pluginId }) => {
      refreshUpdates();
    });
    
    const unsubscribeUpdateCompleted = eventBus.subscribe('pluginSystem.updateCompleted', () => {
      refreshUpdates();
    });
    
    const unsubscribeCheckCompleted = eventBus.subscribe('pluginSystem.updateCheckCompleted', ({ timestamp }) => {
      setChecking(false);
      setLastCheck(timestamp);
      refreshUpdates();
    });
    
    const unsubscribeCheckStarted = eventBus.subscribe('pluginSystem.updateCheckStarted', () => {
      setChecking(true);
    });
    
    const unsubscribeError = eventBus.subscribe('pluginSystem.updateError', ({ error }) => {
      setError(`Error: ${error}`);
      setUpdating(false);
      setChecking(false);
    });
    
    const unsubscribeSettingsChanged = eventBus.subscribe('pluginSystem.updateSettingsChanged', ({ settings }) => {
      setUpdateSettings(settings);
    });
  ...
  const handleApplyAllUpdates = async () => {
    try {
      ...
      if (results.failed.length > 0) {
        setError(`${results.failed.length} actualizaciones fallaron. Detalles en la consola.`);
      }
    ...
    <div className="update-manager">
      ...
      <div className="update-status-bar">
        ...
        <div className="update-status-info">
          ...
          <span className="status-value">{updateSettings.checkAutomatically ? formatInterval(updateSettings.checkInterval) : 'Desactivada'}</span>
        </div>
        
        <div className="update-status-info">
          <span className="status-label">Actualización automática:</span>
          <span className="status-value">{updateSettings.autoUpdate ? 'Activada' : 'Desactivada'}</span>
        </div>
      </div>
        ...
        <>
          <div className="updates-section">
            ...
            <div className="updates-list">
              {Object.keys(availableUpdates).length === 0 ? (
                <p className="no-updates">No hay actualizaciones disponibles.</p>
              ) : (
                  ...
                  <div key={update.id} className="update-item">
                    <div className="update-info">
                      ...
                      <div className="update-versions">
                        <span className="current-version">v{update.currentVersion}</span>
                        <span className="version-arrow">→</span>
                        <span className="new-version">v{update.newVersion}</span>
                      </div>
                    ...
                    <div className="update-actions">
                      <Button 
                        ...
                        size="small"
                      >
                        {updating && selectedPlugin === update.id ? 'Actualizando...' : 'Actualizar'}
                      </Button>
                    </div>
                  </div>
                ))
          ...
          <div className="updates-section">
            ...
            <div className="history-list">
              {Object.keys(updateHistory).length === 0 ? (
                <p className="no-history">No hay historial de actualizaciones.</p>
              ) : (
                  ...
                  <div key={pluginId} className="history-item">
                    <div className="history-info">
                      ...
                      <div className="history-count">
                        {updates.length} {updates.length === 1 ? 'actualización' : 'actualizaciones'}
                      </div>
      ...
      >
        <div className="confirm-update">
          <p>
            ¿Estás seguro de que deseas actualizar este plugin?
          </p>
          ...
          <div className="form-field">
            <label htmlFor="check-auto">
              <input 
                ...
                onChange={(e) => setUpdateSettings({...updateSettings, checkAutomatically: e.target.checked})}
              />
          ...
          {updateSettings.checkAutomatically && (
            <div className="form-field">
              ...
              <select 
                ...
                onChange={(e) => setUpdateSettings({...updateSettings, checkInterval: Number(e.target.value)})}
              >
          ...
          <div className="form-field">
            <label htmlFor="auto-update">
              <input 
                ...
                onChange={(e) => setUpdateSettings({...updateSettings, autoUpdate: e.target.checked})}
              />
          ...
          <div className="form-field">
            <label htmlFor="notifications">
              <input 
                ...
                onChange={(e) => setUpdateSettings({...updateSettings, updateNotificationsEnabled: e.target.checked})}
              />
              ...
              {updateHistory[selectedHistoryPlugin].map((entry, index) => (
                <div key={index} className="history-entry">
                  <div className="history-entry-header">
                    <span className="entry-date">{formatDate(entry.appliedAt)}</span>
                  </div>
