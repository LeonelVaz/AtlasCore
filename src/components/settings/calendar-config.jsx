...
const CalendarConfig = () => {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const savedMaxEvents = await storageService.get(STORAGE_KEYS.MAX_SIMULTANEOUS_EVENTS, 3);
  ...
  useEffect(() => {
    if (!loading) {
      ...
      eventBus.publish('calendar.maxSimultaneousEventsChanged', { value: maxEvents });
      
      console.log('Configuración actualizada: maxSimultaneousEvents =', maxEvents);
    }
  }, [maxEvents, loading]);
  ...
  return (
    <div className="calendar-config">
      ...
      <div className="settings-section">
        ...
        <p>
          Define cuántos eventos pueden ocupar el mismo espacio temporal antes de que se rechacen nuevos eventos en esa franja.
        </p>
        
        <div className="max-events-setting">
          <label htmlFor="max-simultaneous-events">Número máximo de eventos simultáneos (1-10):</label>
          ...
          <p className="max-events-info">
            Al alcanzar este límite, los nuevos eventos que se solapen completamente con los existentes serán rechazados.
          </p>
        </div>
      </div>
      ...
      <div className="settings-section">
        ...
        <p>
          Se añadirán en futuras actualizaciones.
        </p>
      </div>
    </div>
  );
