...
const PluginsPanel = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeCompat = pluginManager.subscribe(
      "compatibilityError",
      (data) => {
    ...
    const unsubscribeCycles = pluginManager.subscribe(
      "cyclesDetected",
      (data) => {
    ...
    const unsubscribeUpdateAvailable = pluginUpdateManager.subscribe(
      "updateAvailable",
      (data) => {
    ...
    const unsubscribeInstalled = pluginManager.subscribe(
      "pluginInstalled",
      () => {
        setRefreshKey((prev) => prev + 1);
      }
    ...
    const unsubscribeUninstalled = pluginManager.subscribe(
      "pluginUninstalled",
      () => {
        setRefreshKey((prev) => prev + 1);
      }
  ...
  const clearPluginStorage = async (pluginId) => {
    ...
    try {
      ...
      alert(
        `Datos de almacenamiento del plugin ${pluginId} eliminados correctamente.`
      );
    } catch (error) {
      console.error(
        `Error al limpiar almacenamiento del plugin ${pluginId}:`,
        error
      );
    } finally {
      setLoading(false);
  ...
  const forceDeactivatePlugin = async (pluginId) => {
    ...
    try {
      ...
      console.error(
        `Error al desactivar forzosamente el plugin ${pluginId}:`,
        error
      );
    } finally {
      setLoading(false);
  ...
  const uninstallPlugin = async (pluginId) => {
    try {
      if (
        !confirm(
          `¿Estás seguro de que deseas desinstalar el plugin ${pluginId}?`
        )
      ) {
        return;
      }
  ...
  const renderCompatibilityBadge = (plugin) => {
    ...
    if (isCompatible) {
      return (
        <span className="plugin-badge plugin-badge-compatible">Compatible</span>
      );
    } else {
      return (
        <span
    ...
    return (
      <div className="dependency-cycles">
        ...
        <p className="cycles-explanation">
          Los ciclos de dependencias pueden causar problemas durante la
          activación de plugins. Se recomienda revisar las dependencias para
          eliminar ciclos.
        </p>
      </div>
    );
                ...
                {Object.entries(apiInfo).map(([pluginId, api]) => (
                  <div key={pluginId} className="api-item">
                    <div className="api-header">
                      ...
                      <span className="api-methods-count">
                        {api.methods.length} métodos
                      </span>
                    </div>
                    <div className="api-methods">
                      {api.methods.map((method) => (
                        <span key={method} className="api-method">
                          {method}
                        </span>
                      ))}
                    </div>
                  ...
                  <div key={channelName} className="channel-item">
                    <div className="channel-header">
                      ...
                      <span className="channel-subscribers">
                        {channel.subscribers.length} suscriptores
                      </span>
                    </div>
                    <div className="channel-info">
                      <span className="channel-creator">
                        Creado por: {channel.creator}
                      </span>
                      <span className="channel-messages">
                        {channel.messagesCount} mensajes
                      </span>
                    </div>
                  </div>
                ))}
      ...
      <>
        <div className="plugins-panel-header">
          ...
          <div className="plugins-header-actions">
            ...
            <Button onClick={handleReloadPlugins} disabled={loading}>
              Recargar Plugins
            </Button>
          ...
          <div className="status-indicator">
            <span
              className={`status-dot ${
                systemStatus.initialized ? "active" : "inactive"
              }`}
            ></span>
          ...
          <div className="status-summary">
            <div className="status-item">
              ...
              <span className="status-value">
                {systemStatus.totalPlugins || 0}
              </span>
            </div>
            <div className="status-item">
              <span className="status-label">Activos:</span>
              <span className="status-value">
                {systemStatus.activePlugins || 0}
              </span>
            </div>
            <div className="status-item">
              <span className="status-label">Compatibles:</span>
              <span className="status-value">
                {systemStatus.compatiblePlugins || 0}
              </span>
            </div>
          ...
          {Object.keys(pluginErrors).length > 0 && (
            <div className="plugins-error-indicator">
              <span className="error-count">
                {Object.keys(pluginErrors).length} plugins con errores
              </span>
              <Button
                variant="text"
                size="small"
          ...
          {cycles.length > 0 && (
            <div className="plugins-issue-indicator">
              <span className="issue-count">
                {cycles.length} ciclos de dependencias
              </span>
              <Button
                variant="text"
                size="small"
        ...
        {showErrors && Object.keys(pluginErrors).length > 0 && (
          <div className="plugins-error-summary">
            <div className="error-summary-header">
              ...
              <Button variant="text" size="small" onClick={clearAllErrors}>
                Limpiar todos
              </Button>
            </div>
            ...
            {plugins.length === 0 ? (
              <div className="plugins-empty">
                ...
                <p className="plugin-help-text">
                  Los plugins deben instalarse en la carpeta{" "}
                  <code>plugins/</code> de la aplicación. Cada plugin debe estar
                  en su propia carpeta con un archivo <code>index.js</code>.
                </p>
        ...
        {showPluginInfo && selectedPlugin && (
          ...
          >
            <div className="plugin-details">
              ...
              <div className="plugin-detail-item">
                <strong>Versión mínima de Atlas:</strong>{" "}
                {selectedPlugin.minAppVersion}
              </div>
              <div className="plugin-detail-item">
                <strong>Versión máxima de Atlas:</strong>{" "}
                {selectedPlugin.maxAppVersion}
              </div>
              <div className="plugin-detail-item">
              ...
              <div className="plugin-detail-item">
                ...
                {selectedPlugin.compatible !== false ? (
                  <span className="compatibility-status compatible">
                    Compatible
                  </span>
                ) : (
                  <span
                    className="compatibility-status incompatible"
                    title={selectedPlugin.incompatibilityReason}
                  >
              ...
              <div className="plugin-detail-item">
                ...
                {pluginPackageManager.isPluginInstalled(selectedPlugin.id) ? (
                  <div className="installation-info">
                    <span className="installation-status installed">
                      Instalado
                    </span>
                    {availableUpdates[selectedPlugin.id] && (
                      <span className="update-status">
                        Actualización disponible: v
                        {availableUpdates[selectedPlugin.id].newVersion}
                      </span>
                    )}
                  </div>
                ) : (
                  <span className="installation-status not-installed">
                    No instalado por el sistema de paquetes
                  </span>
                )}
              </div>
              ...
              {pluginManager.isPluginActive(selectedPlugin.id) && (
                <div className="plugin-detail-item">
                  ...
                  {(() => {
                    const apiInfo =
                      pluginManager.getPluginAPIsInfo()[selectedPlugin.id];
                    if (!apiInfo) {
                  ...
                  <div className="plugin-detail-item">
                    ...
                    <ul className="dependencies-list">
                      {selectedPlugin.dependencies.map((dependency, index) => {
                        ...
                        const isDepActive =
                          depPlugin && pluginManager.isPluginActive(depId);

                        return (
                          <li
                            key={index}
                            className={`dependency-item ${
                              isDepActive ? "active" : "inactive"
                            }`}
                          >
                            {depId} (v{depVersion}) -{" "}
                            {depPlugin
                              ? `${isDepActive ? "Activo" : "Inactivo"}`
                              : "No encontrado"}
                          </li>
                        );
                      })}
              ...
              {selectedPlugin.conflicts &&
                selectedPlugin.conflicts.length > 0 && (
                  <div className="plugin-detail-item">
                    ...
                    <ul className="conflicts-list">
                      {selectedPlugin.conflicts.map((conflict, index) => {
                        ...
                        const conflictPlugin = plugins.find(
                          (p) => p.id === conflictId
                        );
                        const isConflictActive =
                          conflictPlugin &&
                          pluginManager.isPluginActive(conflictId);

                        return (
                          <li
                            key={index}
                            className={`conflict-item ${
                              isConflictActive ? "active" : "inactive"
                            }`}
                          >
                            {conflictId} - {conflictReason} (
                            {conflictPlugin
                              ? `${isConflictActive ? "Activo" : "Inactivo"}`
                              : "No encontrado"}
                            )
                          </li>
                        );
          ...
          >
            <div className="dependencies-graph">
              ...
              <div className="dependencies-tree">
                {dependenciesPlugin.dependencies &&
                dependenciesPlugin.dependencies.length > 0 ? (
                  <ul className="dependencies-tree-list">
                    {dependenciesPlugin.dependencies.map(
                      (dependency, index) => {
                        const depId =
                          typeof dependency === "string"
                            ? dependency
                        ...
                        return (
                          ...
                          >
                            ...
                            {depPlugin &&
                              depPlugin.dependencies &&
                              depPlugin.dependencies.length > 0 && (
                                <ul className="nested-dependencies">
                                  {depPlugin.dependencies.map(
                                    (nestedDep, nestedIndex) => {
                                      const nestedDepId =
                                        typeof nestedDep === "string"
                                          ? nestedDep
                                          : nestedDep.id;
                                      const nestedDepPlugin = plugins.find(
                                        (p) => p.id === nestedDepId
                                      );
                                      const isNestedDepActive =
                                        nestedDepPlugin &&
                                      ...
                                      return (
                                        <li
                                          key={nestedIndex}
                                          className={`nested-dependency-item ${
                                            isNestedDepActive
                                              ? "active"
                                              : "inactive"
                                          }`}
              ...
              <div className="reverse-dependencies">
                {(() => {
                  const reverseDeps = plugins.filter(
                    (p) =>
                      p.dependencies &&
                  ...
                  return (
                    <ul className="reverse-dependencies-list">
                      {reverseDeps.map((plugin, index) => {
                        const isActive = pluginManager.isPluginActive(
                          plugin.id
                        );
                        ...
                        return (
                          <li
                            key={index}
                            className={`reverse-dependency-item ${
                              isActive ? "active" : "inactive"
                            }`}
                          >
                            <span className="reverse-dependency-name">
                              {plugin.name}
                            </span>
                            <span
                              className={`reverse-dependency-status ${
                                isActive ? "active" : "inactive"
                              }`}
                            >
  ...
  return <div className="plugins-panel">{renderContent()}</div>;
};
