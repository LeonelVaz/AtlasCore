...
const PermissionsManager = ({ onPluginClick, compact = false }) => {
    ...
    const loadPermissionsData = async () => {
      try {
        ...
        plugins.forEach((plugin) => {
          const permissionInfo = pluginPermissionChecker.getPluginPermissions(
            plugin.id
          );
  ...
  const handleApprovePermission = async (pluginId, permission) => {
    try {
      setLoading(true);
      const success = await pluginManager.approvePluginPermissions(pluginId, [
        permission,
      ]);
      ...
      console.error(
        `Error al aprobar permiso ${permission} para plugin ${pluginId}:`,
        error
      );
    } finally {
      setLoading(false);
  ...
  const handleRejectPermission = async (pluginId, permission) => {
    try {
      setLoading(true);
      const success = await pluginManager.rejectPluginPermissions(pluginId, [
        permission,
      ]);
      ...
      console.error(
        `Error al rechazar permiso ${permission} para plugin ${pluginId}:`,
        error
      );
    } finally {
      setLoading(false);
  ...
  const handleRevokePermission = async (pluginId, permission) => {
    try {
      setLoading(true);
      const success = await pluginManager.revokePluginPermissions(pluginId, [
        permission,
      ]);
      ...
      console.error(
        `Error al revocar permiso ${permission} para plugin ${pluginId}:`,
        error
      );
    } finally {
      setLoading(false);
          ...
          <div className="permissions-pending-list">
              ...
              <div key={index} className="permission-row compact">
                ...
                <div className="permission-actions">
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      handleApprovePermission(perm.pluginId, perm.permission)
                    }
                  >
                    Aprobar
                  </Button>
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      handleRejectPermission(perm.pluginId, perm.permission)
                    }
                  >
                    Rechazar
                  </Button>
            ...
            {pendingPermissions.length > 3 && (
              <div className="more-permissions">
                <span>
                  +{pendingPermissions.length - 3} permisos pendientes
                </span>
              </div>
            )}
          </div>
        ) : (
          <div className="permissions-empty">No hay permisos pendientes</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="permissions-manager">
      ...
      <div className="permissions-tabs">
        ...
        >
          Pendientes{" "}
          {pendingPermissions.length > 0 && `(${pendingPermissions.length})`}
        </div>
        <div
              ...
              {filterPermissions(pendingPermissions).length > 0 ? (
                <>
                  ...
                  <div className="permissions-list">
                    {filterPermissions(pendingPermissions).map(
                      (perm, index) => (
                ...
                {permissions.length > 0 ? (
                  filterPermissions(permissions).map((perm, index) => (
                    <div key={index} className="permission-row">
                      ...
                      <div className="permission-scope">{perm.permission}</div>
                      <div
                        className={`permission-status ${getStatusClass(
                          perm.status
                        )}`}
                      >
              ...
              {Object.entries(permissionsByType).map(([type, perms]) => {
                ...
                const typeInfo = typeKey
                  ? {
                      ...
                      risk: typeKey.includes("CODE")
                        ? "critical"
                        : typeKey.includes("DOM")
                        ? "high"
                        : typeKey.includes("NETWORK")
                        ? "medium"
                        : "low",
                    }
                  : {
                ...
                return (
                  <div key={type} className="type-card">
                    <div className="type-header">
                      ...
                      <div
                        className={`risk-level ${getRiskClass(typeInfo.risk)}`}
                      >
                        Riesgo:{" "}
                        {typeInfo.risk.charAt(0).toUpperCase() +
                          typeInfo.risk.slice(1)}
                      </div>
                    </div>
