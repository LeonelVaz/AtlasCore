...
class PluginResourceMonitor {
  ...
  trackMemoryUsage(pluginId, bytesUsed) {
    if (!this.initialized || !pluginId || isNaN(bytesUsed)) return;
  ...
  _checkResourceLimits(pluginId) {
    if (!pluginId || !this.operationCounts[pluginId]) return;
  ...
  setSecurityLevel(level) {
    if (!level || !PLUGIN_CONSTANTS.SECURITY.LEVEL[level]) return false;
  ...
  _getEffectiveLimits(pluginId) {
    const baseLimits = this.resourceLimits[this.securityLevel];
    const multiplier = this.restrictedPlugins.has(pluginId) ? this.restrictionMultiplier : 1;
    ...
    try {
      ...
      Object.values(this.operationCounts).forEach(counts => {
        Object.entries(counts).forEach(([key, value]) => {
          if (totalResourceUsage[key] !== undefined) {
