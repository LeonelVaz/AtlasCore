...
class PluginResourceMonitor {
  ...
  initialize(securityLevelInput) {
    // Cambiado el nombre del parámetro para claridad
    if (this.initialized) {
    ...
    try {
      console.log(
        "[ResourceMonitor] Inicializando monitor de recursos para plugins..."
      );
      ...
      console.log(
        `[ResourceMonitor] Monitor de recursos inicializado (nivel: ${this.securityLevel})`
      );
      return true;
    } catch (error) {
      console.error(
        "[ResourceMonitor] Error al inicializar monitor de recursos:",
        error
      );
  ...
  _resetOperationCounters() {
    Object.keys(this.operationCounts).forEach((pluginId) => {
      ...
      if (this.resourceUsage[pluginId].history.length > maxHistorySize) {
        this.resourceUsage[pluginId].history = this.resourceUsage[
          pluginId
    ...
    try {
      ...
      window.fetch = async function (resource, options) {
        ...
        if (
          ...
          typeof window.__resourceMonitor.trackNetworkRequest === "function"
        ) {
      ...
      console.error(
        "[ResourceMonitor] Error al configurar monitoreo de red:",
        error
      );
    }
  }
  ...
  trackOperation(pluginId, operationType = "general") {
    if (
      !this.initialized ||
      !pluginId ||
      !this.activeChecks.has("resourceUsage")
    ) {
    ...
    return () => {
      ...
      if (this.operationCounts[pluginId]) {
        // Verificar que aún exista (pudo ser limpiado)
  ...
  _checkResourceLimits(pluginId) {
    ...
    if (!baseLimits) {
      console.warn(
        `[ResourceMonitor] No se encontraron límites de recursos para el nivel de seguridad: ${this.securityLevel}`
      );
      return;
    }
    ...
    const limits = {
      ...
      networkRequestsPerMinute:
        baseLimits.networkRequestsPerMinute * multiplier,
    };
  ...
  _handleResourceLimitViolations(pluginId, violations) {
    ...
    if (this.securityLevel === PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH) {
      const criticalViolations = violations.filter(
        (v) => v.current > v.limit * 2
      );
      if (criticalViolations.length > 0) {
        eventBus.publish("pluginSystem.securityDeactivateRequest", {
          pluginId,
          reason: `Uso excesivo de recursos: ${criticalViolations
            .map((v) => v.type)
            .join(", ")}`,
        });
      }
  ...
  applyRestrictions(pluginId) {
    ...
    try {
      ...
      console.error(
        `[ResourceMonitor] Error al aplicar restricciones a plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  removeRestrictions(pluginId) {
    ...
    try {
      ...
      console.error(
        `[ResourceMonitor] Error al eliminar restricciones de plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  increaseMonitoring(pluginId) {
    ...
    try {
      ...
      console.error(
        `[ResourceMonitor] Error al aumentar monitoreo para plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  decreaseMonitoring(pluginId) {
    ...
    try {
      ...
      console.error(
        `[ResourceMonitor] Error al reducir monitoreo para plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  setSecurityLevel(level) {
    ...
    if (!level || !validLevels.includes(level)) {
      console.warn(
        `[ResourceMonitor] Nivel de seguridad inválido: ${level}. Los válidos son ${validLevels.join(
          ", "
        )}`
      );
      return false;
    ...
    try {
      if (this.securityLevel === level && this.initialized) {
        // Solo no hacer nada si ya está inicializado y el nivel es el mismo
        return true;
      }
      ...
      console.error(
        `[ResourceMonitor] Error al cambiar nivel de seguridad a ${level}:`,
        error
      );
      return false;
    }
  ...
  updateSecurityChecks(activeChecksInput) {
    if (!activeChecksInput || !Array.isArray(activeChecksInput)) {
      console.warn(
        "[ResourceMonitor] updateSecurityChecks: activeChecksInput debe ser un array."
      );
      return;
    }
    try {
      ...
      if (
        ...
        console.log(
          "[ResourceMonitor] Monitoreo de red desactivado y fetch restaurado."
        );
      }
  ...
  clearPluginData(pluginId) {
    ...
    try {
      ...
      console.error(
        `[ResourceMonitor] Error al limpiar datos de plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  getPluginResourceUsage(pluginId) {
    if (!pluginId) {
      return {
        ...
        limits: this._getEffectiveLimits(null), // Devuelve límites base si no hay pluginId
      };
    }
    try {
      return {
      ...
      console.error(
        `[ResourceMonitor] Error al obtener uso de recursos para plugin ${pluginId}:`,
        error
      );
      return {
        error: error.message,
  ...
  _getEffectiveLimits(pluginId) {
    const baseLimits = this.resourceLimits[this.securityLevel];
    if (!baseLimits) {
      // Fallback si el nivel de seguridad es inválido por alguna razón
      console.warn(
        `[ResourceMonitor] No se pudieron obtener los límites base para el nivel: ${this.securityLevel}. Usando NORMAL.`
      );
      return this.resourceLimits[PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL] || {};
    }
    const multiplier =
      pluginId && this.restrictedPlugins.has(pluginId)
        ? this.restrictionMultiplier
        : 1;
    ...
    try {
      ...
      Object.values(this.operationCounts).forEach((counts) => {
        Object.entries(counts).forEach(([key, value]) => {
          if (totalResourceUsage.hasOwnProperty(key))
            totalResourceUsage[key] += value || 0;
        });
      });
      ...
      stats.topResourceUsers = Object.entries(this.operationCounts)
        ...
        .sort(
          (a, b) =>
            b.operationsCount - a.operationsCount || b.cpuTime - a.cpuTime
        )
        .slice(0, 5);
  ...
  cleanup() {
    try {
      ...
      if (typeof window !== "undefined" && window.__resourceMonitor === this) {
        if (window._originalFetch) {
          // Restaurar fetch si fue modificado
          window.fetch = window._originalFetch;
