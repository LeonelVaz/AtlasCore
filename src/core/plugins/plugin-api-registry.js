...
class PluginAPIRegistry {
  ...
  _checkAPIAccess(callerPluginId, targetPluginId, methodName) {
    ...
    if (callerPlugin?.dependencies) {
      const isDependency = callerPlugin.dependencies.some(dep => 
        (typeof dep === 'string' ? dep : dep.id) === targetPluginId
      );
    ...
    if (conflict) {
      const hasExplicitConflict = conflict.declared?.some(c => 
        (typeof c === 'string' ? c : c.id) === callerPluginId
      );
      
      const hasReversedConflict = conflict.reversed?.some(c => 
  ...
  getPluginAPI(pluginId) {
    return pluginId && this.publicAPIs[pluginId] ? this.publicAPIs[pluginId].processedAPI : null;
  }
