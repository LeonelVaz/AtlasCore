...
class PluginPackageManager {
  ...
  async packagePlugin(pluginId, options = {}) {
    try {
      ...
      packageManifest.checksums = await pluginIntegrityChecker.generateChecksums(pluginFiles);
  ...
  async installPlugin(pluginPackage, options = {}) {
    ...
    try {
      ...
      if (this.installedPlugins[manifest.id] && !options.update) {
        throw new Error(`Ya existe un plugin instalado con el ID: ${manifest.id}`);
      }
      ...
      if (this._compareVersions(appVersion, manifest.minAppVersion) < 0 ||
          this._compareVersions(appVersion, manifest.maxAppVersion) > 0) {
        throw new Error(`El plugin no es compatible con la versión actual ${appVersion}`);
      }
      
      // Verificar dependencias
      const missingDependencies = await this._checkDependencies(manifest.dependencies);
      ...
      if (activeConflicts.length > 0 && !options.ignoreConflicts) {
        throw new Error(`Conflictos con plugins activos: ${activeConflicts.join(', ')}`);
      }
      ...
      if (!plugin) {
        throw new Error(`No se pudo cargar el plugin instalado: ${manifest.id}`);
      }
  ...
  async uninstallPlugin(pluginId, options = {}) {
    ...
    try {
      ...
      if (dependentPlugins.length > 0 && !options.force) {
        throw new Error(`Hay plugins que dependen de este: ${dependentPlugins.join(', ')}`);
      }
  ...
  hasUpdate(pluginId, remoteManifest) {
    ...
    return this._compareVersions(remoteManifest.version, installedPlugin.version) > 0;
  }
  ...
  async _saveInstalledPlugins() {
    try {
      ...
      console.error('Error al guardar información de plugins instalados:', error);
      return false;
    }
  }
