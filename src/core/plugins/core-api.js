...
class CoreAPI {
      ...
      removeExtension: (pluginId, extensionId) => {
        ...
        try {
          ...
          if (success && this._pluginResources[pluginId]?.extensions) {
            this._pluginResources[pluginId].extensions = this._pluginResources[
              pluginId
            ].extensions.filter((ext) => ext.extensionId !== extensionId);
          }
          return success;
        } catch (error) {
  ...
  _initDialogs() {
    ...
    const dialogFunctions = {
      ...
      prompt: async (
        ...
        try {
          if (self._dialogAPI && self._dialogAPI.prompt) {
            return await self._dialogAPI.prompt(
              String(message),
              String(defaultValue),
              title
            );
      ...
      show: function (pluginId, type, message, options = {}) {
        // Usar function() para que 'this' se refiera a dialogFunctions
    ...
    this._errorHandlers.push((pluginId, context, error) => {
      ...
      if (context === "init" || context === "critical") {
        if (this._dialogAPI && this._dialogAPI.alert) {
          // 'this' aquí es la instancia de CoreAPI
          this._dialogAPI
            .alert(
  ...
  _initCommunication() {
    this.plugins = {
      ...
      getPluginAPI: (callerPluginId, targetPluginId) => {
        try {
          if (
            !pluginRegistry.getPlugin(callerPluginId) ||
          ...
          return new Proxy(proxy, {
            get: (target, prop) => {
              ...
              return (...args) =>
                pluginCommunication.callPluginMethod(
      ...
      createChannel: (callerPluginId, channelName, options = {}) => {
        try {
          return pluginCommunication.createChannel(
            channelName,
            callerPluginId,
            options
          );
      ...
      getChannel: (callerPluginId, channelName) => {
        try {
          ...
          const publish = (message) =>
            pluginCommunication.publishToChannel(
              channelName,
              callerPluginId,
              message
            );
          const close = () =>
            pluginCommunication.closeChannel(channelName, callerPluginId);
          return {
            subscribe,
            publish,
            close,
