...
function validateVersionCompatibility(plugin) {
  ...
  if (!compatibility.compatible) {
    console.error(
      `Plugin [${plugin.id}] incompatible: ${compatibility.reason}`
    );
    return false;
  }
...
function validateDependencies(plugin) {
  ...
  if (!Array.isArray(plugin.dependencies)) {
    console.error(
      `Plugin [${plugin.id}] con dependencias declaradas en formato inválido`
    );
    return false;
  }
  ...
  for (const dependency of plugin.dependencies) {
    ...
    if (typeof dependency === "string") {
      if (!dependency.trim()) {
        console.error(
          `Plugin [${plugin.id}] contiene una dependencia con ID vacío`
        );
        return false;
      }
      continue;
    ...
    if (typeof dependency !== "object" || !dependency) {
      console.error(
        `Plugin [${plugin.id}] contiene una dependencia en formato inválido`
      );
      return false;
    }
    ...
    if (
      ...
      console.error(
        `Plugin [${plugin.id}] contiene una dependencia sin ID válido`
      );
      return false;
    }
    ...
    if (!dependency.version || typeof dependency.version !== "string") {
      console.error(
        `Plugin [${plugin.id}] contiene una dependencia sin versión especificada`
      );
      return false;
    }
  }
...
function validateConflicts(plugin) {
  ...
  if (!Array.isArray(plugin.conflicts)) {
    console.error(
      `Plugin [${plugin.id}] con conflictos declarados en formato inválido`
    );
    return false;
  }
  ...
  for (const conflict of plugin.conflicts) {
    ...
    if (typeof conflict === "string") {
      if (!conflict.trim()) {
        console.error(
          `Plugin [${plugin.id}] contiene un conflicto con ID vacío`
        );
        return false;
      }
      continue;
    ...
    if (typeof conflict !== "object" || !conflict) {
      console.error(
        `Plugin [${plugin.id}] contiene un conflicto en formato inválido`
      );
      return false;
    }
    ...
    if (
      ...
      console.error(
        `Plugin [${plugin.id}] contiene un conflicto sin ID válido`
      );
      return false;
    }
    ...
    if (conflict.reason && typeof conflict.reason !== "string") {
      console.error(
        `Plugin [${plugin.id}] contiene un conflicto con razón inválida`
      );
      return false;
    }
  }
...
export function validatePluginComplete(plugin) {
  try {
    ...
    else if (!results.compatibility)
      mainReason = "Incompatible con la versión de la aplicación";
    else if (!results.dependencies)
      mainReason = "Declaración de dependencias inválida";
    else if (!results.conflicts)
      mainReason = "Declaración de conflictos inválida";
    ...
    return {
      ...
      reason: isValid
        ? compatDetails.compatible
          ? "Plugin válido"
          : compatDetails.reason
        : mainReason,
      details: {
    ...
    console.error(
      `Error en validación completa del plugin ${plugin?.id}:`,
      error
    );
    return {
      valid: false,
