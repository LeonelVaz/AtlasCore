...
class PluginCommunication {
  ...
  async callPluginMethod(
    ...
    const communicationId = this._registerCommunication(
      callerPluginId,
      targetPluginId,
      methodName
    );
    ...
    try {
      ...
      const isCompatible = this._checkPluginsCompatibility(
        callerPluginId,
        targetPluginId
      );
  ...
  _checkPluginsCompatibility(pluginId1, pluginId2) {
    try {
      ...
      if (conflicts1 && conflicts1.declared) {
        const hasConflict = conflicts1.declared.some(
          (conflict) =>
            (typeof conflict === "string" ? conflict : conflict.id) ===
            pluginId2
        );
        if (hasConflict) {
          const reason = conflicts1.declared.find(
            (conflict) =>
              (typeof conflict === "string" ? conflict : conflict.id) ===
              pluginId2
          );
      ...
      if (conflicts2 && conflicts2.declared) {
        const hasConflict = conflicts2.declared.some(
          (conflict) =>
            (typeof conflict === "string" ? conflict : conflict.id) ===
            pluginId1
        );
        if (hasConflict) {
          const reason = conflicts2.declared.find(
            (conflict) =>
              (typeof conflict === "string" ? conflict : conflict.id) ===
              pluginId1
          );
  ...
  _registerCommunication(callerPluginId, targetPluginId, methodName) {
    ...
    if (
      this.communicationHistory[callerPluginId].length > this.maxHistorySize
    ) {
      this.communicationHistory[callerPluginId] = this.communicationHistory[
        callerPluginId
    ...
    if (
      this.communicationHistory[targetPluginId].length > this.maxHistorySize
    ) {
      this.communicationHistory[targetPluginId] = this.communicationHistory[
        targetPluginId
      ].slice(0, this.maxHistorySize);
    }
    return id;
  ...
  getChannel(callerPluginId, channelName) {
    if (!channelName) {
      this._handleError(
        callerPluginId,
        "getChannel",
        new Error("Nombre de canal no proporcionado")
      );
      return null;
    }
  ...
  subscribeToChannel(channelName, pluginId, callback) {
    ...
    const compatibility = this._checkPluginsCompatibility(
      pluginId,
      channel.creator
    );
    if (!compatibility.compatible)
      throw new Error(
        `No se puede suscribir al canal: ${compatibility.reason}`
      );
    ...
    if (channel.messages.length > 0 && channel.options.sendHistoryOnSubscribe) {
      ...
      messagesToSend.forEach((message) => {
        try {
          ...
          console.error(
            `Error al enviar mensaje histórico a ${pluginId}:`,
            error
          );
        }
      });
  ...
  publishToChannel(channelName, publisherPluginId, message) {
    ...
    const canPublish =
      publisherPluginId === channel.creator ||
      channel.subscribers[publisherPluginId] ||
      channel.options.allowAnyPublisher === true;
    if (!canPublish) {
      console.error(
        `Plugin ${publisherPluginId} no puede publicar en el canal ${channelName}`
      );
      return false;
    }
    const messageObject = {
      id: `msg_${channelName}_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`,
      timestamp: Date.now(),
    ...
    Object.entries(channel.subscribers).forEach(([pluginId, subscription]) => {
      try {
        ...
        console.error(
          `Error al notificar mensaje a plugin ${pluginId}:`,
          error
        );
        pluginErrorHandler.handleError(pluginId, "channelMessage", error, {
          channelName,
  ...
  closeChannel(channelName, pluginId) {
    ...
    if (pluginId !== channel.creator && !channel.options.allowAnyClose) {
      console.error(
        `Plugin ${pluginId} no puede cerrar el canal ${channelName}`
      );
      return false;
    }
    Object.keys(channel.subscribers).forEach((subscriberId) => {
      try {
        ...
        console.error(
          `Error al notificar cierre a plugin ${subscriberId}:`,
          error
        );
      }
    });
  ...
  _createChannelAPI(channelName, pluginId) {
    return {
      publish: (message) =>
        this.publishToChannel(channelName, pluginId, message),
      subscribe: (callback) =>
        this.subscribeToChannel(channelName, pluginId, callback),
  ...
  _handleError(callerPluginId, operation, error) {
    ...
    console.error(
      `PluginCommunication Error (${operation}) para ${callerPluginId}:`,
      error.message
    );
  }
}
