...
class PluginRepositoryManager {
  ...
  async addRepository(repository) {
    ...
    try {
      ...
      if (!isValid) {
        throw new Error('No se pudo validar el repositorio. URL no accesible o formato incorrecto.');
      }
  ...
  async toggleRepository(repositoryId, enabled) {
    ...
    try {
      ...
      console.error(`Error al cambiar estado del repositorio ${repositoryId}:`, error);
  ...
  async syncAllRepositories() {
    ...
    for (const [repositoryId, repository] of Object.entries(this.repositories)) {
  ...
  async getRepositoryPlugins(repositoryId, forceSync = false) {
    ...
    try {
      ...
      console.error(`Error al obtener plugins del repositorio ${repositoryId}:`, error);
      throw error;
    }
  }
    ...
    try {
      ...
      for (const [repositoryId, repository] of Object.entries(this.repositories)) {
        ...
        try {
          ...
          console.warn(`Error al buscar en repositorio ${repositoryId}:`, error);
        }
      }
      ...
      results.sort((a, b) => {
        ...
        const repoPriorityA = this.repositories[a.repositoryId]?.priority || 100;
        const repoPriorityB = this.repositories[b.repositoryId]?.priority || 100;
