...
class PluginUpdateManager {
  ...
  async initialize() {
    ...
    try {
      ...
      if (this.updateSettings.checkAutomatically && 
  ...
  async checkForUpdates(options = {}) {
    ...
    try {
      ...
      for (const [pluginId, installedInfo] of Object.entries(installedPlugins)) {
        ...
        for (const [repoId, repo] of Object.entries(repositories)) {
          try {
            ...
            if (remoteInfo && pluginPackageManager.hasUpdate(pluginId, remoteInfo)) {
            ...
            console.warn(`Error al verificar ${pluginId} en repositorio ${repoId}:`, repoError);
          }
        }
      }
      ...
      if (this.updateSettings.autoUpdate && Object.keys(this.availableUpdates).length > 0) {
  ...
  async applyUpdate(pluginId) {
    ...
    try {
      ...
      if (!this.availableUpdates[pluginId]) {
        throw new Error(`No hay actualización disponible para el plugin: ${pluginId}`);
      }
      ...
      const repository = pluginRepositoryManager.getRepository(updateInfo.repositoryId);
      
      if (!repository) {
        throw new Error(`Repositorio no encontrado: ${updateInfo.repositoryId}`);
      }
      ...
      await pluginPackageManager.uninstallPlugin(pluginId, { keepSettings: true });
        ...
        files: {
          'index.js': {
            content: 'export default ' + JSON.stringify({
              ...
              conflicts: [],
              permissions: ['storage', 'events', 'ui', 'notifications'],
              init: function(core) { return true; },
              cleanup: function() { return true; }
            }, null, 2),
            type: 'application/javascript'
          },
          'README.md': {
            content: '# Plugin de Ejemplo v0.4.0\n\nPlugin de ejemplo actualizado.',
            type: 'text/markdown'
          }
        }
      };
