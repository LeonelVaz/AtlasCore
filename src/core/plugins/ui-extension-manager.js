...
class UIExtensionManager {
  ...
  _initializeExtensionPoints() {
    ...
    console.log(
      "Puntos de extensión UI inicializados:",
      Object.keys(this.extensionPoints)
    );
  }
  ...
  registerExtension(pluginId, zoneId, componentInfo) {
    ...
    try {
      ...
      console.log(
        `[UIExtensionManager] Extensión registrada: ${pluginId} en ${zoneId}`
      );
      return extensionId;
    } catch (error) {
      console.error(
        `Error al registrar extensión para plugin ${pluginId} en ${zoneId}:`,
        error
      );
      return null;
    }
  ...
  removeExtension(extensionId) {
    ...
    try {
      ...
      Object.entries(this.extensionPoints).forEach(([zoneId, extensions]) => {
        ...
        this.extensionPoints[zoneId] = extensions.filter(
      ...
      if (removed) {
        Object.entries(this.pluginComponents).forEach(
          ([pluginId, registrations]) => {
            const filtered = registrations.filter(
              (reg) => reg.extensionId !== extensionId
            );
  ...
  removeAllPluginExtensions(pluginId) {
    ...
    try {
      ...
      console.error(
        `Error al eliminar extensiones del plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  getPluginExtensions(pluginId) {
    ...
    return this.pluginComponents[pluginId]
      .map((reg) => {
        const { extensionId, zoneId } = reg;
        const extension = this.extensionPoints[zoneId]?.find(
          (ext) => ext.id === extensionId
        );
