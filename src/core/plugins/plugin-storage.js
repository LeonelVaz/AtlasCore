...
class PluginStorage {
  constructor() {
    ...
    this.pluginStorageLimit = 1048576 * 1048576; // 1MB por defecto (Pasado a 1GB para desarrollo)
  ...
  _estimateSize(value) {
    try {
      ...
      console.error(
        "Error estimando tamaño del valor:",
        error,
        "Valor:",
        value
      );
  ...
  _updatePluginDataSize(pluginId, sizeChange) {
    ...
    this.pluginDataSizes[pluginId] =
      (this.pluginDataSizes[pluginId] || 0) + sizeChange;
  ...
  async setItem(pluginId, key, value) {
    if (
      ...
      console.error(
        "Argumentos inválidos para setItem: pluginId y key son requeridos y deben ser strings."
      );
      return false;
    }
    ...
    try {
      ...
      if (this._exceedsStorageLimit(pluginId, sizeDelta)) {
        console.error(
          `Plugin ${pluginId} ha excedido su límite de almacenamiento. Actual: ${
            this.pluginDataSizes[pluginId] || 0
          }, Intento añadir delta: ${sizeDelta}, Límite: ${
            this.pluginStorageLimit
          }`
        );
      ...
      console.error(
        `Error al guardar datos para plugin ${pluginId}, clave ${key}:`,
        error
      );
      return false;
    }
  ...
  async getItem(pluginId, key, defaultValue = null) {
    if (
      ...
      console.error(
        "Argumentos inválidos para getItem: pluginId y key son requeridos y deben ser strings."
      );
      return defaultValue;
    }
    ...
    try {
      ...
      if (storedValue !== undefined) {
        const isNewKeyForSizing =
          !this.pluginKeys[pluginId] || !this.pluginKeys[pluginId]?.has(key);
        this._registerPluginKey(pluginId, key);
      ...
      console.error(
        `Error al recuperar datos para plugin ${pluginId}, clave ${key}:`,
        error
      );
      return defaultValue;
    }
  ...
  async removeItem(pluginId, key) {
    if (
      ...
      console.error(
        "Argumentos inválidos para removeItem: pluginId y key son requeridos y deben ser strings."
      );
      return false;
    }
    ...
    try {
      ...
      if (
        ...
        this.pluginKeys[pluginId] &&
        this.pluginKeys[pluginId].has(key)
      ) {
      ...
      console.error(
        `Error al eliminar datos para plugin ${pluginId}, clave ${key}:`,
        error
      );
      return false;
    }
  ...
  async getKeys(pluginId) {
    ...
    return this.pluginKeys[pluginId]
      ? Array.from(this.pluginKeys[pluginId])
      : [];
  }
  ...
  async clearPluginData(pluginId) {
    ...
    try {
      const keys = this.pluginKeys[pluginId]
        ? Array.from(this.pluginKeys[pluginId])
        : [];
