...
class PluginPermissionChecker {
  constructor() {
    ...
    this.autoApprovedPermissions = {
      ...
      [PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH]: ["ui", "events"],
    };
    ...
    this.availablePermissions = {
      ...
      events: {
        name: "events",
        description: "...",
        methods: ["core.events.subscribe"],
        risk: "low",
      },
      communication: {
        name: "communication",
        description: "...",
        methods: ["core.plugins.getPluginAPI"],
        risk: "medium",
      },
      dom: {
        name: "dom",
  ...
  initialize(securityLevelInput) {
    ...
    if (
      this.initialized &&
      securityLevelInput &&
      this.securityLevel === securityLevelInput
    ) {
      return true;
    }
    ...
    try {
      ...
      if (!levelSetSuccessfully && securityLevelInput) {
        console.warn(
          `Nivel de seguridad de inicialización '${securityLevelInput}' inválido. Se mantiene el nivel '${this.securityLevel}'.`
        );
      }
      ...
      console.log(
        `Verificador de permisos inicializado (nivel: ${String(
          this.securityLevel
        ).toUpperCase()})`
      );
      return true;
  ...
  setSecurityLevel(levelInput) {
    // levelInput debe ser uno de los VALORES de las constantes (ej. "high")
    const validLevels = Object.values(PLUGIN_CONSTANTS.SECURITY.LEVEL); // ["low", "normal", "high"]

    if (levelInput === undefined || levelInput === null) {
      console.warn(
        `[SET_SEC_LEVEL] Intento de establecer un nivel de seguridad nulo o indefinido. No se realizarán cambios.`
      );
      return false;
    }
    ...
    if (!validLevels.includes(levelToValidate)) {
      console.warn(
        `[SET_SEC_LEVEL] Nivel de seguridad inválido: '${levelInput}'. Los niveles válidos son: ${validLevels.join(
          ", "
        )}. No se cambiará el nivel actual ('${this.securityLevel}').`
      );
      return false;
    }
  ...
  validatePermissions(pluginId, permissions) {
    ...
    try {
      ...
      const currentPluginPerms = this.pluginPermissions[pluginId] || {
        approved: [],
        pending: [],
        revoked: [],
      };
      ...
      permissionsArray.forEach((permission) => {
        ...
        const autoApprovedList =
          this.autoApprovedPermissions[this.securityLevel] || [];
      ...
      const reasons = invalidPermissionsInfo.map(
        (p) => `Permiso inválido: ${p.name} - ${p.reason}`
      );
      if (pendingPermissions.length > 0) {
        reasons.push(
          `${
            pendingPermissions.length
          } permisos requieren revisión manual: ${pendingPermissions.join(
            ", "
          )}`
        );
      }
      ...
      this._logPermissionValidation(pluginId, {
        ...
        invalid: invalidPermissionsInfo.map((p) => p.name),
      ...
      console.error(
        `Error al validar permisos para plugin ${pluginId}:`,
        error
      );
      return {
        valid: false,
  ...
  _registerPluginPermissions(pluginId, permissionsToApprove) {
    // Sin cambios
    ...
    permissionsToApprove.forEach((permission) =>
      currentApproved.add(permission)
    );
    this.pluginPermissions[pluginId].approved = Array.from(currentApproved);

    this.pluginPermissions[pluginId].pending = this.pluginPermissions[
      pluginId
    ].pending.filter((p) => !permissionsToApprove.includes(p));
    this.pluginPermissions[pluginId].revoked = this.pluginPermissions[
      pluginId
    ].revoked.filter((p) => !permissionsToApprove.includes(p));
  ...
  _registerPendingPermissions(pluginId, permissionsToAdd) {
    // Sin cambios
    ...
    const existingRequest = this.permissionRequests.find(
      (req) =>
        req.pluginId === pluginId &&
        req.status === "pending" &&
        JSON.stringify(req.permissions.sort()) ===
          JSON.stringify([...permissionsToAdd].sort())
    );
    if (!existingRequest && permissionsToAdd.length > 0) {
      ...
      if (this.permissionRequests.length > this.maxLogSize) {
        this.permissionRequests = this.permissionRequests.slice(
          -this.maxLogSize
        );
      }
    }
  ...
  checkMethodAccess(pluginId, method) {
    ...
    try {
      ...
      Object.entries(this.availablePermissions).forEach(
        ([permName, permInfo]) => {
          if (
            permInfo.methods &&
            permInfo.methods.some((m) => method.includes(m))
          ) {
      ...
      console.error(
        `Error al verificar acceso a método ${method} para plugin ${pluginId}:`,
        error
      );
      return {
        permitted: false,
  ...
  _logApiAccess(pluginId, method, permission, permitted) {
    ...
    if (this.apiAccessLogs[pluginId].length > this.maxLogSize) {
      this.apiAccessLogs[pluginId] = this.apiAccessLogs[pluginId].slice(
        -this.maxLogSize
      );
    }
  }

  approvePermissions(pluginId, permissionsToApprove) {
    // Sin cambios
    if (!this.initialized) this.initialize();
    if (
      !pluginId ||
      !Array.isArray(permissionsToApprove) ||
      permissionsToApprove.length === 0
    ) {
      return false;
    }
    try {
      ...
      this.permissionRequests.forEach((request) => {
        if (request.pluginId === pluginId && request.status === "pending") {
          request.permissions = request.permissions.filter(
            (p) => !permissionsToApprove.includes(p)
          );
      ...
      console.error(
        `Error crítico al aprobar permisos para plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  rejectPermissions(pluginId, permissionsToReject) {
    // Sin cambios
    if (!this.initialized) this.initialize();
    if (
      !pluginId ||
      !Array.isArray(permissionsToReject) ||
      permissionsToReject.length === 0
    )
      return false;
    try {
      if (!this.pluginPermissions[pluginId]) {
      ...
      permissionsToReject.forEach((permission) => {
        const pendingIdx =
          this.pluginPermissions[pluginId].pending.indexOf(permission);
        if (pendingIdx > -1) {
      ...
      this.permissionRequests.forEach((request) => {
        if (request.pluginId === pluginId && request.status === "pending") {
          ...
          request.permissions = request.permissions.filter(
            (p) => !actualPermissionsRejected.includes(p)
          );
          if (
            request.permissions.length === 0 &&
            originalRequestPermissions.some((p) =>
              actualPermissionsRejected.includes(p)
            )
          ) {
      ...
      eventBus.publish("pluginSystem.permissionsRejected", {
        pluginId,
        permissions: actualPermissionsRejected,
      });
      return true;
    } catch (error) {
      console.error(
  ...
  revokePermissions(pluginId, permissionsToRevoke) {
    // Sin cambios
    if (!this.initialized) this.initialize();
    if (
      !pluginId ||
    ...
    try {
      ...
      permissionsToRevoke.forEach((permission) => {
        const approvedIdx =
          this.pluginPermissions[pluginId].approved.indexOf(permission);
        if (approvedIdx > -1) {
      ...
      eventBus.publish("pluginSystem.permissionsRevoked", {
        pluginId,
        permissions: actualPermissionsRevoked,
      });
      return true;
    } catch (error) {
      console.error(
        `Error al revocar permisos para plugin ${pluginId}:`,
        error
      );
      return false;
    }
  ...
  _updateElevatedStatus(pluginId) {
    ...
    const hasHighRiskPermissions = this.pluginPermissions[
      pluginId
    ].approved.some((permission) => {
      const permInfo = this.availablePermissions[permission];
      return (
        permInfo && (permInfo.risk === "high" || permInfo.risk === "critical")
      );
    });
  ...
  getPermissionStats() {
    ...
    try {
      ...
      const pluginsWithElevatedPermissions =
        this.elevatedPermissionPlugins.size;
      ...
      Object.values(this.pluginPermissions).forEach((perms) => {
        (perms.approved || []).forEach((permission) => {
          permissionsByType[permission] =
            (permissionsByType[permission] || 0) + 1;
        });
      });
