...
class PluginSecurityManager {
  ...
  initialize(options = {}) {
    ...
    try {
      ...
      this.securityLevel =
        options.securityLevel || PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL;
      ...
      console.log(
        `Sistema de seguridad inicializado (nivel: ${this.securityLevel})`
      );
      return true;
    } catch (error) {
  ...
  _setupEventListeners() {
    ...
    eventBus.subscribe(
      "pluginSystem.suspiciousExternalCommunication",
      (data) => {
    ...
    switch (severity) {
      case "critical":
        ...
        this.deactivatePlugin(
          pluginId,
          `Amenaza de seguridad crítica: ${type}`
        );
      ...
      case "high":
        ...
        if (this._getPluginWarningsCount(pluginId) > 3) {
          ...
          this.deactivatePlugin(
            pluginId,
            `Múltiples amenazas de seguridad altas: ${type}`
          );
        } else {
    ...
    try {
      ...
      if (this.blacklistedPlugins.has(pluginId)) {
        return {
          valid: false,
          reasons: [
            "Plugin en lista negra por violaciones de seguridad previas",
          ],
        };
      }
      ...
      const permissionsCheck = pluginPermissionChecker.validatePermissions(
        pluginId,
        plugin.permissions
      );
  ...
  deactivatePlugin(pluginId, reason) {
    try {
      ...
      // Registrar el error
      pluginErrorHandler.handleError("security", "deactivatePlugin", error, {
        pluginId,
        reason,
      });
  ...
  whitelistPlugin(pluginId) {
    ...
    try {
      ...
      console.error(
        `Error al eliminar plugin ${pluginId} de lista negra:`,
        error
      );
      return false;
    }
  ...
  getPluginSecurityInfo(pluginId) {
    ...
    try {
      ...
      console.error(
        `Error al obtener información de seguridad para plugin ${pluginId}:`,
        error
      );
  ...
  _calculatePluginSecurityScore(pluginId) {
    try {
      ...
      console.error(
        `Error al calcular puntuación de seguridad para plugin ${pluginId}:`,
        error
      );
      return 0;
    }
  ...
  setSecurityLevel(level) {
    ...
    try {
      ...
      console.log(
        `Nivel de seguridad del sistema de plugins establecido a ${level}`
      );
  ...
  getSecurityStats() {
    try {
      ...
      const pluginsWithWarningsCount = Object.keys(
        this.pluginsWithWarnings
      ).length;
  ...
  clearPluginSecurityData(pluginId) {
    ...
    try {
      ...
      console.error(
        `Error al limpiar datos de seguridad para plugin ${pluginId}:`,
        error
      );
      return false;
    }
