  ...
  try {
    ...
    try {
      ...
      if (vitePlugins.length > 0) {
        console.log(
          `Cargados ${vitePlugins.length} plugins con import.meta.glob`
        );
    ...
    if (plugins.length === 0) {
      ...
      if (webpackPlugins.length > 0) {
        console.log(
          `Cargados ${webpackPlugins.length} plugins con require.context`
        );
        ...
        for (const dir of pluginDirs) {
          try {
            ...
            const module = await import(
              /* @vite-ignore */ `/plugins/${dir}/index.js`
            );
            if (module && module.default) {
    ...
    if (plugins.length === 0) {
      console.error(
        "No se pudieron cargar plugins con ningún método. Comprueba la estructura del proyecto."
      );
    }
...
export async function loadPluginById(pluginId) {
  try {
    ...
    try {
      // Añadimos @vite-ignore para evitar las advertencias de análisis estático
      const module = await import(
        /* @vite-ignore */ `/plugins/${pluginId}/index.js`
      );
      if (module && module.default) {
      ...
      console.warn(
        `No se pudo cargar directamente el plugin ${pluginId}:`,
        error
      );
    }
...
export function validatePluginCompatibility(plugin) {
  try {
    ...
    console.error(
      `Error al validar compatibilidad del plugin ${plugin?.id}:`,
      error
    );
    return {
      valid: false,
