...
class PluginIntegrityChecker {
  ...
  async verifyPackage(pluginPackage) {
    try {
      ...
      for (const [filePath, expectedChecksum] of Object.entries(
        ...
        const actualChecksum = this._simulateChecksum(
          filePath,
          files[filePath]
        );
  ...
  _simulateSignature(manifest, signingKey) {
    ...
    for (let i = 0; i < manifestStr.length; i++) {
      signatureValue =
        ((signatureValue << 5) - signatureValue + manifestStr.charCodeAt(i)) |
        0;
    }

    return `${keyPart}-${signatureValue.toString(36)}-${Date.now().toString(
      36
    )}`;
  }
