...
class PluginManager {
  ...
  async _initializeSecuritySystem(options = {}) {
    try {
      ...
      eventBus.subscribe(
        "pluginSystem.securityDeactivateRequest",
        async (data) => {
          if (data?.pluginId) {
  ...
  async rejectPluginPermissions(pluginId, permissions) {
    if (!pluginId || !Array.isArray(permissions) || permissions.length === 0) {
      console.error("[PM] Argumentos inválidos para rejectPluginPermissions", {
        pluginId,
        permissions,
      });
      return false;
    ...
    try {
      const success = pluginPermissionChecker.rejectPermissions(
        pluginId,
        permissions
      );
      if (success) {
        eventBus.publish("pluginSystem.permissionsRejectedByManager", {
          pluginId,
          permissions,
        });
  ...
  async _loadSecuritySettings() {
    try {
      const settings = await storageService.get(
        STORAGE_KEYS.PLUGIN_SECURITY_SETTINGS_KEY,
        {
  ...
  async _loadPluginStates() {
    try {
      const pluginStatesFromStorage = await storageService.get(
        PLUGIN_STATE_KEY,
        {}
      );
  ...
  async _activatePluginsFromState() {
    try {
      ...
      console.error(
        "[PM] Error al activar plugins desde estado previo:",
        error
      );
      // Lanzar para que initialize() lo capture
      throw new Error(
        `Error al activar plugins desde estado previo: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      for (const plugin of allPlugins) {
        ...
        try {
          ...
          console.error(
            `[PM] Error al verificar compatibilidad para ${plugin.id}:`,
            error
          );
      ...
      // Lanzar para que initialize() lo capture
      throw new Error(
        `Error general en _verifyPluginCompatibility: ${error.message}`
      );
    }
  }
    ...
    if (this._subscribers[eventName]) {
      Object.values(this._subscribers[eventName]).forEach((callback) => {
        try {
          ...
          console.error(`Error en suscriptor a ${eventName}:`, e);
        }
      });
    }
  }
    ...
    try {
      ...
      if (this.securityInitialized) {
        const securityValidation =
          pluginSecurityManager.validatePlugin(pluginId);
      ...
      try {
        if (this.securityInitialized) {
          const finishOperation = pluginResourceMonitor.trackOperation(
            pluginId,
            "activation"
          );
        ...
        // Error específico de la ejecución de init
        console.error(
          `[PM] Error durante la ejecución de init de ${pluginId}:`,
          activationError
        );
        if (this.securityInitialized) {
      ...
      // Catch para errores generales en activatePlugin (ej. de _activateDependencies)
      console.error(
        `[PM] Error general al activar plugin [${pluginId}]:`,
        error
      );
      this._activatingPlugins.delete(pluginId);
      this._publishEvent("error", {
  ...
  async _loadAndRegisterPlugin(pluginId) {
    try {
      ...
      console.error(
        `[PM] Error en _loadAndRegisterPlugin para ${pluginId}:`,
        error
      );
      return null;
    }
  ...
  async _activateDependencies(pluginDefinition) {
    if (
      ...
      pluginDefinition.dependencies.length === 0
    ) {
  ...
  async deactivatePlugin(pluginId, force = false) {
    ...
    try {
      ...
      if (!plugin && pluginId) {
        // Si getPlugin devuelve null pero tenemos un ID
      ...
      if (!force) {
        const dependentPlugins = pluginDependencyResolver
      ...
      try {
        if (this.securityInitialized) {
          ...
          registryCallSuccessful = await pluginSandbox.executeSandboxed(
            pluginId,
          ...
          registryCallSuccessful = await pluginRegistry.deactivatePlugin(
            pluginId
          ); // deactivatePlugin es async
        }
      } catch (error) {
        console.error(
          `[PM] Error durante la ejecución de cleanup de ${pluginId}:`,
          error
        );
      ...
      console.error(
        `[PM] Error general al desactivar plugin [${pluginId}]:`,
        error
      );
      this._publishEvent("error", {
        pluginId,
  ...
  getAllPlugins() {
    ...
    return plugins
      .map((plugin) => {
        ...
        const compatInfo = this._compatibilityResults[plugin.id] || {
          compatible: true,
          reason: "No verificado aún",
        };
        const pluginInfo = {
          ...plugin,
          compatible: compatInfo.compatible,
          incompatibilityReason: !compatInfo.compatible
            ? compatInfo.reason
            : null,
        };
        if (this.securityInitialized) {
          const securityInfo = pluginSecurityManager.getPluginSecurityInfo(
            plugin.id
          );
          Object.assign(pluginInfo, {
            ...
            permissions: pluginPermissionChecker.getPluginPermissions(
              plugin.id
            ),
          });
        }
  ...
  async reloadPlugins(preserveState = true) {
    ...
    try {
      ...
      const currentActivePluginsForDeactivation = [
        ...pluginRegistry.getActivePlugins().map((p) => p.id),
      ];
      ...
      this._publishEvent("pluginsReloaded", {
        ...
        reactivatedCount,
        reactivatedPluginIds: pluginRegistry
          .getActivePlugins()
          .map((p) => p.id),
      });
    } catch (error) {
      // Errores críticos como fallo en loadPlugins o _verifyPluginCompatibility
  ...
  getStatus() {
    ...
    const status = {
      ...
      compatiblePlugins: allPlugins.filter(
        (p) => p?.id && this.isPluginCompatible(p.id)
      ).length,
      incompatiblePlugins: allPlugins.filter(
        (p) => p?.id && !this.isPluginCompatible(p.id)
      ).length,
  ...
  async setSecurityLevel(level) {
    ...
    if (!newLevel) {
      console.error(
        `[PM] Nivel de seguridad inválido proporcionado a setSecurityLevel: ${level}`
      );
      return false;
    }
    ...
    if (this.securityInitialized) {
      try {
        ...
        console.error(
          `[PM] Error al aplicar nuevo nivel de seguridad ${newLevel} a subsistemas:`,
          error
        );
  ...
  async approvePluginPermissions(pluginId, permissions) {
    ...
    try {
      const permissionsInfo =
        pluginPermissionChecker.getPluginPermissions(pluginId);
      const success = pluginPermissionChecker.approvePermissions(
        pluginId,
        permissions
      );
      if (success) {
        pluginRegistry.setPluginState(pluginId, {
          permissionsApproved: [
            ...new Set([...(permissionsInfo?.approved || []), ...permissions]),
          ],
          permissionsPending: (permissionsInfo?.pending || []).filter(
            (p) => !permissions.includes(p)
          ),
        });
        if (!(await this._savePluginStates())) {
          // Guardar y verificar si falló
  ...
  async blacklistPlugin(pluginId, reason) {
    ...
    try {
      if (pluginRegistry.isPluginActive(pluginId)) {
        if (!(await this.deactivatePlugin(pluginId, true))) {
          // Verificar si la desactivación falló
  ...
  getPluginSecurityInfo(pluginId) {
    ...
    try {
      const securityInfo =
        pluginSecurityManager.getPluginSecurityInfo(pluginId);
      const additionalInfo = {
        ...
        auditHistory: pluginSecurityAudit
  ...
  getPluginAPIsInfo() {
    ...
    try {
      ...
      Object.entries(apiInfo).forEach(([pluginId, infoEntry]) => {
        ...
        if (plugin) {
          // Solo añadir si el plugin existe en el registro
          result[pluginId] = {
            ...infoEntry,
      ...
      console.error("[PM] Error en getPluginAPIsInfo:", e);
      return {};
    }
  }
  ...
  getSecurityStats() {
    ...
    try {
      ...
      console.error("[PM] Error en getSecurityStats:", e);
      return {
        error: e.message,
        securityEnabled: true,
        securityLevel: this.securityLevel,
  ...
  getPendingPermissionRequests() {
    ...
    try {
      ...
      console.error("[PM] Error en getPendingPermissionRequests:", e);
      return [];
    }
  }
  ...
  getChannelsInfo() {
    ...
    try {
      ...
      Object.entries(channels).forEach(([channelName, channelInfo]) => {
        ...
        result[channelName] = {
          ...
          subscribersInfo: (channelInfo.subscribers || []).map((id) => {
            ...
            return {
              id,
              name: plugin?.name || "Desconocido",
              active: pluginRegistry.isPluginActive(id),
            };
          }),
        };
      });
      ...
      console.error("[PM] Error en getChannelsInfo:", e);
      return {};
    }
  }
