...
class PluginDependencyResolver {
  ...
  calculateLoadOrder() {
    try {
      ...
      if (cycles.length > 0) {
        console.warn(
          `Detectados ${cycles.length} ciclos de dependencias, se romperán arbitrariamente para calcular orden de carga`
        );
      }
  ...
  getPluginPriority(plugin) {
    ...
    try {
      ...
      console.error(
        `Error al calcular prioridad del plugin ${plugin.id}:`,
        error
      );
    ...
    try {
      ...
      for (const plugin of allPlugins) {
        ...
        const reversedConflicts =
          pluginCompatibility.checkReversedConflicts(plugin);

        // Verificar si está en algún ciclo
        const pluginCycles = cycles.filter((cycle) =>
          cycle.nodes.includes(plugin.id)
        );
