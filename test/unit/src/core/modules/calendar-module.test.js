...
describe('CalendarModule', () => {
  ...
  beforeEach(() => {
    ...
    mockCalendarService = {
      getCurrentDate: jest.fn(() => new OriginalDate('2023-01-15T10:00:00.000Z')),
      getEvents: jest.fn(() => [
        { id: '1', start: new OriginalDate('2023-01-15T09:00:00Z'), end: new OriginalDate('2023-01-15T10:00:00Z'), title: 'Event 1' },
        { id: '2', start: new OriginalDate('2023-01-15T11:00:00Z'), end: new OriginalDate('2023-01-15T12:00:00Z'), title: 'Event 2' },
      ]),
  ...
  describe('init', () => {
    test('no debe inicializar si no se proporciona calendarService y loguear error', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('CalendarModule: No se proporcionó servicio de calendario');
  ...
  describe('Métodos de obtención de datos (después de init)', () => {
    ...
    test('getSelectedDate debe devolver la fecha seleccionada', () => {
      expect(calendarModule.getSelectedDate()).toEqual(new OriginalDate('2023-01-15T10:00:00.000Z'));
    });
    
    test('getEvent debe devolver un evento por ID o null', () => {
        expect(calendarModule.getEvent('1')).toEqual(mockCalendarService.getEvents()[0]);
    ...
    test('getCalendarConfig debe devolver la configuración del servicio o una por defecto', () => {
        expect(calendarModule.getCalendarConfig()).toEqual(mockCalendarService.getConfig());
        
        const serviceWithoutConfig = { ...mockCalendarService, getConfig: undefined };
  ...
  describe('Manipulación de Eventos (después de init)', () => {
    ...
    test('createEvent debe llamar al método del servicio', () => {
      const eventData = { title: 'New Event', start: new OriginalDate(), end: new OriginalDate() };
    ...
    test('updateEvent debe llamar al método del servicio', () => {
      ...
      expect(mockCalendarService.updateEvent).toHaveBeenCalledWith('1', eventData);
    });
  ...
  describe('Suscripciones a EventBus (después de init)', () => {
    ...
    test('debe actualizar eventos internos cuando se dispara EVENTS_LOADED', () => {
      ...
      expect(capturedCallbacks[CalendarEvents.EVENTS_LOADED]).toBeInstanceOf(Function);
      act(() => { 
    ...
    test('debe añadir evento cuando se dispara EVENT_CREATED', () => {
      const newEvent = { id: 'new1', title: 'Created Event', start: new OriginalDate(), end: new OriginalDate() };
      const initialEventCount = calendarModule.getEvents().length;
      expect(capturedCallbacks[CalendarEvents.EVENT_CREATED]).toBeInstanceOf(Function);
      act(() => {
    ...
    test('no debe añadir evento si ya existe al disparar EVENT_CREATED', () => {
        ...
        expect(capturedCallbacks[CalendarEvents.EVENT_CREATED]).toBeInstanceOf(Function);
        act(() => {
          capturedCallbacks[CalendarEvents.EVENT_CREATED]({ event: existingEvent });
        });
        expect(calendarModule.getEvents().length).toBe(initialEventCount);
      });
  
    test('debe actualizar evento cuando se dispara EVENT_UPDATED', () => {
      const updatedEventData = { id: '1', title: 'Event 1 Updated', start: new OriginalDate('2023-01-15T09:00:00Z'), end: new OriginalDate('2023-01-15T10:30:00Z') };
      expect(capturedCallbacks[CalendarEvents.EVENT_UPDATED]).toBeInstanceOf(Function);
      act(() => {
        capturedCallbacks[CalendarEvents.EVENT_UPDATED]({ newEvent: updatedEventData });
      });
      const updatedEvent = calendarModule.getEvent('1');
      expect(updatedEvent.title).toBe('Event 1 Updated');
      expect(new OriginalDate(updatedEvent.end)).toEqual(new OriginalDate('2023-01-15T10:30:00Z'));
    });
  
    test('debe eliminar evento cuando se dispara EVENT_DELETED', () => {
      const eventToDelete = { id: '1' };
      const initialEventCount = calendarModule.getEvents().length;
      expect(capturedCallbacks[CalendarEvents.EVENT_DELETED]).toBeInstanceOf(Function);
      act(() => {
        capturedCallbacks[CalendarEvents.EVENT_DELETED]({ event: eventToDelete });
      });
      expect(calendarModule.getEvents().length).toBe(initialEventCount - 1);
  ...
  describe('Filtrado y Agrupación de Eventos (después de init)', () => {
    const mockEventsForFiltering = [
        { id: 'ev1', title: 'Morning Meeting', start: '2023-10-26T09:00:00Z', end: '2023-10-26T10:00:00Z', color: 'blue' },
        { id: 'ev2', title: 'Lunch', start: '2023-10-26T12:00:00Z', end: '2023-10-26T13:00:00Z', color: 'green' },
        { id: 'ev3', title: 'Afternoon Task', start: '2023-10-26T14:00:00Z', end: '2023-10-26T16:00:00Z', color: 'blue' },
        { id: 'ev4', title: 'Next Day Prep', start: '2023-10-27T10:00:00Z', end: '2023-10-27T11:00:00Z', color: 'red' },
        { id: 'ev5', title: 'All Day Event', start: '2023-10-26T00:00:00Z', end: '2023-10-26T23:59:59Z', color: 'yellow' },
    ];

    beforeEach(() => {
        mockCalendarService.getEvents.mockReturnValue(mockEventsForFiltering.map(e => ({
            ...e,
    ...
    test('getEventsForDate debe devolver eventos para la fecha dada', () => {
        const events = calendarModule.getEventsForDate(new OriginalDate('2023-10-26T10:00:00Z'));
        expect(events.length).toBe(4); 
        expect(events.map(e => e.id).sort()).toEqual(['ev1', 'ev2', 'ev3', 'ev5'].sort());
    });

    test('getEventsForDateRange debe devolver eventos dentro del rango (considerando normalización del método)', () => {
        const events = calendarModule.getEventsForDateRange('2023-10-26T00:00:00Z', '2023-10-26T12:30:00Z');
        expect(events.length).toBe(4); 
        expect(events.map(e => e.id).sort()).toEqual(['ev1', 'ev2', 'ev3', 'ev5'].sort());
    });
    ...
    test('getUpcomingEvents debe devolver eventos futuros ordenados (con mock de Date)', () => {
        ...
        const dateSpy = jest.spyOn(global, 'Date').mockImplementation((...args) => {
            if (args.length > 0 && typeof args[0] === 'string' && args[0].includes('-')) { // Si es un string de fecha ISO
    ...
    test('getMonthMetadata debe devolver metadatos para los días del mes (Octubre - 31 días)', () => {
      ...
      if (day26) { // Solo acceder a props si day26 existe
          expect(day26.hasEvents).toBe(true);
          expect(day26.eventCount).toBe(4); 
          expect(day26.eventColors).toEqual(expect.arrayContaining(['blue', 'green', 'yellow']));
      }
