...
jest.mock("../../../../../src/core/config/constants", () => ({
  PLUGIN_CONSTANTS: {
    UI_EXTENSION_ZONES: {
      // Define aquí las zonas que usarás en los tests
...
describe("UIExtensionManager", () => {
  ...
  describe("Initialization and Zone Management", () => {
    test("debe inicializar puntos de extensión basados en PLUGIN_CONSTANTS", () => {
      ...
      expect(Array.isArray(uiExtensionManager.extensionPoints[zoneId])).toBe(
        true
      );
    });
    ...
    test("extensionZoneExists debe verificar si una zona existe", () => {
      ...
      expect(uiExtensionManager.extensionZoneExists("nonExistentZone")).toBe(
        false
      );
    });
  });
  ...
  describe("registerExtension", () => {
    test("debe registrar una extensión en una zona válida", () => {
      const extensionId = uiExtensionManager.registerExtension(
        pluginId,
        zoneId,
        { component: mockComponent, order: 50 }
      );

      expect(extensionId).toMatch(/^ext_\d+_\d+$/);
      expect(uiExtensionManager.extensionPoints[zoneId].length).toBe(1);
      expect(uiExtensionManager.extensionPoints[zoneId][0].id).toBe(
        extensionId
      );
      expect(uiExtensionManager.extensionPoints[zoneId][0].pluginId).toBe(
        pluginId
      );
      expect(uiExtensionManager.extensionPoints[zoneId][0].order).toBe(50);
      expect(uiExtensionManager.pluginComponents[pluginId]).toContainEqual({
        extensionId,
        zoneId,
      });
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.extensionPointChanged",
        { zoneId, extensions: expect.any(Array) }
      );
    });
    ...
    test("debe devolver null si la zona no es válida", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Zona de extensión no válida: invalidZone"
      );
    });

    test('debe ordenar las extensiones por la propiedad "order"', () => {
      uiExtensionManager.registerExtension(pluginId, zoneId, {
        component: () => "CompA",
        order: 100,
      });
      uiExtensionManager.registerExtension(pluginId, zoneId, {
  ...
  describe("removeAllPluginExtensions", () => {
    test("debe eliminar todas las extensiones de un plugin específico", () => {
      const extId1 = uiExtensionManager.registerExtension(pluginId, zoneId, {
        component: mockComponent,
      });
      const zoneId2 = PLUGIN_CONSTANTS.UI_EXTENSION_ZONES.SIDEBAR_TOP; // Usar el valor del mock
      const extId2 = uiExtensionManager.registerExtension(pluginId, zoneId2, {
        component: mockComponent,
      });
      uiExtensionManager.registerExtension("otherPlugin", zoneId, {
        component: mockComponent,
      });
      ...
      expect(
        uiExtensionManager
          ...
          .some((ext) => ext.id === extId1)
      ).toBe(false);
      expect(
        uiExtensionManager
          .getExtensionsForZone(zoneId2)
          .some((ext) => ext.id === extId2)
      ).toBe(false);
      expect(uiExtensionManager.pluginComponents[pluginId]).toBeUndefined();
  ...
  describe("getExtensionsForZone", () => {
    test("debe devolver todas las extensiones para una zona específica", () => {
      uiExtensionManager.registerExtension(pluginId, zoneId, {
        component: mockComponent,
        order: 100,
      }); // Ajustar order si es necesario para la aserción
      uiExtensionManager.registerExtension("otherPlugin", zoneId, {
        component: mockComponent,
        order: 10,
      });
  ...
  describe("Event Bus Integration", () => {
    test("debe eliminar extensiones de un plugin cuando se desactiva", () => {
      ...
      if (global.mockPluginDeactivatedCallback) {
        ...
        console.warn(
          "global.mockPluginDeactivatedCallback no está definido, llamando directamente a removeAllPluginExtensions para el test"
        );
  ...
  describe("getExtensionStats", () => {
    test("debe devolver estadísticas correctas de las extensiones", () => {
      uiExtensionManager.registerExtension("p1", zoneId, {
        component: mockComponent,
      });
      const zoneId2 = PLUGIN_CONSTANTS.UI_EXTENSION_ZONES.SIDEBAR_BOTTOM; // Usar el valor del mock
      uiExtensionManager.registerExtension("p1", zoneId2, {
        component: mockComponent,
      });
      uiExtensionManager.registerExtension("p2", zoneId, {
        component: mockComponent,
      });
