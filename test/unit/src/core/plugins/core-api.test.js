...
jest.mock("../../../../../src/core/plugins/ui-extension-manager", () => ({
  ...
  removeAllPluginExtensions: jest.fn(),
}));
...
jest.mock("../../../../../src/core/modules/calendar-module", () => ({
  __esModule: true,
  default: { init: jest.fn(), getEvents: jest.fn() },
}));
...
const calendarModule =
  require("../../../../../src/core/modules/calendar-module").default;
const {
  createPluginDialogAPI,
} = require("../../../../../src/utils/dialog-interceptor");
...
describe("CoreAPI", () => {
  ...
  beforeEach(() => {
    ...
    mockServices = {
      ...
      calendar: {
        /* mock del servicio de calendario, puede estar vacío si no se testea su uso */
      },
    };
  ...
  describe("API de Eventos (events)", () => {
    ...
    test("events.subscribe debe llamar a pluginEvents.subscribe", () => {
      ...
      expect(pluginEvents.subscribe).toHaveBeenCalledWith(
        "pluginTest",
        "eventName",
        cb
      );
    ...
    test("events.publish debe llamar a pluginEvents.publish", () => {
      ...
      expect(pluginEvents.publish).toHaveBeenCalledWith(
        "pluginTest",
        "eventName",
        { data: "test" }
      );
  ...
  describe("API de Almacenamiento (storage)", () => {
    ...
    test("storage.setItem debe llamar a pluginStorage.setItem", async () => {
      ...
      expect(pluginStorage.setItem).toHaveBeenCalledWith(
        "pluginTest",
        "key",
        "value"
      );
    ...
    test("storage.getItem debe llamar a pluginStorage.getItem", async () => {
      ...
      expect(pluginStorage.getItem).toHaveBeenCalledWith(
        "pluginTest",
        "key",
        "default"
      );
    ...
    test("storage.removeItem debe llamar a pluginStorage.removeItem", async () => {
      ...
      expect(pluginStorage.removeItem).toHaveBeenCalledWith(
        "pluginTest",
        "key"
      );
    });
  ...
  describe("API de UI (ui)", () => {
    ...
    test("ui.registerExtension debe llamar a uiExtensionManager.registerExtension y registrar recurso", () => {
      ...
      const extId = coreAPI.ui.registerExtension("pluginTest", "zone1", comp, {
        order: 50,
      });

      expect(uiExtensionManager.registerExtension).toHaveBeenCalledWith(
        "pluginTest",
        "zone1",
        { component: comp, props: {}, order: 50 }
      );
      expect(extId).toBe("ext123");
      expect(coreAPI._pluginResources["pluginTest"].extensions).toContainEqual({
        extensionId: "ext123",
        zoneId: "zone1",
      });
    });
  ...
  describe("API de Comunicación entre Plugins (plugins)", () => {
    ...
    test("plugins.registerAPI debe llamar a pluginAPIRegistry.registerAPI", () => {
      ...
      expect(pluginAPIRegistry.registerAPI).toHaveBeenCalledWith(
        "pluginTest",
        apiObj
      );
    });
  ...
  describe("Manejo de Errores", () => {
    ...
    test("_handleError debe llamar a los handlers registrados y a console.error, y dialog si es crítico", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error en plugin [pluginTest] (init):",
        error
      );
      expect(mockDialogApiAlert).toHaveBeenCalledWith(
  ...
  describe("getModule", () => {
    ...
    test("getModule debe devolver null y loguear advertencia si el módulo no se encuentra", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "Módulo no encontrado: nonExistentModule"
      );
    });
  });
  ...
  describe("cleanupPluginResources", () => {
    ...
    test("debe llamar a los métodos de limpieza correspondientes", async () => {
      coreAPI._pluginResources["pluginToClean"] = {
        extensions: [{ extensionId: "ext1", zoneId: "zoneA" }],
      };

      await coreAPI.cleanupPluginResources("pluginToClean");
      expect(pluginEvents.unsubscribeAll).toHaveBeenCalledWith("pluginToClean");
      expect(uiExtensionManager.removeAllPluginExtensions).toHaveBeenCalledWith(
        "pluginToClean"
      );
      expect(pluginAPIRegistry.unregisterAPI).toHaveBeenCalledWith(
        "pluginToClean"
      );
      expect(pluginCommunication.clearPluginResources).toHaveBeenCalledWith(
        "pluginToClean"
      );
