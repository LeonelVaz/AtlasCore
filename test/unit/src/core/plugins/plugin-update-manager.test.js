...
describe('PluginUpdateManager', () => {
  ...
  const availableUpdatesKey = STORAGE_KEYS.PLUGIN_DATA_PREFIX + 'available_updates';
  ...
  const installedPluginInfo = { id: pluginId, version: '1.0.0', name: 'Test Plugin' };
  const remotePluginInfo = { id: pluginId, version: '1.1.0', name: 'Test Plugin Updated', releaseNotes: 'New features!' };
  const mockRepo = { id: 'official', name: 'Official Repo', url: 'http://official.com', apiEndpoint: 'http://official.com/api' };
  ...
  beforeEach(() => {
    ...
    storageService.get.mockImplementation(async (key, defaultValue) => {
        ...
        if (key === settingsKey) return Promise.resolve({ // Devolver settings base
    ...
    pluginPackageManager.getInstalledPlugins.mockReturnValue({ [pluginId]: installedPluginInfo });
  ...
  describe('initialize', () => {
    ...
    test('debe inicializar y cargar datos desde storageService', async () => {
      ...
      storageService.get.mockImplementation(async (key) => {
        if (key === settingsKey) return Promise.resolve({ ...pluginUpdateManager.updateSettings, checkAutomatically: false });
    ...
    test('debe verificar actualizaciones automáticamente si está configurado y el intervalo ha pasado', async () => {
        const checkForUpdatesSpy = jest.spyOn(pluginUpdateManager, 'checkForUpdates');
        
        // Configurar mocks para ESTE test
        storageService.get.mockImplementation(async (key) => {
            if (key === lastCheckKey) return Promise.resolve(Date.now() - (pluginUpdateManager.updateSettings.checkInterval + 1000)); // Intervalo pasado
            if (key === settingsKey) return Promise.resolve({ ...pluginUpdateManager.updateSettings, checkAutomatically: true }); // Habilitado
            return Promise.resolve({});
        });
        pluginRepositoryManager.getRepositories.mockReturnValueOnce({ 'official': mockRepo }); // Proveer un repo
  ...
  describe('checkForUpdates', () => {
    ...
    beforeEach(async () => {
      ...
      storageService.get.mockImplementation(async (key) => { // Mock base para settings
         if (key === settingsKey) return Promise.resolve({ ...pluginUpdateManager.updateSettings, checkAutomatically: false });
         return Promise.resolve({});
      });
      pluginRepositoryManager.getRepositories.mockReturnValue({ 'official': mockRepo });
      await pluginUpdateManager.initialize();
      ...
      checkPluginInRepositorySpy = jest.spyOn(pluginUpdateManager, '_checkPluginInRepository').mockImplementation(async () => null);
    });
    afterEach(() => {
    ...
    test('debe verificar actualizaciones y encontrar una disponible', async () => {
      ...
      expect(checkPluginInRepositorySpy).toHaveBeenCalledWith(pluginId, installedPluginInfo.version, 'official');
      expect(updates[pluginId]).toBeDefined();
      expect(updates[pluginId].newVersion).toBe('1.1.0');
      expect(storageService.set).toHaveBeenCalledWith(availableUpdatesKey, expect.any(Object));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateCheckStarted', expect.any(Object));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateAvailable', expect.objectContaining({ pluginId }));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateCheckCompleted', expect.any(Object));
    });
    ...
    test('no debe encontrar actualizaciones si no las hay', async () => {
      ...
      expect(eventBus.publish).not.toHaveBeenCalledWith('pluginSystem.updateAvailable', expect.any(Object));
    });
    ...
    test('debe lanzar error si no hay repositorios configurados (controlado por mock)', async () => {
        ...
        await expect(pluginUpdateManager.checkForUpdates()).rejects.toThrow('No hay repositorios configurados');
    });
  });

  describe('applyUpdate', () => {
    let downloadPluginPackageSpy;
    const mockPackageContent = { manifest: remotePluginInfo, files: { 'index.js': 'code' } };
    ...
    beforeEach(async () => {
      ...
      pluginRepositoryManager.getRepositories.mockReturnValue({ 'official': mockRepo }); // Para initialize
      await pluginUpdateManager.initialize();
      ...
      downloadPluginPackageSpy = jest.spyOn(pluginUpdateManager, '_downloadPluginPackage').mockResolvedValue(mockPackageContent);
    ...
    test('debe aplicar una actualización disponible', async () => {
      ...
      expect(downloadPluginPackageSpy).toHaveBeenCalledWith(pluginId, '1.1.0', 'official');
      expect(pluginPackageManager.uninstallPlugin).toHaveBeenCalledWith(pluginId, { keepSettings: true });
      expect(pluginPackageManager.installPlugin).toHaveBeenCalledWith(mockPackageContent, { update: true });
      ...
      expect(pluginUpdateManager.updateHistory[pluginId][0].toVersion).toBe('1.1.0');
      expect(pluginUpdateManager.availableUpdates[pluginId]).toBeUndefined(); 
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateStarted', expect.objectContaining({ pluginId }));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateCompleted', expect.objectContaining({ pluginId }));
    });

    test('debe lanzar error si no hay actualización disponible', async () => {
      delete pluginUpdateManager.availableUpdates[pluginId]; 
      await expect(pluginUpdateManager.applyUpdate(pluginId))
        .rejects.toThrow(`No hay actualización disponible para el plugin: ${pluginId}`);
    });
  });
  ...
  describe('applyAllUpdates', () => {
    ...
    test('debe intentar aplicar todas las actualizaciones disponibles', async () => {
        pluginUpdateManager.availableUpdates = {
            'p1': { id: 'p1', currentVersion: '1.0', newVersion: '1.1', repositoryId: 'repo1', releaseNotes: 'notes p1' },
            'p2': { id: 'p2', currentVersion: '2.0', newVersion: '2.1', repositoryId: 'repo1', releaseNotes: 'notes p2' }
        };
        pluginRepositoryManager.getRepository.mockReturnValue(mockRepo);
        const mockPackageP1 = { manifest: { id: 'p1', version: '1.1' }, files: {} };
        const mockPackageP2 = { manifest: { id: 'p2', version: '2.1' }, files: {} };
        
        const downloadSpy = jest.spyOn(pluginUpdateManager, '_downloadPluginPackage')
        ...
        const applyUpdateSpy = jest.spyOn(pluginUpdateManager, 'applyUpdate')
            .mockImplementation(async (id) => {
                if (id === 'p1') {
        ...
        expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.massUpdateStarted', expect.any(Object));
        expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.massUpdateCompleted', expect.any(Object));
  ...
  describe('configureUpdateSettings', () => {
    ...
    test('debe actualizar y guardar la configuración', async () => {
      const newSettings = { checkAutomatically: false, autoUpdate: true, checkInterval: 120000 };
      ...
      expect(storageService.set).toHaveBeenCalledWith(settingsKey, pluginUpdateManager.updateSettings);
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.updateSettingsChanged', { settings: pluginUpdateManager.updateSettings });
    });
  });
  ...
  describe('Utility methods', () => {
    ...
    test('getAvailableUpdates debe devolver las actualizaciones actuales', () => {
      pluginUpdateManager.availableUpdates = { 'test': { id: 'test', version: '2.0' }}; 
      expect(pluginUpdateManager.getAvailableUpdates()).toEqual({ 'test': { id: 'test', version: '2.0' }});
    });

    test('getUpdateHistory debe devolver el historial', () => {
      pluginUpdateManager.updateHistory = { 'test': [{ version: '1.0' }]};
      expect(pluginUpdateManager.getUpdateHistory()).toEqual({ 'test': [{ version: '1.0' }]});
    });
