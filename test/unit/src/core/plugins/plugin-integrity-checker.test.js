...
describe("PluginIntegrityChecker", () => {
  ...
  describe("generateChecksums", () => {
    test("debe generar checksums para los archivos proporcionados", async () => {
      ...
      expect(checksums["index.js"]).toEqual(
        testSimulateChecksum("index.js", files["index.js"])
      );
      expect(checksums["style.css"]).toEqual(
        testSimulateChecksum("style.css", files["style.css"])
      );
    });
    ...
    test("debe lanzar error si la generación falla (simulado)", async () => {
      ...
      pluginIntegrityChecker._simulateChecksum = jest.fn(() => {
        throw new Error("Checksum failed");
      });

      const files = { "file.js": "content" };
      await expect(
        pluginIntegrityChecker.generateChecksums(files)
      ).rejects.toThrow("Checksum failed");
      expect(console.error).toHaveBeenCalledWith(
        "Error al generar checksums:",
        expect.any(Error)
      );
  ...
  describe("signPackage", () => {
    test("debe simular la firma de un manifiesto", async () => {
      ...
      const signature = await pluginIntegrityChecker.signPackage(
        manifest,
        signingKey
      );
  ...
  describe("verifyPackage", () => {
    ...
    beforeEach(() => {
      ...
      validPackage = {
        manifest: {
          ...
          checksums: {
            ...
            "manifest.json": testSimulateChecksum(
              "manifest.json",
              files["manifest.json"]
            ),
          },
      ...
      jest
        .spyOn(pluginIntegrityChecker, "_verifySignature")
    ...
    test("debe verificar un paquete válido (con firma válida por mock)", async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.integrityVerified",
        {
          pluginId: "pluginA",
          isValid: true,
      ...
      expect(
        pluginIntegrityChecker.getVerificationResults("pluginA").isValid
      ).toBe(true);
    });
    ...
    test("debe fallar si faltan checksums en el manifiesto", async () => {
      const pkg = {
        ...validPackage,
        manifest: { ...validPackage.manifest, checksums: {} },
      };
      const isValid = await pluginIntegrityChecker.verifyPackage(pkg);
      expect(isValid).toBe(false);
      expect(
        pluginIntegrityChecker.getVerificationResults("pluginA").reason
      ).toBe("No se encontraron checksums");
    });
    ...
    test("debe fallar si un archivo listado en checksums no existe en files", async () => {
      ...
      expect(
        pluginIntegrityChecker.getVerificationResults("pluginA").reason
      ).toBe("El archivo missing.js no existe en el paquete");
    });
    ...
    test("debe fallar si el checksum de un archivo no coincide", async () => {
      ...
      expect(
        pluginIntegrityChecker.getVerificationResults("pluginA").reason
      ).toBe("Checksum inválido para index.js");
    });
  ...
  describe("Trusted Keys Management", () => {
    test("addTrustedKey debe añadir una clave y removeTrustedKey debe eliminarla", () => {
      const resultAdd = pluginIntegrityChecker.addTrustedKey(
        "key1",
        "publicKeyContent",
        "Test Key"
      );
      ...
      expect(pluginIntegrityChecker.getTrustedKeys()["key1"].description).toBe(
        "Test Key"
      );
      ...
      expect(pluginIntegrityChecker.removeTrustedKey("nonExistentKey")).toBe(
        false
      );
    });
  });
  ...
  describe("getVerificationResults", () => {
    test("debe devolver resultados específicos de un plugin o todos los resultados", async () => {
      await pluginIntegrityChecker.verifyPackage({
        manifest: { id: "p1" },
        files: {},
      }); // Falla
      await pluginIntegrityChecker.verifyPackage({
        manifest: { id: "p2", checksums: {} },
        files: {},
      }); // Falla

      expect(pluginIntegrityChecker.getVerificationResults("p1")).toBeDefined();
      expect(pluginIntegrityChecker.getVerificationResults("p1").isValid).toBe(
        false
      );
      expect(
        pluginIntegrityChecker.getVerificationResults("nonExistent")
      ).toBeNull();
