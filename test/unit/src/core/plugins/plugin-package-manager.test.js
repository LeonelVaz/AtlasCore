...
jest.mock("../../../../../src/core/plugins/plugin-integrity-checker", () => ({
  generateChecksums: jest.fn(() =>
    Promise.resolve({ "file.js": "checksum123" })
  ),
...
const {
  validatePluginComplete,
} = require("../../../../../src/core/plugins/plugin-validator");
...
describe("PluginPackageManager", () => {
  ...
  describe("initialize", () => {
    test("debe cargar datos desde storageService", async () => {
      ...
      expect(storageService.get).toHaveBeenCalledWith(
        "atlas_plugin_installed_plugins",
        {}
      );
      expect(storageService.get).toHaveBeenCalledWith(
        "atlas_plugin_package_manifests",
        {}
      );
  ...
  describe("packagePlugin", () => {
    test("debe empaquetar un plugin válido", async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.pluginPackaged",
        expect.any(Object)
      );
      expect(pluginPackageManager.packageManifests["testPlugin"]).toEqual(
        pkg.manifest
      );
    });
    ...
    test("debe firmar el paquete si se proporciona signingKey", async () => {
      ...
      expect(pluginIntegrityChecker.signPackage).toHaveBeenCalledWith(
        expect.anything(),
        "mykey"
      );
    ...
    test("debe lanzar error si el plugin no se encuentra en el registro", async () => {
      ...
      await expect(
        pluginPackageManager.packagePlugin("unknownPlugin")
      ).rejects.toThrow("Plugin no encontrado: unknownPlugin");
    });

    test("debe lanzar error si la validación del plugin falla", async () => {
      validatePluginComplete.mockReturnValueOnce({
        valid: false,
        reason: "Validation failed",
      });
      await expect(
        pluginPackageManager.packagePlugin("testPlugin")
  ...
  describe("installPlugin", () => {
    ...
    test("debe instalar un plugin desde un paquete válido", async () => {
      ...
      expect(pluginIntegrityChecker.verifyPackage).toHaveBeenCalledWith(
        mockPackage
      );
      expect(
        pluginPackageManager.installedPlugins["installTest"]
      ).toBeDefined();
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.pluginInstallStarted",
        expect.any(Object)
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
    ...
    test("debe lanzar error si el paquete no supera la verificación de integridad", async () => {
      ...
      await expect(
        pluginPackageManager.installPlugin(mockPackage)
    ...
    test("debe lanzar error si el plugin ya está instalado y no es una actualización", async () => {
      ...
      await expect(
        pluginPackageManager.installPlugin(mockPackage, { update: false })
      ) // Explicitly update: false
        .rejects.toThrow(
          "Ya existe un plugin instalado con el ID: installTest"
        );
    });
    ...
    test("debe permitir la instalación si es una actualización (update: true)", async () => {
      ...
      jest
        .spyOn(pluginPackageManager, "_checkDependencies")
        .mockResolvedValue([]);
    ...
    test("debe lanzar error si la versión de la app no es compatible", async () => {
      const incompatiblePkg = {
        ...mockPackage,
        manifest: { ...mockPackage.manifest, minAppVersion: "0.5.0" },
      };
      // PLUGIN_CONSTANTS.CURRENT_APP_VERSION es '0.4.0'
      await expect(
        pluginPackageManager.installPlugin(incompatiblePkg)
      ).rejects.toThrow(
        "El plugin no es compatible con la versión actual 0.4.0"
      );
    });
  });
  ...
  describe("uninstallPlugin", () => {
    beforeEach(() => {
      ...
      pluginPackageManager.installedPlugins["testPlugin"] = {
        id: "testPlugin",
        version: "1.0.0",
      };
    });
    ...
    test("debe desinstalar un plugin instalado", async () => {
      ...
      jest
        .spyOn(pluginPackageManager, "_deactivatePlugin")
        .mockResolvedValue(true);

      const result = await pluginPackageManager.uninstallPlugin("testPlugin");
      expect(result).toBe(true);
      expect(pluginPackageManager._deactivatePlugin).toHaveBeenCalledWith(
        "testPlugin"
      );
      expect(pluginRegistry.unregisterPlugin).toHaveBeenCalledWith(
        "testPlugin"
      );
      expect(
        pluginPackageManager.installedPlugins["testPlugin"]
      ...
      expect(storageService.remove).toHaveBeenCalledWith(
        "atlas_plugin_testPlugin_files"
      ); // Simulación de _removePluginFiles
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.pluginUninstallStarted",
        { pluginId: "testPlugin" }
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
    ...
    test("debe lanzar error si el plugin no está instalado", async () => {
      await expect(
        pluginPackageManager.uninstallPlugin("unknownPlugin")
    ...
    test("debe lanzar error si hay dependientes y no se fuerza la desinstalación", async () => {
      jest
        .spyOn(pluginPackageManager, "_getDependentPlugins")
        .mockResolvedValueOnce(["dependentPlugin"]);
      await expect(
        pluginPackageManager.uninstallPlugin("testPlugin", { force: false })
  ...
  describe("Utility Methods", () => {
    test("getInstalledPlugins debe devolver la lista de plugins instalados", () => {
      ...
      expect(pluginPackageManager.getInstalledPlugins()).toEqual({
        p1: { id: "p1" },
        p2: { id: "p2" },
      });
    });
    ...
    test("isPluginInstalled debe verificar correctamente", () => {
      ...
      expect(pluginPackageManager.isPluginInstalled("pNonExistent")).toBe(
        false
      );
    });

    test("hasUpdate debe comparar versiones correctamente", () => {
      pluginPackageManager.installedPlugins["p1"] = {
        id: "p1",
        version: "1.0.0",
      };
      expect(
        pluginPackageManager.hasUpdate("p1", { id: "p1", version: "1.1.0" })
      ).toBe(true);
      expect(
        pluginPackageManager.hasUpdate("p1", { id: "p1", version: "1.0.0" })
      ).toBe(false);
      expect(
        pluginPackageManager.hasUpdate("p1", { id: "p1", version: "0.9.0" })
      ).toBe(false);
      expect(
        pluginPackageManager.hasUpdate("pNonExistent", {
          id: "p1",
          version: "1.1.0",
        })
      ).toBe(false);
