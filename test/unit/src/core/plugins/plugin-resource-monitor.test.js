...
describe("PluginResourceMonitor - Enhanced Coverage", () => {
  ...
  beforeEach(() => {
    ...
    pluginResourceMonitor.securityLevel =
  ...
  describe("initialize - casos edge", () => {
    test("debería manejar doble inicialización con mismo nivel", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "[ResourceMonitor] Monitor de recursos ya inicializado."
      );
    });
    ...
    test("debería manejar doble inicialización con nivel diferente", () => {
      ...
      expect(result).toBe(true);
      expect(pluginResourceMonitor.securityLevel).toBe(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH
      );
    });
    ...
    test("debería manejar error durante inicialización", () => {
      ...
      const originalStartPeriodicCleanup =
        pluginResourceMonitor._startPeriodicCleanup;
  ...
  describe("trackOperation - casos edge", () => {
    ...
    test("debería retornar función no-op cuando pluginId es null", () => {
      ...
      expect(Object.keys(pluginResourceMonitor.operationCounts)).toHaveLength(
        0
      );
    });
  ...
  describe("trackNetworkRequest", () => {
    ...
    test("debería rastrear requests de red correctamente", () => {
      ...
      expect(
        pluginResourceMonitor.operationCounts[pluginId].networkRequests
      ).toBe(1);
      expect(
        pluginResourceMonitor.operationCounts[pluginId].totalOperations
      ).toBe(1);
    });
  ...
  describe("_checkResourceLimits - casos edge", () => {
    ...
    test("debería manejar nivel de seguridad inválido", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("No se encontraron límites de recursos")
      );
    });

    test("debería verificar límites de memoria y storage", () => {
      const limits =
        pluginResourceMonitor.resourceLimits[
          PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL
        ];
  ...
  describe("_handleResourceLimitViolations", () => {
    ...
    test("debería crear estructura de resourceUsage si no existe", () => {
      ...
      pluginResourceMonitor._handleResourceLimitViolations(
        pluginId,
        violations
      );
      ...
      expect(
        pluginResourceMonitor.resourceUsage[pluginId].violations
      ).toHaveLength(1);
    });
    ...
    test("debería solicitar desactivación para violaciones críticas en nivel HIGH", () => {
      pluginResourceMonitor.setSecurityLevel(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH
      );
      const limits =
        pluginResourceMonitor.resourceLimits[
  ...
  describe("applyRestrictions y removeRestrictions", () => {
    ...
    test("debería aplicar restricciones correctamente", () => {
      ...
      expect(
        pluginResourceMonitor.enhancedMonitoringPlugins.has(pluginId)
      ).toBe(true);
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.pluginRestricted",
        {
          pluginId,
    ...
    test("debería remover restricciones correctamente", () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.pluginUnrestricted",
        { pluginId }
      );
    });
  ...
  describe("increaseMonitoring y decreaseMonitoring", () => {
    ...
    test("debería manejar error al reducir monitoreo", () => {
      const originalDelete =
        pluginResourceMonitor.enhancedMonitoringPlugins.delete;
  ...
  describe("setSecurityLevel - casos edge", () => {
    test("debería manejar nivel null", () => {
      ...
      expect(result).toBe(false);
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("Nivel de seguridad inválido")
      );
    });
    ...
    test("debería retornar true para mismo nivel cuando está inicializado", () => {
      ...
      const result = pluginResourceMonitor.setSecurityLevel(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL
      );
  ...
  describe("updateSecurityChecks", () => {
    ...
    test("debería manejar input null", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("debe ser un array")
      );
    });
  ...
  describe("_getEffectiveLimits", () => {
    ...
    test("debería manejar nivel de seguridad inválido", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("No se pudieron obtener los límites base")
      );
    });
  });
  ...
  describe("fetch wrapper integration", () => {
    beforeEach(() => {
      pluginResourceMonitor.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
      pluginResourceMonitor.updateSecurityChecks([
        "resourceUsage",
        "networkRequests",
      ]);
    });
  ...
  describe("_resetOperationCounters", () => {
    ...
    test("debería resetear contadores y crear historial", () => {
      ...
      expect(
        pluginResourceMonitor.resourceUsage[pluginId].history
      ).toHaveLength(1);
      expect(
        pluginResourceMonitor.resourceUsage[pluginId].history[0].operations
          .totalOperations
      ).toBe(11);
    });
