...
jest.mock('../../../../../src/core/bus/event-bus', () => ({
  ...
  subscribe: jest.fn((eventName, callback) => {
    ...
    if (!global.mockEventBusSubscriptions) global.mockEventBusSubscriptions = {};
    if (!global.mockEventBusSubscriptions[eventName]) global.mockEventBusSubscriptions[eventName] = [];
    global.mockEventBusSubscriptions[eventName].push({ callback, unsubscribe: mockUnsubscribe });
...
describe('PluginEvents', () => {
  ...
  describe('subscribe', () => {
    test('debe suscribirse a un evento y registrar la suscripción', () => {
      ...
      const unsubscribe = pluginEvents.subscribe('pluginA', 'testEvent', callback);

      expect(eventBus.subscribe).toHaveBeenCalledWith('plugin.testEvent', expect.any(Function));
      ...
      expect(pluginEvents.pluginSubscriptions['pluginA'][subId].eventName).toBe('testEvent');
    ...
    test('debe llamar al callback del plugin cuando eventBus publica un evento', () => {
      ...
      eventBusCallbackWrapper({ sourcePlugin: 'pluginB', data: { message: 'hello' } });
    ...
    test('debe manejar errores en el callback y publicar en pluginSystem.eventHandlerError', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.eventHandlerError', {
        pluginId: 'pluginA',
    ...
    test('debe devolver una función de desuscripción que llama a unsubscribe del módulo', () => {
      const callback = jest.fn();
      const unsubscribeFn = pluginEvents.subscribe('pluginA', 'testEvent', callback);
      const subId = Object.keys(pluginEvents.pluginSubscriptions['pluginA'])[0];
      
      jest.spyOn(pluginEvents, 'unsubscribe');
      unsubscribeFn();
      expect(pluginEvents.unsubscribe).toHaveBeenCalledWith('pluginA', parseInt(subId, 10));
    });
    ...
    test('debe loguear error y devolver función vacía si los argumentos son inválidos', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Argumentos inválidos para subscribe');
  ...
  describe('unsubscribe', () => {
    test('debe eliminar una suscripción y llamar a la función de desuscripción de eventBus', () => {
      ...
      pluginEvents.subscribe('pluginA', 'testEvent', callback);
      const subId = Object.keys(pluginEvents.pluginSubscriptions['pluginA'])[0];
      const mockEventBusUnsubscribe = global.mockEventBusSubscriptions['plugin.testEvent'][0].unsubscribe;
  ...
  describe('publish', () => {
    test('debe publicar un evento en eventBus con el prefijo y sourcePlugin', () => {
      ...
      expect(eventBus.publish).not.toHaveBeenCalledWith('plugin.*', expect.anything());
    });
    ...
    test('debe devolver false y loguear error si los argumentos son inválidos', () => {
        ...
        expect(console.error).toHaveBeenCalledWith('Argumentos inválidos para publish');
    });
  });
  ...
  describe('unsubscribeAll', () => {
    test('debe eliminar todas las suscripciones de un plugin', () => {
      ...
      const mockUnsub1 = global.mockEventBusSubscriptions['plugin.event1'][0].unsubscribe;
      const mockUnsub2 = global.mockEventBusSubscriptions['plugin.event2'][0].unsubscribe;
  ...
  describe('getStats', () => {
    test('debe devolver estadísticas correctas sobre las suscripciones', () => {
      ...
      expect(stats.eventsWithListeners).toEqual(expect.arrayContaining(['eventA1', 'eventA2', 'eventB1']));
