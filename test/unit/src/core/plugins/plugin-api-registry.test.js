...
describe('PluginAPIRegistry', () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.getPlugin.mockImplementation(pluginId => ({ id: pluginId, name: pluginId, dependencies: [] }));
  ...
  describe('registerAPI', () => {
    test('debe registrar una API y publicar evento', () => {
      ...
      expect(pluginAPIRegistry.publicAPIs['pluginA'].methods).toEqual(['method1']);
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.apiRegistered', {
        pluginId: 'pluginA',
        methods: ['method1'],
      });
      expect(console.log).toHaveBeenCalledWith('API pública registrada para plugin: pluginA');
    });
    ...
    test('no debe registrar API si el plugin no está registrado en pluginRegistry', () => {
      ...
      const result = pluginAPIRegistry.registerAPI('pluginB', { test: () => {} });
      expect(result).toBe(false);
      expect(console.error).toHaveBeenCalledWith('No se puede registrar API para plugin no registrado: pluginB');
    });
    ...
    test('debe advertir si se sobrescribe una API existente', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('Sobrescribiendo API existente para plugin: pluginA');
      expect(console.log).toHaveBeenCalledWith('API pública registrada para plugin: pluginA'); 
    });

    test('debe manejar errores durante el registro y llamar a pluginErrorHandler', () => {
      pluginRegistry.getPlugin.mockImplementationOnce(() => { throw new Error('Registry error during getPlugin'); });
      const result = pluginAPIRegistry.registerAPI('pluginC', { test: () => {} });
      expect(result).toBe(false);
  ...
  describe('callPluginMethod', () => {
    ...
    beforeEach(() => {
      ...
      pluginRegistry.getPlugin.mockImplementation(id => {
        if (id === 'pluginA') return { id: 'pluginA', name: 'Plugin A', dependencies: [] };
        if (id === 'pluginB') return { id: 'pluginB', name: 'Plugin B', dependencies: ['pluginA'] };
        if (id === 'pluginC') return { id: 'pluginC', name: 'Plugin C', dependencies: [] };
        return null;
      });
    ...
    test('debe llamar a un método de la API de otro plugin si tiene permiso (dependencia)', () => {
      const result = pluginAPIRegistry.callPluginMethod('pluginB', 'pluginA', 'methodA', [1, 2]);
      ...
      expect(log[0]).toMatchObject({ caller: 'pluginB', target: 'pluginA', method: 'methodA', success: true });
    });

    test('debe permitir a "app" (o caller no plugin) llamar a cualquier API', () => {
      const result = pluginAPIRegistry.callPluginMethod('app', 'pluginA', 'methodA');
      expect(mockMethodAImpl).toHaveBeenCalled();
      expect(result).toBe('resultFromA');
    });
    
    test('debe permitir a un plugin llamar a su propia API', () => {
        const result = pluginAPIRegistry.callPluginMethod('pluginA', 'pluginA', 'methodA');
        expect(mockMethodAImpl).toHaveBeenCalled();
        expect(result).toBe('resultFromA');
    });
    ...
    test('debe lanzar error "Plugin no encontrado..." si targetAPI es null porque el plugin no tiene API registrada', () => {
      ...
      pluginRegistry.getPlugin.mockImplementation(id => { // Asegurar que pluginD "existe" como plugin
          if (id === 'pluginD') return {id: 'pluginD', dependencies: []};
          return {id, dependencies: []}; // Para otros como pluginB
      });
      expect(() => pluginAPIRegistry.callPluginMethod('pluginB', 'pluginD', 'someMethod'))
    ...
    test('debe lanzar error "Método no encontrado..." si targetAPI existe pero el método no', () => {
      ...
      expect(() => pluginAPIRegistry.callPluginMethod('pluginB', 'pluginA', 'nonExistentMethod'))
    ...
    test('debe lanzar error y loguear si no hay permiso (sin dependencia)', () => {
        ...
        pluginAPIRegistry._checkAPIAccess = jest.fn((caller, target, method) => {
            ...
            return originalCheckAPIAccess.call(pluginAPIRegistry, caller, target, method); // Usar original para otros
        });

        expect(() => pluginAPIRegistry.callPluginMethod('pluginC', 'pluginA', 'methodA'))
          .toThrow('Acceso denegado: pluginC no tiene permiso para acceder a pluginA.methodA');
        
        expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.unauthorizedAPIAccess', 
          expect.objectContaining({ callerPluginId: 'pluginC', targetPluginId: 'pluginA', method: 'methodA' })
        );
        const log = pluginAPIRegistry.getAccessLog('pluginC');
        expect(log.length).toBeGreaterThan(0);
        expect(log[0]).toMatchObject({ success: false, error: expect.stringContaining('Acceso denegado') });
  ...
  describe('unregisterAPI', () => {
    test('debe eliminar una API registrada y publicar evento', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.apiUnregistered', { pluginId: 'pluginA' });
    });
