...
describe('PluginRepositoryManager', () => {
  ...
  const syncTimestampsKey = STORAGE_KEYS.PLUGIN_DATA_PREFIX + 'repository_sync_timestamps';
  ...
  beforeEach(async () => {
    ...
    jest.clearAllMocks();
     storageService.get.mockImplementation(async (key, defaultValue) => {
      if (key === officialRepoKey) return Promise.resolve({'atlas-official': {...mockOfficialRepo, addedAt: Date.now()}});
      if (key === cacheKey) return Promise.resolve({});
      if (key === syncTimestampsKey) return Promise.resolve({});
      return Promise.resolve(defaultValue);
    });
  ...
  describe('initialize', () => {
    test('debe inicializar y crear repositorio oficial si no existe', async () => {
      ...
      expect(pluginRepositoryManager.repositories['atlas-official']).toBeDefined();
      expect(pluginRepositoryManager.repositories['atlas-official'].official).toBe(true);
    ...
    test('debe cargar datos de repositorios desde storageService', async () => {
        const storedRepos = { 'custom-repo': { id: 'custom-repo', name: 'Custom', url: 'http://custom.com' } };
        ...
        expect(pluginRepositoryManager.repositories['custom-repo']).toEqual(storedRepos['custom-repo']);
    });
  });

  describe('addRepository', () => {
    const newRepo = { id: 'repo1', name: 'Repo 1', url: 'http://repo1.com', apiEndpoint: 'http://repo1.com/api' };
    ...
    beforeEach(() => {
      ...
      pluginRepositoryManager._validateRepository = jest.fn().mockResolvedValue(true);
    });
    afterEach(() => {
    ...
    test('debe añadir un repositorio válido y sincronizarlo', async () => {
      jest.spyOn(pluginRepositoryManager, 'syncRepository').mockResolvedValueOnce([]); // Mock sync
      ...
      expect(storageService.set).toHaveBeenCalledWith(officialRepoKey, expect.any(Object));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.repositoryAdded', expect.any(Object));
      expect(pluginRepositoryManager.syncRepository).toHaveBeenCalledWith('repo1');
    });

    test('debe lanzar error si la información del repositorio es incompleta', async () => {
      await expect(pluginRepositoryManager.addRepository({ id: 'incomplete' }))
    ...
    test('debe lanzar error si el repositorio ya existe', async () => {
     ...
     test('debe lanzar error si _validateRepository devuelve false', async () => {
      ...
      await expect(pluginRepositoryManager.addRepository(newRepo))
        .rejects.toThrow('No se pudo validar el repositorio. URL no accesible o formato incorrecto.');
    });
  });

  describe('updateRepository', () => {
    const repoToUpdate = { id: 'atlas-official', name: 'Official Updated', url: 'http://newofficial.com' };
     beforeEach(() => {
      pluginRepositoryManager._validateRepository = jest.fn().mockResolvedValue(true);
    });

    test('debe actualizar un repositorio existente', async () => {
      const result = await pluginRepositoryManager.updateRepository('atlas-official', { name: 'Official Updated Name' });
      expect(result).toBe(true);
      expect(pluginRepositoryManager.repositories['atlas-official'].name).toBe('Official Updated Name');
      expect(storageService.set).toHaveBeenCalledWith(officialRepoKey, expect.any(Object));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.repositoryUpdated', expect.any(Object));
    });

    test('debe lanzar error si el repositorio no existe', async () => {
      await expect(pluginRepositoryManager.updateRepository('nonexistent', { name: 'Update Fail' }))
  ...
  describe('removeRepository', () => {
    test('debe eliminar un repositorio no oficial', async () => {
      ...
      pluginRepositoryManager._validateRepository = jest.fn().mockResolvedValue(true);
      const customRepo = { id: 'custom', name: 'Custom Repo', url: 'http://custom.com' };
      ...
      expect(storageService.set).toHaveBeenCalledWith(officialRepoKey, expect.any(Object)); // Al guardar repos
      expect(storageService.remove).not.toHaveBeenCalled(); // Asume que no había cache ni timestamps para este nuevo repo
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.repositoryRemoved', { repositoryId: 'custom' });
    });

    test('no debe permitir eliminar el repositorio oficial', async () => {
      await expect(pluginRepositoryManager.removeRepository('atlas-official'))
  ...
  describe('syncRepository', () => {
    ...
    const mockPluginsData = [{ id: 'plugin1', name: 'Plugin 1', version: '1.0.0' }];

    beforeEach(() => {
      originalFetchPlugins = pluginRepositoryManager._fetchRepositoryPlugins;
      pluginRepositoryManager._fetchRepositoryPlugins = jest.fn().mockResolvedValue(mockPluginsData);
    });
    afterEach(() => {
    ...
    test('debe sincronizar un repositorio y actualizar la cache', async () => {
      const plugins = await pluginRepositoryManager.syncRepository('atlas-official');
      expect(plugins).toEqual(mockPluginsData);
      expect(pluginRepositoryManager.repositoryCache['atlas-official'].plugins).toEqual(mockPluginsData);
      expect(pluginRepositoryManager.lastSyncTimestamps['atlas-official']).toBeDefined();
      expect(storageService.set).toHaveBeenCalledWith(cacheKey, expect.any(Object));
      expect(storageService.set).toHaveBeenCalledWith(syncTimestampsKey, expect.any(Object));
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.repositorySyncStarted', { repositoryId: 'atlas-official' });
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.repositorySyncCompleted', expect.objectContaining({ repositoryId: 'atlas-official' }));
    });
    ...
    test('debe lanzar error si el repositorio está deshabilitado', async () => {
        ...
        await expect(pluginRepositoryManager.syncRepository('atlas-official'))
  ...
  describe('searchPlugins', () => {
    const mockPluginList = [
        { id: 'search-plugin-1', name: 'Awesome Search Plugin', description: 'Does awesome things', tags: ['search', 'utility'] },
        { id: 'another-plugin', name: 'Another Utility', description: 'Some other description', tags: ['utility'] }
    ];
    beforeEach(() => {
        // Mock getRepositoryPlugins para devolver una lista controlada
        jest.spyOn(pluginRepositoryManager, 'getRepositoryPlugins').mockResolvedValue(mockPluginList);
    });
    afterEach(() => {
    ...
    test('debe buscar plugins por nombre', async () => {
        ...
        expect(pluginRepositoryManager.getRepositoryPlugins).toHaveBeenCalledWith('atlas-official'); // Asume que solo atlas-official está habilitado
    });

    test('debe buscar plugins por descripción', async () => {
        const results = await pluginRepositoryManager.searchPlugins('other description');
        expect(results.length).toBe(1);
    ...
    test('debe devolver un array vacío si no hay coincidencias', async () => {
        const results = await pluginRepositoryManager.searchPlugins('nonexistentqueryterm');
