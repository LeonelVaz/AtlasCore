...
describe("PluginSecurityManager - Enhanced Coverage", () => {
  ...
  beforeEach(() => {
    ...
    pluginSecurityManager.securityLevel =
  ...
  describe("initialize - casos edge", () => {
    test("debería manejar doble inicialización", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "El gestor de seguridad ya está inicializado"
      );
    });
    ...
    test("debería manejar error durante inicialización", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error al inicializar sistema de seguridad:",
        expect.any(Error)
      );
    ...
    test("debería configurar listeners de eventos correctamente", () => {
      ...
      expect(pluginSecurityManager.activeSecurityChecks.size).toBeGreaterThan(
        0
      );
    });
  });
  ...
  describe("_calculateEventSeverity", () => {
    ...
    test("debería calcular severidad base correctamente", () => {
      expect(
        pluginSecurityManager._calculateEventSeverity("unauthorizedAccess", {})
      ).toBe("high");
      expect(
        pluginSecurityManager._calculateEventSeverity("unsafeCodeExecution", {})
      ).toBe("critical");
      expect(
        pluginSecurityManager._calculateEventSeverity(
          "suspiciousExternalCommunication",
          {}
        )
      ).toBe("high");
      expect(
        pluginSecurityManager._calculateEventSeverity(
          "suspiciousDomOperation",
          {}
        )
      ).toBe("medium");
      expect(
        pluginSecurityManager._calculateEventSeverity("resourceOveruse", {})
      ).toBe("medium");
      expect(
        pluginSecurityManager._calculateEventSeverity("unknownEvent", {})
      ).toBe("low");
    });

    test("debería escalar severidad por comportamiento repetido", () => {
      const result = pluginSecurityManager._calculateEventSeverity(
        "resourceOveruse",
        { repeated: 5 }
      );
  ...
  describe("_escalateSeverity", () => {
    test("debería escalar severidad correctamente", () => {
      ...
      expect(pluginSecurityManager._escalateSeverity("critical")).toBe(
        "critical"
      ); // máximo
    });
  });
  ...
  describe("_takeActionForSecurityEvent - todos los casos de severidad", () => {
    beforeEach(() => {
      pluginSecurityManager.initialize();
      jest
        .spyOn(pluginSecurityManager, "deactivatePlugin")
        .mockImplementation(() => true);
      jest
        .spyOn(pluginSecurityManager, "blacklistPlugin")
        .mockImplementation(() => true);
    });
    ...
    test("debería manejar evento crítico", () => {
      const event = {
        pluginId,
        severity: "critical",
        type: "unsafeCodeExecution",
      };

      pluginSecurityManager._takeActionForSecurityEvent(event);

      expect(pluginSecurityManager.deactivatePlugin).toHaveBeenCalledWith(
        pluginId,
        expect.stringContaining("Amenaza de seguridad crítica")
      );
      expect(pluginSecurityManager.blacklistPlugin).toHaveBeenCalledWith(
        pluginId
      );
    });
    ...
    test("debería manejar evento alto con múltiples advertencias", () => {
      ...
      expect(pluginSecurityManager.deactivatePlugin).toHaveBeenCalledWith(
        pluginId,
        expect.stringContaining("Múltiples amenazas de seguridad altas")
      );
    });
    ...
    test("debería manejar evento alto sin múltiples advertencias", () => {
      ...
      expect(pluginResourceMonitor.applyRestrictions).toHaveBeenCalledWith(
        pluginId
      );
    ...
    test("debería manejar evento medio con muchas advertencias", () => {
      ...
      pluginSecurityManager.pluginsWithWarnings[pluginId] = Array(6).fill({
        type: "warning",
      });
    ...
    test("debería manejar evento bajo", () => {
      ...
      expect(pluginResourceMonitor.increaseMonitoring).toHaveBeenCalledWith(
        pluginId
      );
    });
  });
  ...
  describe("_addWarningToPlugin y _getPluginWarningsCount", () => {
    test("debería agregar advertencia a plugin", () => {
      ...
      expect(pluginSecurityManager.pluginsWithWarnings[pluginId]).toHaveLength(
        1
      );
      expect(pluginSecurityManager.pluginsWithWarnings[pluginId][0].type).toBe(
        "test"
      );
    });
    ...
    test("debería manejar plugin ID null/undefined", () => {
      ...
      expect(
        Object.keys(pluginSecurityManager.pluginsWithWarnings)
      ).toHaveLength(0);
    });
    ...
    test("debería contar advertencias correctamente", () => {
      ...
      expect(pluginSecurityManager._getPluginWarningsCount("nonexistent")).toBe(
        0
      );
  ...
  describe("validatePlugin - casos edge", () => {
    ...
    test("debería manejar sistema no inicializado", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "Sistema de seguridad no inicializado"
      );
    });
  ...
  describe("deactivatePlugin", () => {
    ...
    test("debería manejar plugin ID null/undefined", () => {
      ...
      expect(pluginSecurityManager.deactivatePlugin(undefined, "test")).toBe(
        false
      );
    });
    ...
    test("debería manejar plugin ya desactivado", () => {
      ...
      const result = pluginSecurityManager.deactivatePlugin(
        pluginId,
        "test reason"
      );
  ...
  describe("_calculatePluginSecurityScore", () => {
    ...
    test("debería retornar 0 para plugin no encontrado", () => {
      ...
      const score =
        pluginSecurityManager._calculatePluginSecurityScore(pluginId);
  ...
  describe("toggleSecurityCheck", () => {
    ...
    test("debería manejar checkName null/undefined", () => {
      ...
      expect(pluginSecurityManager.toggleSecurityCheck(undefined, true)).toBe(
        false
      );
    });

    test("debería habilitar verificación de seguridad", () => {
      const result = pluginSecurityManager.toggleSecurityCheck(
        "newCheck",
        true
      );

      expect(result).toBe(true);
      expect(pluginSecurityManager.activeSecurityChecks.has("newCheck")).toBe(
        true
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.securityCheckToggled",
        {
          checkName: "newCheck",
          enabled: true,
    ...
    test("debería deshabilitar verificación de seguridad", () => {
      ...
      expect(result).toBe(true);
      expect(pluginSecurityManager.activeSecurityChecks.has("testCheck")).toBe(
        false
      );
    });
  ...
  describe("clearPluginSecurityData", () => {
    ...
    test("debería manejar plugin ID null/undefined", () => {
      ...
      expect(pluginSecurityManager.clearPluginSecurityData(undefined)).toBe(
        false
      );
    });
    ...
    test("debería limpiar todos los datos del plugin", () => {
      ...
      expect(pluginSecurityManager.blacklistedPlugins.has(pluginId)).toBe(
        false
      );
      expect(pluginSecurityManager.detectedThreats).toHaveLength(0);
      expect(pluginSecurityAudit.clearPluginData).toHaveBeenCalledWith(
        pluginId
      );
      expect(pluginResourceMonitor.clearPluginData).toHaveBeenCalledWith(
        pluginId
      );
      expect(pluginPermissionChecker.clearPluginData).toHaveBeenCalledWith(
        pluginId
      );
    });
  ...
  describe("event listeners simulation", () => {
    ...
    test("debería manejar simulación de _handleSecurityEvent", () => {
      const validateSpy = jest
        .spyOn(pluginSecurityManager, "validatePlugin")
        .mockImplementation(() => ({}));

      pluginSecurityManager.initialize();

      // Simular evento directamente usando _handleSecurityEvent
      pluginSecurityManager._handleSecurityEvent("testEvent", {
        pluginId: "newPlugin",
      });
    ...
    test("debería configurar el sistema de eventos correctamente", () => {
      ...
      expect(typeof pluginSecurityManager._handleSecurityEvent).toBe(
        "function"
      );
    });
  });
