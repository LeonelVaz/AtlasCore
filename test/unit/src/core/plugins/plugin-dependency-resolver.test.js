...
describe('PluginDependencyResolver', () => {
  ...
  describe('_buildDependencyGraph', () => {
    test('debe construir el grafo de dependencias y dependencias inversas correctamente', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.dependencyGraphBuilt', expect.any(Object));
    });
  ...
  describe('_detectCycles', () => {
    test('debe detectar ciclos simples A -> B -> A', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.cyclesDetected', { cycles });
    });
    ...
    test('no debe detectar ciclos si no los hay', () => {
      ...
      expect(eventBus.publish).not.toHaveBeenCalledWith('pluginSystem.cyclesDetected', expect.anything());
    });
  ...
  describe('calculateLoadOrder', () => {
    test('debe calcular el orden de carga correcto para dependencias lineales', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.loadOrderCalculated', 
    ...
    test('debe advertir y romper ciclos al calcular el orden de carga', () => {
        ...
        pluginRegistry.getAllPlugins.mockReturnValue(plugins);
        const order = pluginDependencyResolver.calculateLoadOrder();
        expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('Detectados 1 ciclos de dependencias'));
        expect(order.length).toBe(3);
    ...
    test('debe devolver todos los plugins si falla la construcción del grafo', () => {
      ...
      pluginDependencyResolver._buildDependencyGraph = jest.fn(() => { throw new Error('Graph build error'); });
      
      pluginRegistry.getAllPlugins.mockReturnValue([{id: 'X'}, {id: 'Y'}]);
      
      const order = pluginDependencyResolver.calculateLoadOrder();
      expect(order).toEqual(['X', 'Y']); 
      expect(console.error).toHaveBeenCalledWith('Error al calcular orden de carga de plugins:', expect.any(Error));
  ...
  describe('getPluginPriority', () => {
    ...
    test('debe devolver la prioridad definida en el plugin si existe', () => {
        expect(pluginDependencyResolver.getPluginPriority({ id: 'E', priority: 5 })).toBe(5);
    });

    test('debe devolver prioridad alta (10) para plugins core', () => {
        expect(pluginDependencyResolver.getPluginPriority({ id: 'D', core: true })).toBe(10);
    });
    ...
    test('debe calcular prioridad basada en dependientes inversos', () => {
        ...
        expect(pluginDependencyResolver.getPluginPriority({id: 'C'})).toBeLessThan(pluginDependencyResolver.getPluginPriority({id: 'A'}));
    });
  });

  describe('validateAllPlugins', () => {
    test('debe validar todos los plugins y publicar resultados', () => {
      const pluginOk = { id: 'pOk', minAppVersion: '0.1.0', maxAppVersion: '1.0.0', dependencies:[], conflicts:[] };
      const pluginBadDep = { id: 'pBadDep', minAppVersion: '0.1.0', maxAppVersion: '1.0.0', dependencies: [{id: 'missing', version: '1.0.0'}], conflicts:[] };
      pluginRegistry.getAllPlugins.mockReturnValue([pluginOk, pluginBadDep]);
      
      pluginCompatibility.checkAppCompatibility.mockImplementation(p => ({ compatible: true, reason: ''}));
      pluginCompatibility.checkDependencies.mockImplementation(p => {
          if (p.id === 'pOk') return { satisfied: true, missing: [] };
          return { satisfied: false, missing: [{id: 'missing'}], reason: 'Falta dependencia' };
      });
      pluginCompatibility.checkConflicts.mockReturnValue({ hasConflicts: false, conflicts: [] });
      pluginCompatibility.checkReversedConflicts.mockReturnValue({ hasConflicts: false, conflicts: [] });
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.pluginsValidated', expect.any(Object));
    });
  });
});