...
} from "../../../../../src/core/plugins/plugin-validator";
...
describe("PluginValidator", () => {
  ...
  beforeEach(() => {
    ...
    pluginCompatibility.checkAppCompatibility.mockReturnValue({
      compatible: true,
      reason: "",
    });
    pluginCompatibility.runFullCompatibilityCheck.mockReturnValue({
      compatible: true,
      reason: "Compatible",
      details: {},
    });
  ...
  describe("validatePlugin", () => {
    ...
    test("debe devolver false si el plugin no es un objeto", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "El plugin no es un objeto válido"
      );
    });
    ...
    ["id", "name", "version", "description", "author"].forEach((field) => {
      test(`debe devolver false si falta ${field}`, () => {
        ...
        const pluginIdForMessage =
          field === "id" ? undefined : invalidPlugin.id;
    ...
    ["init", "cleanup"].forEach((method) => {
      test(`debe devolver false si falta el método ${method}`, () => {
        ...
        expect(validatePlugin(invalidPlugin)).toBe(false);
        expect(console.error).toHaveBeenCalledWith(
          `Plugin [${mockPlugin.id}] sin método '${method}' válido`
        );
      });
    });
    ...
    test("debe devolver false si checkAppCompatibility falla", () => {
      pluginCompatibility.checkAppCompatibility.mockReturnValue({
        compatible: false,
        reason: "Versión de app incorrecta",
      });
      expect(validatePlugin(mockPlugin)).toBe(false);
      expect(console.error).toHaveBeenCalledWith(
        `Plugin [${mockPlugin.id}] incompatible: Versión de app incorrecta`
      );
    });
    ...
    test("debe devolver false si las dependencias son inválidas (no array)", () => {
      ...
      expect(validatePlugin(invalidPlugin)).toBe(false);
      expect(console.error).toHaveBeenCalledWith(
        `Plugin [${mockPlugin.id}] con dependencias declaradas en formato inválido`
      );
    });
    test("debe devolver false si una dependencia tiene ID vacío", () => {
      ...
      expect(validatePlugin(invalidPlugin)).toBe(false);
      expect(console.error).toHaveBeenCalledWith(
        `Plugin [${mockPlugin.id}] contiene una dependencia sin ID válido`
      );
    });
    ...
    test("debe devolver false si los conflictos son inválidos (no array)", () => {
      ...
      expect(validatePlugin(invalidPlugin)).toBe(false);
      expect(console.error).toHaveBeenCalledWith(
        `Plugin [${mockPlugin.id}] con conflictos declarados en formato inválido`
      );
    });
  });
