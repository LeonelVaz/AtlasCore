...
jest.mock('../../../../../src/core/bus/event-bus', () => {
  ...
  const subscribe = jest.fn((eventName, callback) => {
    ...
    return jest.fn(() => { 
      if (subscribers[eventName]) {
        subscribers[eventName] = subscribers[eventName].filter(cb => cb !== currentCallback);
      }
    });
  });
...
import { PLUGIN_CONSTANTS, STORAGE_KEYS } from '../../../../../src/core/config/constants';
...
describe('PluginSecurityAudit', () => {
  ...
  describe('initialize', () => {
    test('debe inicializar y llamar a _setupEventListeners, y eventBus.subscribe debe ser llamado', async () => {
      const setupListenersSpy = jest.spyOn(pluginSecurityAudit, '_setupEventListeners');
      await pluginSecurityAudit.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
    ...
    test('debe cargar log desde storageService', async () => {
      storageService.get.mockResolvedValueOnce([{ id: 'entry1', pluginId, auditType: 'validation' }]);
      await pluginSecurityAudit.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
  ...
  describe('Event Handling and Recording', () => {
    ...
    test('debe registrar un evento de seguridad directamente', async () => {
      ...
      expect(storageService.set).toHaveBeenCalledWith(storageKey, expect.any(Array));
    });
    ...
    test('debe suscribirse a eventos del sistema y registrarlos', async () => {
      ...
      expect(pluginSecurityAudit.auditLog.length).toBe(1);
      const loggedEntry = pluginSecurityAudit.auditLog.find(e => e.eventType === 'pluginActivated');
  ...
  describe('Batch Processing', () => {
    ...
    test('debe encolar eventos en modo batch y procesarlos', async () => {
      ...
      expect(pluginSecurityAudit.auditLog.length).toBe(1);
      expect(pluginSecurityAudit.auditLog[0].details.message).toBe('Batch Event 1');
    });
  });
  ...
  describe('Log Management', () => {
    ...
    test('getPluginAuditHistory debe devolver el historial de un plugin', () => {
      ...
      pluginSecurityAudit.recordSecurityEvent({ pluginId: 'otherPlugin', message: 'Event 2' });
    ...
    test('getAuditLog debe filtrar por criterios', () => {
      pluginSecurityAudit.recordSecurityEvent({ pluginId, auditType: 'securityEvent' });
      pluginSecurityAudit.recordBlacklistAction(pluginId, { action: 'add' });
      
      const filtered = pluginSecurityAudit.getAuditLog({ auditType: 'blacklistAction' });
  ...
  describe('setSecurityLevel', () => {
    ...
    test('debe cambiar el nivel de seguridad y reconfigurar el modo de auditoría', () => {
        const result = pluginSecurityAudit.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH);
        expect(result).toBe(true);
        expect(pluginSecurityAudit.securityLevel).toBe(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH);
    ...
    test('debe cambiar a modo batch si el nivel es LOW', () => {
        const result = pluginSecurityAudit.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW);
        expect(result).toBe(true);
        expect(pluginSecurityAudit.securityLevel).toBe(PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW);
        expect(pluginSecurityAudit.auditMode).toBe('batch');
  ...
  describe('getAuditStats', () => {
    ...
    test('debe devolver estadísticas correctas de auditoría', () => {
        pluginSecurityAudit.recordSecurityEvent({ pluginId: 'p1', message: 'Event Alpha' });
