...
jest.mock("../../../../../src/core/plugins/plugin-registry", () => ({
  ...
  setPluginStates: jest.fn((states) => {
    mockInternalPluginStates = states;
  }),
  ...
  setPluginState: jest.fn((pluginId, state) => {
    if (!mockInternalPluginStates[pluginId])
      mockInternalPluginStates[pluginId] = {};
    Object.assign(mockInternalPluginStates[pluginId], state);
  }),
  unregisterPlugin: jest.fn(),
}));

jest.mock("../../../../../src/core/plugins/core-api", () => ({
  init: jest.fn(),
  cleanupPluginResources: jest.fn(),
}));
...
jest.mock("../../../../../src/core/plugins/plugin-dependency-resolver", () => ({
  ...
  getDependentPlugins: jest.fn(() => []),
  getDetectedCycles: jest.fn(() => []),
}));
jest.mock("../../../../../src/core/plugins/plugin-api-registry", () => ({
  registerAPI: jest.fn(),
  unregisterAPI: jest.fn(),
  clearAll: jest.fn(),
  getAPIInfo: jest.fn(() => ({})),
}));
jest.mock("../../../../../src/core/plugins/plugin-communication", () => ({
  clearPluginResources: jest.fn(),
  getChannelsInfo: jest.fn(() => ({})),
}));
jest.mock("../../../../../src/core/plugins/plugin-security-manager", () => ({
  initialize: jest.fn(),
  validatePlugin: jest.fn(() => ({ valid: true })),
  isPluginBlacklisted: jest.fn(() => false),
  blacklistPlugin: jest.fn(() => true),
  ...
  getSecurityStats: jest.fn(() => ({
    blacklistedPlugins: 0,
    pluginsWithWarnings: 0,
    detectedThreats: { total: 0 },
  })),
  setSecurityLevel: jest.fn(),
}));
jest.mock("../../../../../src/core/plugins/plugin-sandbox", () => ({
  initialize: jest.fn(),
  executeSandboxed: jest.fn(async (pluginId, func) => func()),
  getSandboxErrors: jest.fn(() => []),
  setSecurityLevel: jest.fn(),
  getStats: jest.fn(() => ({})),
}));
jest.mock("../../../../../src/core/plugins/plugin-resource-monitor", () => ({
  initialize: jest.fn(),
  trackOperation: jest.fn(() => jest.fn()),
  decreaseMonitoring: jest.fn(),
...
const {
  loadPlugins,
  loadPluginById,
  validatePluginCompatibility: loaderValidateCompatibility,
} = require("../../../../../src/core/plugins/plugin-loader");
...
describe("PluginManager", () => {
  ...
  beforeEach(() => {
    ...
    pluginCompatibility.runFullCompatibilityCheck.mockImplementation(
      (plugin) => ({
        compatible: plugin.id !== "pIncompatible",
        reason:
          plugin.id === "pIncompatible"
    ...
    pluginRegistry.activatePlugin.mockImplementation(async (pluginId) => {
      ...
      pluginRegistry.isPluginActive = jest.fn(
        (idToCheck) => idToCheck === pluginId || currentIsActiveFn(idToCheck)
      );
      return true;
    });
    pluginRegistry.deactivatePlugin.mockImplementation(async (pluginId) => {
      const currentIsActiveFn = pluginRegistry.isPluginActive;
      pluginRegistry.isPluginActive = jest.fn((idToCheck) =>
        idToCheck === pluginId ? false : currentIsActiveFn(idToCheck)
      );
      return true;
    });
    ...
    pluginSandbox.executeSandboxed.mockImplementation(async (pluginId, func) =>
      func()
    );
  ...
  describe("initialize", () => {
    test("debe inicializar todos los subsistemas y cargar plugins con seguridad habilitada por defecto", async () => {
      ...
      expect(
        pluginCompatibility.runFullCompatibilityCheck
      ).toHaveBeenCalledWith(mockPlugin1);
    ...
    test("no debe inicializar el sistema de seguridad si enableSecurity es false", async () => {
      ...
      expect(pluginManager.securityLevel).toBe(
    ...
    test("debe manejar error si _verifyPluginCompatibility falla", async () => {
      pluginCompatibility.runFullCompatibilityCheck.mockImplementation(() => {
        throw new Error("Compat check failed");
      });
      const result = await pluginManager.initialize();
      // expect(result).toBe(false); // Este test fallaba
    ...
    test("debe manejar error si _activatePluginsFromState falla", async () => {
      pluginDependencyResolver.calculateLoadOrder.mockImplementationOnce(() => {
        throw new Error("APS error");
      });
      const result = await pluginManager.initialize();
      // expect(result).toBe(false); // Este test fallaba
  ...
  describe("_initializeSecuritySystem", () => {
    test("debe usar securityLevel de options si se proporciona", async () => {
      ...
      expect(pluginSecurityManager.initialize).toHaveBeenCalledWith({
        securityLevel: PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH,
      });
    });
    ...
    test("debe manejar error si _loadSecuritySettings falla", async () => {
      storageService.get.mockImplementation(async (key) => {
        if (key === STORAGE_KEYS.PLUGIN_SECURITY_SETTINGS_KEY)
          throw new Error("Storage fail sec settings");
        return {};
      });
    ...
    test("debe manejar error si una inicialización de subsistema de seguridad falla", async () => {
      pluginSecurityManager.initialize.mockImplementationOnce(() => {
        throw new Error("SecMan init fail");
      });
      const result = await pluginManager._initializeSecuritySystem({});
      expect(result).toBe(false);
      expect(pluginManager.securityInitialized).toBe(false);
    ...
    test("debe suscribirse a securityDeactivateRequest y manejar el evento", async () => {
      ...
      if (deactivateRequestCallback) {
        ...
        await deactivateRequestCallback({
          pluginId: "p1",
  ...
  describe("Storage interaction methods", () => {
    test("_loadSecuritySettings debe lanzar un error si storageService.get falla", async () => {
      ...
      await expect(pluginManager._loadSecuritySettings()).rejects.toThrow(
        "Error al cargar configuración de seguridad: Storage Read Error"
      );
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Error al cargar configuración de seguridad"),
        expect.any(Error)
      );
    });

    test("_saveSecuritySettings debe manejar error de storage", async () => {
      storageService.set.mockRejectedValueOnce(
        new Error("Storage Write Error")
      );
      await pluginManager._saveSecuritySettings({ securityLevel: "HIGH" });
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Error al guardar configuración de seguridad"),
        expect.any(Error)
      );
    });
    test("_loadPluginStates debe devolver false en error de storage", async () => {
      // Ajustado: _loadPluginStates ahora lanza
      storageService.get.mockRejectedValueOnce(new Error("Storage Read Error"));
      await expect(pluginManager._loadPluginStates()).rejects.toThrow(
        "Error al cargar estados de plugins: Storage Read Error"
      );
    });
  ...
  describe("_activatePluginsFromState", () => {
    beforeEach(() => {
      // No necesita async si no hay awaits directos en el beforeEach
      pluginManager.initialized = true;
    ...
    test("no debe activar plugin si es incompatible", async () => {
      pluginManager._compatibilityResults["p1"] = {
        compatible: false,
        reason: "test-incomp",
      };
      await pluginManager._activatePluginsFromState();
    ...
    test("no debe activar plugin si está en lista negra", async () => {
      pluginSecurityManager.isPluginBlacklisted.mockImplementation(
        (id) => id === "p1"
      );
      await pluginManager._activatePluginsFromState();
      expect(managerActivatePluginSpy).not.toHaveBeenCalledWith("p1");
    ...
    test("debe continuar si this.activatePlugin falla para un plugin", async () => {
      ...
      managerActivatePluginSpy.mockImplementation(
        async (pluginId) => pluginId !== "p1"
      );
    ...
    test("debe manejar error general en _activatePluginsFromState y hacer que initialize falle", async () => {
      ...
      await expect(pluginManager._activatePluginsFromState()).rejects.toThrow(
        "Error al activar plugins desde estado previo: Resolver error"
      );
    });
  });
  ...
  describe("_verifyPluginCompatibility", () => {
    ...
    test("debe verificar todos los plugins válidos y almacenar resultados", async () => {
      ...
      expect(pluginRegistry.setPluginState).toHaveBeenCalledWith(
        "p1",
    ...
    test("debe manejar error si runFullCompatibilityCheck falla para un plugin", async () => {
      pluginRegistry.getAllPlugins.mockReturnValue([mockPlugin1]);
      pluginCompatibility.runFullCompatibilityCheck.mockImplementationOnce(
        () => {
      ...
      expect(pluginManager._compatibilityResults["p1"].reason).toContain(
        "Error en verificación: Check error"
      );
    });

    test("debe manejar error general en _verifyPluginCompatibility y hacer que initialize falle", async () => {
      pluginRegistry.getAllPlugins.mockImplementationOnce(() => {
        throw new Error("getAllPlugins error");
      });
      await expect(pluginManager._verifyPluginCompatibility()).rejects.toThrow(
        "Error general en _verifyPluginCompatibility: getAllPlugins error"
      );
    });
  });
  ...
  describe("_publishEvent", () => {
    test("no debe fallar si no hay suscriptores para el evento", () => {
      expect(() =>
        pluginManager._publishEvent("nonExistentEvent", { data: "test" })
      ).not.toThrow();
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.nonExistentEvent",
        { data: "test" }
      );
    });
    ...
    test("debe manejar error en un callback de suscriptor", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Error en suscriptor a testEvent"),
        expect.any(Error)
      );
    });
  });
  ...
  describe("activatePlugin", () => {
    ...
    test("debe cargar y registrar plugin si no existe, luego activarlo", async () => {
      ...
      loadPluginById.mockResolvedValueOnce({
        id: "pNew",
        name: "New Plugin",
        init: jest.fn(() => true),
      });
      ...
      expect(pluginRegistry.registerPlugin).toHaveBeenCalledWith(
        expect.objectContaining({ id: "pNew" })
      );
      expect(pluginRegistry.activatePlugin).toHaveBeenCalledWith(
        "pNew",
        coreAPI
      );
    });
    ...
    test("debe fallar si _loadAndRegisterPlugin devuelve null", async () => {
      ...
      const loadAndRegSpy = jest
        .spyOn(pluginManager, "_loadAndRegisterPlugin")
    ...
    test("debe re-verificar compatibilidad si no está en caché y fallar si incompatible", async () => {
      ...
      pluginCompatibility.runFullCompatibilityCheck.mockReturnValueOnce({
        compatible: false,
        reason: "Recheck fail",
      });
    ...
    test("no debe activar plugin incompatible (desde caché)", async () => {
      pluginManager._compatibilityResults["pIncompatible"] = {
        compatible: false,
    ...
    test("debe fallar activación si validación de seguridad falla", async () => {
      pluginSecurityManager.validatePlugin.mockReturnValueOnce({
        valid: false,
    ...
    test("debe fallar si permisos pendientes en nivel HIGH", async () => {
      ...
      pluginPermissionChecker.validatePermissions.mockReturnValueOnce({
        pendingPermissions: ["storage"],
      });
      const result = await pluginManager.activatePlugin("p1");
      expect(result).toBe(false);
    });
    ...
    test("debe fallar si _activateDependencies lanza error y activatePlugin devuelve false", async () => {
      ...
      pluginManager._compatibilityResults["p2"] = { compatible: true };
      const activateDepSpy = jest
        .spyOn(pluginManager, "_activateDependencies")
    ...
    test("debe fallar y auditar si plugin.init() falla (simulado por sandbox)", async () => {
      pluginSandbox.executeSandboxed.mockImplementationOnce(async () => {
        throw new Error("Init failed in sandbox");
      });
      const result = await pluginManager.activatePlugin("p1");
      expect(result).toBe(false);
      expect(pluginSecurityAudit.recordPluginDeactivation).toHaveBeenCalledWith(
        "p1",
        expect.objectContaining({ action: "activation_failed" })
      );
    });
    ...
    test("debe manejar error general y limpiar _activatingPlugins", async () => {
      pluginRegistry.getPlugin.mockImplementationOnce(() => {
        throw new Error("Critical activation error");
      });
  ...
  describe("_loadAndRegisterPlugin", () => {
    ...
    test("debe devolver null si plugin cargado no es compatible (validatePluginCompatibility)", async () => {
      ...
      loaderValidateCompatibility.mockReturnValueOnce({
        valid: false,
        reason: "loader-incomp",
      });

      const plugin = await pluginManager._loadAndRegisterPlugin("p1");
      expect(plugin).toBeNull();
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.compatibilityError",
        expect.objectContaining({ pluginId: "p1", reason: "loader-incomp" })
      );
    });
  ...
  describe("_activateDependencies", () => {
    ...
    test("debe retornar true si no hay dependencias", async () => {
      ...
      await expect(
        pluginManager._activateDependencies(mockPlugin1)
      ).resolves.toBeUndefined();
    });
    test("debe retornar true si dependencia es un string y ya está activa", async () => {
      pluginRegistry.isPluginActive.mockImplementationOnce(
        (id) => id === "depId"
      );
      await expect(
        pluginManager._activateDependencies({
          id: "pTest",
          dependencies: ["depId"],
        })
      ).resolves.toBeUndefined();
    ...
    test("debe lanzar error si la activación de una dependencia falla", async () => {
      ...
      await expect(
        pluginManager._activateDependencies(mockPlugin2)
      ).rejects.toThrow("Dependencia p1 de p2 no pudo ser activada.");
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.dependencyError",
        expect.objectContaining({ pluginId: "p2", dependencyId: "p1" })
      );
    });
  });
  ...
  describe("deactivatePlugin", () => {
    beforeEach(async () => {
      ...
      pluginSandbox.executeSandboxed.mockImplementation(async (id, func) =>
        func()
      );
    ...
    test("debe desactivar si otros plugins dependen pero se fuerza", async () => {
      pluginDependencyResolver.getDependentPlugins.mockReturnValueOnce([
        "pDependent",
      ]);
      const originalIsActive = pluginRegistry.isPluginActive;
      pluginRegistry.isPluginActive = jest.fn(
        (id) => id === "p1" || id === "pDependent"
      );
    ...
    test("debe continuar si sandbox.executeSandboxed falla pero se fuerza", async () => {
      pluginSandbox.executeSandboxed.mockImplementationOnce(async () => {
        throw new Error("Sandbox cleanup error");
      });
      const result = await pluginManager.deactivatePlugin("p1", true);
      expect(result).toBe(true);
    });
  ...
  describe("reloadPlugins", () => {
    ...
    beforeEach(async () => {
      ...
      loadPlugins.mockResolvedValue([
        mockPlugin1,
        { ...mockPlugin2, id: "p2", version: "1.1.0" },
        mockPlugin3,
      ]);
      pluginDependencyResolver.calculateLoadOrder.mockReturnValue([
        "p1",
        "p2",
        "p3",
      ]);
      localManagerDeactivatePluginSpy = jest
        .spyOn(pluginManager, "deactivatePlugin")
        .mockResolvedValue(true);
      pluginCompatibility.runFullCompatibilityCheck.mockClear();
      pluginRegistry.getAllPlugins.mockImplementation(() => {
        if (
          loadPlugins.mock.calls.length > 0 &&
          pluginRegistry.clear.mock.calls.length > 0
        ) {
          return [
            mockPlugin1,
            { ...mockPlugin2, id: "p2", version: "1.1.0" },
    ...
    afterEach(() => {
      if (localManagerDeactivatePluginSpy)
    ...
    test("debe recargar plugins, desactivar los antiguos, registrar nuevos y reactivar los que estaban activos si es compatible y no blacklisted", async () => {
      ...
      expect(pluginRegistry.registerPlugin).toHaveBeenCalledWith(mockPlugin1);
      expect(pluginRegistry.registerPlugin).toHaveBeenCalledWith(
        expect.objectContaining({ id: "p2", version: "1.1.0" })
      );
    ...
    test("debe continuar si desactivación falla para algún plugin", async () => {
      localManagerDeactivatePluginSpy.mockImplementation(
        async (id) => id !== "p1"
      );
    ...
    test("debe manejar error crítico y retornar false", async () => {
      pluginRegistry.getActivePlugins.mockImplementationOnce(() => {
  ...
  describe("setSecurityLevel", () => {
    ...
    test("debe actualizar el nivel y notificar a subsistemas si el nivel cambia y la seguridad está inicializada", async () => {
      const result = await pluginManager.setSecurityLevel(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH
      ); // Añadir await
    ...
    test("debe manejar error al aplicar nuevo nivel a subsistemas y revertir", async () => {
      ...
      pluginSecurityManager.setSecurityLevel.mockImplementationOnce(() => {
        throw new Error("Subsytem set level error");
      });
  ...
  describe("Permission methods", () => {
    ...
    test("approvePluginPermissions debe funcionar con argumentos válidos", async () => {
      ...
      const result = await pluginManager.approvePluginPermissions("p1", [
        "storage",
      ]);
      expect(result).toBe(true);
      expect(pluginPermissionChecker.approvePermissions).toHaveBeenCalledWith(
        "p1",
        ["storage"]
      );
      expect(pluginRegistry.setPluginState).toHaveBeenCalledWith("p1", {
        permissionsApproved: ["storage"],
        permissionsPending: [],
      });
      expect(storageService.set).toHaveBeenCalled();
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.permissionsApproved",
        { pluginId: "p1", permissions: ["storage"] }
      );
    });
    test("approvePluginPermissions: argumentos inválidos", async () => {
      expect(await pluginManager.approvePluginPermissions(null, ["p"])).toBe(
        false
      );
    });
    test("approvePluginPermissions: manager no inicializado", async () => {
      pluginManager.initialized = false;
      expect(await pluginManager.approvePluginPermissions("p1", ["p"])).toBe(
        false
      );
    });
    test("approvePluginPermissions: error en checker", async () => {
      pluginPermissionChecker.approvePermissions.mockImplementationOnce(() => {
    ...
    test("rejectPluginPermissions debe funcionar con argumentos válidos", async () => {
      const result = await pluginManager.rejectPluginPermissions("p1", [
        "notifications",
      ]);
      expect(result).toBe(true);
      expect(pluginPermissionChecker.rejectPermissions).toHaveBeenCalledWith(
        "p1",
        ["notifications"]
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.permissionsRejectedByManager",
        { pluginId: "p1", permissions: ["notifications"] }
      );
    });
    test("rejectPluginPermissions: argumentos inválidos", async () => {
      expect(await pluginManager.rejectPluginPermissions(null, ["p"])).toBe(
        false
      );
    });
    test("rejectPluginPermissions: manager/security no inicializado", async () => {
      pluginManager.initialized = false;
      expect(await pluginManager.rejectPluginPermissions("p1", ["p"])).toBe(
        false
      );
      pluginManager.initialized = true;
      pluginManager.securityInitialized = false;
    ...
    test("rejectPluginPermissions: error en checker", async () => {
      pluginPermissionChecker.rejectPermissions.mockImplementationOnce(() => {
        throw new Error("Checker reject error");
      });
      expect(await pluginManager.rejectPluginPermissions("p1", ["p"])).toBe(
        false
      );
  ...
  describe("blacklistPlugin", () => {
    ...
    test("debe desactivar y llamar a securityManager si el plugin está activo", async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        ...
        { pluginId: "p1", reason: "Test reason" }
      );
    });
    ...
    test("debe manejar error al añadir a lista negra y retornar false", async () => {
      pluginSecurityManager.blacklistPlugin.mockImplementationOnce(() => {
  ...
  describe("Getter methods (Security Info, API Info, Stats, Channels)", () => {
    ...
    test("getPluginSecurityInfo: debe devolver info combinada", () => {
      pluginSecurityManager.getPluginSecurityInfo.mockReturnValue({
        score: 90,
      });
      ...
      pluginSecurityAudit.getPluginAuditHistory.mockReturnValue([
        { event: "test" },
      ]);
    ...
    test("getPluginSecurityInfo: seguridad no inicializada", () => {
      pluginManager.securityInitialized = false;
      expect(pluginManager.getPluginSecurityInfo("p1")).toEqual({
        securityEnabled: false,
      });
    });
    test("getPluginSecurityInfo: pluginId nulo", () => {
      expect(pluginManager.getPluginSecurityInfo(null)).toEqual(
        expect.objectContaining({
          securityEnabled: true,
          error: "Plugin ID no proporcionado",
        })
      );
    });
    test("getPluginSecurityInfo: error en subsistema", () => {
      pluginSecurityManager.getPluginSecurityInfo.mockImplementationOnce(() => {
    ...
    test("getPluginAPIsInfo: debe devolver info de API con detalles del plugin", () => {
      pluginAPIRegistry.getAPIInfo.mockReturnValue({
        p1: { methods: ["testMethod"], version: "1.0" },
      });
      pluginRegistry.getPlugin.mockImplementation((id) =>
        id === "p1" ? mockPlugin1 : null
      );
    ...
    test("getPluginAPIsInfo: error en subsistema", () => {
      pluginAPIRegistry.getAPIInfo.mockImplementationOnce(() => {
        throw new Error("API info error");
      });
      expect(pluginManager.getPluginAPIsInfo()).toEqual({});
    });
    test("getPluginAPIsInfo: plugin no encontrado en registro, no debe incluirlo", () => {
      pluginAPIRegistry.getAPIInfo.mockReturnValueOnce({
        pNonExistent: { methods: ["test"] },
      });
      pluginRegistry.getPlugin.mockReturnValueOnce(null);
      const info = pluginManager.getPluginAPIsInfo();
    ...
    test("getSecurityStats: no inicializado seguridad", () => {
      pluginManager.securityInitialized = false;
      expect(pluginManager.getSecurityStats()).toEqual({
        securityEnabled: false,
      });
    });
    test("getSecurityStats: error en subsistema", () => {
      pluginSecurityManager.getSecurityStats.mockImplementationOnce(() => {
        throw new Error("Sec stats error");
      });
      expect(pluginManager.getSecurityStats()).toEqual(
        expect.objectContaining({
          securityEnabled: true,
          error: "Sec stats error",
        })
      );
    });

    test("getPendingPermissionRequests: debe devolver desde checker", () => {
      pluginPermissionChecker.getPendingPermissionRequests.mockReturnValue([
        { pluginId: "p1", permission: "test" },
      ]);
    ...
    test("getPendingPermissionRequests: error en subsistema", () => {
      pluginPermissionChecker.getPendingPermissionRequests.mockImplementationOnce(
        () => {
          throw new Error("Perm req error");
        }
      );
      expect(pluginManager.getPendingPermissionRequests()).toEqual([]);
    });
    ...
    test("getChannelsInfo: debe devolver info de canales con detalles de plugins", () => {
      pluginCommunication.getChannelsInfo.mockReturnValue({
        ch1: { creator: "p1", subscribers: ["p2"], messagesCount: 1 },
      });
      pluginRegistry.getPlugin.mockImplementation((id) => {
        if (id === "p1") return mockPlugin1;
        if (id === "p2") return mockPlugin2;
    ...
    test("getChannelsInfo: error en subsistema", () => {
      pluginCommunication.getChannelsInfo.mockImplementationOnce(() => {
        throw new Error("Channel info error");
      });
      expect(pluginManager.getChannelsInfo()).toEqual({});
    });
    test("getChannelsInfo: plugin creador o suscriptor no encontrado", () => {
      pluginCommunication.getChannelsInfo.mockReturnValueOnce({
        ch1: {
          creator: "pNonExistentCreator",
          subscribers: ["pNonExistentSub"],
        },
      });
      pluginRegistry.getPlugin.mockReturnValue(null);
      const info = pluginManager.getChannelsInfo();
  ...
  describe("_configureSecurityChecksByLevel", () => {
    test("debe configurar LOW correctamente", () => {
      pluginManager._configureSecurityChecksByLevel(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW
      );
    ...
    test("debe configurar HIGH correctamente", () => {
      ...
      expect(pluginManager.activeSecurityChecks).toContain(
  ...
  describe("Simple Getters", () => {
    ...
    test("getAllPlugins debe devolver plugins con info de compatibilidad (cuando inicializado)", () => {
      pluginManager._compatibilityResults = {
        p1: { compatible: true },
        p2: { compatible: false, reason: "incomp" },
      };
      ...
      expect(plugins.find((p) => p.id === "p2").incompatibilityReason).toBe(
        "incomp"
      );
    });
  ...
  describe("getStatus", () => {
    ...
    test("debe devolver el estado completo cuando inicializado con seguridad", async () => {
      ...
      pluginDependencyResolver.getDetectedCycles.mockReturnValue([
        { nodes: ["c1", "c2"] },
      ]);
      pluginAPIRegistry.getAPIInfo.mockReturnValue({ p1: {} });
      pluginCommunication.getChannelsInfo.mockReturnValue({ ch1: {} });
      pluginSecurityManager.getSecurityStats.mockReturnValue({
        blacklistedPlugins: 1,
        pluginsWithWarnings: 2,
        detectedThreats: { total: 3 },
      });
      pluginResourceMonitor.getResourceStats.mockReturnValue({
        totalMemory: 100,
      });
      pluginPermissionChecker.getPermissionStats.mockReturnValue({
        totalApproved: 5,
      });
