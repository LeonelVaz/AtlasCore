...
describe("PluginSandbox", () => {
  ...
  beforeEach(() => {
    ...
    pluginSandbox.activeChecks = new Set([
      "domManipulation",
      "codeExecution",
  ...
  describe("initialize", () => {
    test("debe inicializar y cachear métodos nativos", () => {
      ...
      expect(pluginSandbox.securityLevel).toBe(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL
      );
      expect(Object.keys(pluginSandbox.cachedNatives).length).toBeGreaterThan(
        0
      );
    ...
    test("debe aplicar protecciones globales si se inicializa en nivel HIGH", () => {
      const spyProtectGlobalObjects = jest.spyOn(
        pluginSandbox,
  ...
  describe("validatePluginCode", () => {
    ...
    test("debe validar código sin violaciones", () => {
      const pluginCode = {
        ...mockPlugin,
        someFunc: '() => console.log("safe")',
      };
      const result = pluginSandbox.validatePluginCode(
        pluginId,
        JSON.parse(JSON.stringify(pluginCode))
      );
    ...
    test('debe detectar "eval" como violación crítica', () => {
      const pluginCode = {
        ...mockPlugin,
        someFunction: 'function() { eval("unsafe") }',
      };
      const result = pluginSandbox.validatePluginCode(
        pluginId,
        JSON.parse(JSON.stringify(pluginCode))
      ...
      expect(
        result.violations.some(
          (v) => v.rule === "preventEval" && v.severity === "critical"
        )
      ).toBe(true);
    });
    ...
    test("debe filtrar violaciones según el nivel de seguridad LOW", () => {
      ...
      expect(result.violations.some((v) => v.rule === "preventEval")).toBe(
        true
      );
      expect(result.violations.some((v) => v.rule === "preventInnerHTML")).toBe(
        false
      );
    });
  });
  ...
  describe("executeSandboxed", () => {
    ...
    test("debe ejecutar una función de forma segura", async () => {
      ...
      const result = await pluginSandbox.executeSandboxed(
        pluginId,
        mockFunc,
        [1, 2]
      );
    ...
    test("debe manejar errores dentro de la función ejecutada", async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ pluginId, error: "Execution failed" })
      );
    ...
    test("debe manejar timeout de ejecución", async () => {
      ...
      const mockFunc = jest.fn(
        () =>
          new Promise((resolve) => {
            ...
            timer(
              () => resolve("nunca debería llegar aquí"),
              sandboxTimeoutDuration + 1000
            );
          })
      );
      ...
      expect(pluginErrorHandler.handleError).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          message: expect.stringContaining("excedió el límite de tiempo"),
        }),
        expect.any(Object)
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
  ...
  describe("DOMProxy", () => {
    ...
    test("debe interceptar acceso a innerHTML en nivel HIGH", () => {
      ...
      expect(() => {
        ...
        "Operación potencialmente peligrosa bloqueada: dom_set innerHTML = [sospechoso]"
      );
    ...
    test("debe permitir acceso a innerHTML en nivel LOW pero loguear si es sospechoso", () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.sensitiveAccessAttempt",
        expect.objectContaining({
          pluginId,
          type: "dom_set",
  ...
  describe("setSecurityLevel", () => {
    test("debe cambiar el nivel de seguridad y notificar (si está inicializado)", () => {
      ...
      const result = pluginSandbox.setSecurityLevel(
        PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH
      );
      ...
      expect(eventBus.publish).toHaveBeenCalledWith(
        "pluginSystem.sandboxSecurityLevelChanged",
        { level: PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH }
      );
    });
  ...
  describe("getStats", () => {
    ...
    test("debe devolver estadísticas correctas del sandbox", async () => {
      ...
      try {
        await pluginSandbox.executeSandboxed("p2", () => {
          throw new Error("err");
        });
      } catch (e) {
        /* Se espera y se maneja en el test de errores */
      }
