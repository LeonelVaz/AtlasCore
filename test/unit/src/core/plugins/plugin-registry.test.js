...
describe('PluginRegistry', () => {
  const mockPlugin1 = { id: 'p1', name: 'Plugin 1', version: '1.0.0', init: jest.fn(() => true), cleanup: jest.fn(() => true) };
  const mockPlugin2 = { id: 'p2', name: 'Plugin 2', version: '1.0.0', dependencies: [{ id: 'p1' }], init: jest.fn(() => true), cleanup: jest.fn(() => true) };
  ...
  describe('registerPlugin', () => {
    test('debe registrar un plugin y sus dependencias/conflictos', () => {
      ...
      expect(pluginRegistry.pluginDependencies['p1']).toEqual([{ id: 'depA', version: '1.0' }]);
      expect(pluginRegistry.pluginConflicts['p1']).toEqual([{ id: 'confB', reason: 'test conflict' }]);
    });
    ...
    test('debe advertir si se sobrescribe un plugin existente', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('El plugin p1 ya está registrado, será sobrescrito');
    });
    ...
    test('debe devolver false y loguear error si el plugin no tiene ID', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Intento de registrar plugin sin ID');
    });
  });
  ...
  describe('unregisterPlugin', () => {
    ...
    test('debe devolver false si el plugin no existe', () => {
      ...
      expect(result).toBe(false);
      expect(console.warn).toHaveBeenCalledWith('Intento de desregistrar plugin inexistente: nonExistent');
    });
  });
  ...
  describe('activatePlugin', () => {
    ...
    test('debe devolver false y registrar error si init falla', () => {
      mockPlugin1.init.mockImplementationOnce(() => { throw new Error('Init failed'); });
      const result = pluginRegistry.activatePlugin('p1', mockCoreAPI);
      expect(result).toBe(false);
    ...
    test('debe devolver false si el plugin no está registrado', () => {
        ...
        expect(result).toBe(false);
        expect(console.error).toHaveBeenCalledWith('Intento de activar plugin no registrado: nonExistent');
    });
  });
  ...
  describe('deactivatePlugin', () => {
    ...
    test('debe desactivar y registrar error si cleanup falla', () => {
      mockPlugin1.cleanup.mockImplementationOnce(() => { throw new Error('Cleanup failed'); });
      const result = pluginRegistry.deactivatePlugin('p1');
  ...
  describe('Query Methods', () => {
    ...
    test('getPluginDependencies debe devolver las dependencias', () => {
        expect(pluginRegistry.getPluginDependencies('p2')).toEqual([{id: 'p1'}]);
    ...
    test('setPluginState y setPluginStates deben actualizar los estados', () => {
        ...
        const newStates = { p1: { active: false }, pNonExistent: { registered: Date.now() } };
        pluginRegistry.setPluginStates(newStates);
        expect(pluginRegistry.getPluginState('p1').active).toBe(false);
        expect(pluginRegistry.getPluginState('pNonExistent').registered).toBeDefined();
    });
