...
describe('PluginPermissionChecker', () => {
  ...
  describe('initialize', () => {
    test('debe inicializar con un nivel de seguridad y loguear', () => {
      ...
      const result = permissionChecker.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH); 
      ...
      expect(console.log).toHaveBeenCalledWith('Inicializando verificador de permisos para plugins...');
      // El log en el código fuente convierte a MAYÚSCULA para mostrar
      expect(console.log).toHaveBeenCalledWith('Verificador de permisos inicializado (nivel: HIGH)');
    });
    ...
    test('debe usar nivel NORMAL por defecto si no se especifica ninguno en initialize', () => {
        ...
        // El log en el código fuente convierte a MAYÚSCULA para mostrar
        expect(console.log).toHaveBeenCalledWith('Verificador de permisos inicializado (nivel: NORMAL)');
    });
    ...
    test('debe manejar un nivel inválido en initialize y usar default', () => {
        ...
        expect(permissionChecker.securityLevel).toBe(LEVEL_NORMAL); 
        expect(console.warn).toHaveBeenCalledWith(expect.stringContaining("Nivel de seguridad de inicialización 'INVALID_LEVEL' inválido"));
    });
  });
  ...
  describe('setSecurityLevel', () => {
    ...
    test('debe cambiar el nivel de seguridad y publicar evento', () => {
        ...
        const result = permissionChecker.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW);
        expect(result).toBe(true);
        // Espera que this.securityLevel sea la versión minúscula
        expect(permissionChecker.securityLevel).toBe(LEVEL_LOW);
        expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.permissionCheckerSecurityLevelChanged', {
          level: LEVEL_LOW, // El evento publica el nivel almacenado (minúscula)
        });
    });

    test('no debe cambiar el nivel si es el mismo', () => {
        const result = permissionChecker.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
        expect(result).toBe(true); 
        expect(permissionChecker.securityLevel).toBe(LEVEL_NORMAL);
    ...
    test('debe devolver false y no cambiar el nivel si es inválido', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith(expect.stringContaining("Nivel de seguridad inválido: 'BOGUS_LEVEL'"));
    });
  });
  ...
  describe('validatePermissions', () => {
    ...
    test('debe auto-aprobar permisos según el nivel NORMAL', () => {
      const result = permissionChecker.validatePermissions('pluginA', ['storage', 'network', 'ui']);
      expect(result.valid).toBe(false);
      expect(result.approvedPermissions).toEqual(expect.arrayContaining(['storage', 'ui']));
      expect(result.pendingPermissions).toEqual(['network']);
      expect(permissionChecker.pluginPermissions['pluginA'].approved).toEqual(expect.arrayContaining(['storage', 'ui']));
      expect(permissionChecker.pluginPermissions['pluginA'].pending).toEqual(['network']);
    });
    ...
    test('debe auto-aprobar todos los permisos relevantes en nivel LOW', () => {
      ...
      const result = permissionChecker.validatePermissions('pluginB', ['storage', 'network', 'dom']);
      expect(result.valid).toBe(true);
      expect(result.approvedPermissions).toEqual(['storage', 'network', 'dom']);
      expect(result.pendingPermissions).toEqual([]);
      expect(permissionChecker.elevatedPermissionPlugins.has('pluginB')).toBe(true); 
    });
    ...
    test('debe marcar permisos como pendientes en nivel HIGH si no son básicos', () => {
      ...
      const result = permissionChecker.validatePermissions('pluginC', ['ui', 'storage', 'events']);
      expect(result.valid).toBe(false); 
      expect(result.approvedPermissions).toEqual(expect.arrayContaining(['ui', 'events']));
      expect(result.pendingPermissions).toEqual(['storage']);
    });

    test('debe manejar permisos inválidos o no reconocidos', () => {
      const result = permissionChecker.validatePermissions('pluginD', ['storage', 'nonExistentPermission']);
      expect(result.valid).toBe(false); 
      expect(result.approvedPermissions).toEqual(['storage']);
      expect(result.invalidPermissions).toEqual(['nonExistentPermission']);
      expect(result.reasons).toContain('Permiso inválido: nonExistentPermission - Permiso no reconocido');
    });

    test('debe usar permisos UI y EVENTS por defecto si no se especifican permisos', () => {
        const result = permissionChecker.validatePermissions('pluginE', undefined);
        expect(result.valid).toBe(true);
        expect(result.permissions).toEqual(['ui', 'events']);
        expect(permissionChecker.pluginPermissions['pluginE'].approved).toEqual(['ui', 'events']);
    });

    test('debe mantener permisos previamente aprobados si se revalida con un subconjunto', () => {
        permissionChecker.validatePermissions('pluginF', ['storage', 'ui', 'network']);
        ...
        const finalPermissions = permissionChecker.getPluginPermissions('pluginF');
  ...
  describe('hasPermission', () => {
    beforeEach(() => {
      permissionChecker.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
      permissionChecker.pluginPermissions['pluginA'] = { approved: ['storage', 'ui'], pending: [], revoked: [] };
    });
  ...
  describe('checkMethodAccess', () => {
    beforeEach(() => {
      permissionChecker.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
      permissionChecker.pluginPermissions['pluginA'] = { approved: ['storage'], pending: [], revoked: [] };
      permissionChecker.pluginPermissions['pluginB'] = { approved: ['network'], pending: [], revoked: [] };
    });

    test('debe permitir acceso si el plugin tiene el permiso requerido para el método', () => {
      const result = permissionChecker.checkMethodAccess('pluginA', 'core.storage.setItem');
    ...
    test('debe permitir acceso a métodos "libres" como console.log', () => {
        const result = permissionChecker.checkMethodAccess('pluginA', 'console.log');
        expect(result.permitted).toBe(true);
        expect(result.permission).toBe('free');
    });

    test('debe denegar acceso a métodos no especificados en nivel NORMAL/HIGH', () => {
        permissionChecker.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
        const resultNormal = permissionChecker.checkMethodAccess('pluginA', 'some.unknown.method');
        expect(resultNormal.permitted).toBe(false);
        
        permissionChecker.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH);
        const resultHigh = permissionChecker.checkMethodAccess('pluginA', 'some.unknown.method');
        expect(resultHigh.permitted).toBe(false);
    });

    test('debe permitir acceso a métodos no especificados en nivel LOW', () => {
        permissionChecker.setSecurityLevel(PLUGIN_CONSTANTS.SECURITY.LEVEL.LOW);
        const result = permissionChecker.checkMethodAccess('pluginA', 'some.unknown.method');
        expect(result.permitted).toBe(true);
    ...
    test('debe loguear acceso y publicar evento en nivel HIGH si se deniega', () => {
        ...
        expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.unauthorizedAccess', {
          pluginId: 'pluginA',
          method: 'fetch',
  ...
  describe('approvePermissions, rejectPermissions, revokePermissions', () => {
    beforeEach(() => {
      permissionChecker.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
      permissionChecker.pluginPermissions['pluginX'] = { approved: ['ui'], pending: ['storage', 'network'], revoked: [] };
      
      permissionChecker.permissionRequests.push(
        { pluginId: 'pluginX', permissions: ['storage'], timestamp: Date.now(), status: 'pending', originalPermissionsForTest_DoNotModify: ['storage'] }
      );
      permissionChecker.permissionRequests.push(
         { pluginId: 'pluginX', permissions: ['network'], timestamp: Date.now(), status: 'pending', originalPermissionsForTest_DoNotModify: ['network'] }
      );
    });

    test('approvePermissions debe mover permisos de pendiente a aprobado y actualizar la solicitud específica', () => {
      const result = permissionChecker.approvePermissions('pluginX', ['storage']);
      expect(result).toBe(true);
      expect(permissionChecker.pluginPermissions['pluginX'].approved).toContain('storage');
      expect(permissionChecker.pluginPermissions['pluginX'].approved).toContain('ui');
      expect(permissionChecker.pluginPermissions['pluginX'].pending).not.toContain('storage');
      expect(permissionChecker.pluginPermissions['pluginX'].pending).toContain('network');
      
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.permissionsApproved', expect.objectContaining({
        pluginId: 'pluginX', permissions: ['storage']
      }));
      
      const storageRequest = permissionChecker.permissionRequests.find(
        r => r.pluginId === 'pluginX' && r.originalPermissionsForTest_DoNotModify && r.originalPermissionsForTest_DoNotModify[0] === 'storage'
      );
      ...
      const networkRequest = permissionChecker.permissionRequests.find(
        r => r.pluginId === 'pluginX' && r.originalPermissionsForTest_DoNotModify && r.originalPermissionsForTest_DoNotModify[0] === 'network'
      );
    ...
    test('rejectPermissions debe mover permisos de pendiente a revocado', () => {
      const result = permissionChecker.rejectPermissions('pluginX', ['network']);
      expect(result).toBe(true);
      expect(permissionChecker.pluginPermissions['pluginX'].pending).not.toContain('network');
      expect(permissionChecker.pluginPermissions['pluginX'].revoked).toContain('network');
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.permissionsRejected', expect.objectContaining({
        pluginId: 'pluginX', permissions: ['network']
      }));
    });
    ...
    test('revokePermissions debe mover permisos de aprobado a revocado', () => {
      ...
      const result = permissionChecker.revokePermissions('pluginX', ['ui', 'dom']);
      expect(result).toBe(true);
      expect(permissionChecker.pluginPermissions['pluginX'].approved).not.toContain('ui');
      expect(permissionChecker.pluginPermissions['pluginX'].approved).not.toContain('dom');
      expect(permissionChecker.pluginPermissions['pluginX'].revoked).toContain('ui');
      expect(permissionChecker.pluginPermissions['pluginX'].revoked).toContain('dom');
      expect(permissionChecker.elevatedPermissionPlugins.has('pluginX')).toBe(false);
      expect(eventBus.publish).toHaveBeenCalledWith('pluginSystem.permissionsRevoked', expect.objectContaining({
        pluginId: 'pluginX', permissions: ['ui', 'dom']
      }));
    });
  });
  ...
  describe('getPluginPermissions', () => {
    test('debe devolver la estructura de permisos correcta para un plugin', () => {
      permissionChecker.initialize(PLUGIN_CONSTANTS.SECURITY.LEVEL.NORMAL);
      permissionChecker.pluginPermissions['myPlugin'] = { approved: ['storage'], pending: ['network'], revoked: ['dom']};
