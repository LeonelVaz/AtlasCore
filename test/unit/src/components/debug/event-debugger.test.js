...
import eventBus, { CalendarEvents, AppEvents, StorageEvents } from '../../../../../src/core/bus/event-bus';
...
Object.defineProperty(global.navigator, 'userAgent', { value: 'TestAgent/1.0', configurable: true, writable: true });
Object.defineProperty(global.navigator, 'platform', { value: 'TestPlatform', configurable: true, writable: true });
Object.defineProperty(global.navigator, 'language', { value: 'en-US', configurable: true, writable: true });
Object.defineProperty(global.navigator, 'cookieEnabled', { value: true, configurable: true, writable: true });
Object.defineProperty(global.navigator, 'onLine', { value: true, configurable: true, writable: true });
Object.defineProperty(global, 'screen', {
  value: { width: 1920, height: 1080, availWidth: 1920, availHeight: 1040, colorDepth: 24 },
  configurable: true, writable: true,
});
Object.defineProperty(global.window, 'innerWidth', { value: 1024, configurable: true, writable: true });
Object.defineProperty(global.window, 'innerHeight', { value: 768, configurable: true, writable: true });
Object.defineProperty(global.window, 'outerWidth', { value: 1024, configurable: true, writable: true });
Object.defineProperty(global.window, 'outerHeight', { value: 768, configurable: true, writable: true });
...
describe('EventDebugger Component', () => {
  ...
  test('renders null if not enabled', async () => {
    ...
    await waitFor(() => expect(storageService.get).toHaveBeenCalledWith(STORAGE_KEYS.DEV_EVENT_DEBUGGER_ENABLED, false));
    expect(container.firstChild).toBeNull();
  });

  test('renders debugger UI if enabled from storage', async () => {
    storageService.get.mockImplementation(async (key) => { // Ensure it's enabled
        if (key === STORAGE_KEYS.DEV_EVENT_DEBUGGER_ENABLED) return true;
        return false;
    });
    render(<EventDebugger />);
    await waitFor(() => expect(screen.getByText('Event Debugger')).toBeInTheDocument());
  ...
  test('loads initial config from storageService and UI reflects it when expanded', async () => {
    ...
    render(<EventDebugger />);
    await waitFor(() => expect(screen.getByText('Event Debugger')).toBeInTheDocument()); // Enabled
    fireEvent.click(screen.getByText('Event Debugger')); // Expand

    const logToggle = screen.getByText('Logs detallados').closest('.debug-toggle').querySelector('input');
    const perfToggle = screen.getByText('Monitor rendimiento').closest('.debug-toggle').querySelector('input');
  ...
  test('handles error when loading config from storage', async () => {
    ...
    render(<EventDebugger />);
    await waitFor(() => {
        expect(mockConsoleError).toHaveBeenCalledWith('Error al cargar configuración del Event Debugger:', expect.any(Error));
    });
  ...
  describe('when enabled', () => {
    ...
    test('close button toggles debugger enabled state and saves to storage', async () => {
      ...
      await waitFor(() => {
        expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.DEV_EVENT_DEBUGGER_ENABLED, false);
        expect(eventBus.publish).toHaveBeenCalledWith('developer.eventDebuggerToggled', { enabled: false });
      });
    });
    
    test('log toggle updates state, storage, and eventBus', async () => {
      const logToggleInput = screen.getByText('Logs detallados').closest('.debug-toggle').querySelector('input');
      ...
      await waitFor(() => {
        expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.DEV_CONSOLE_LOGS_ENABLED, true);
      ...
      await waitFor(() => {
          expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.DEV_CONSOLE_LOGS_ENABLED, false);
      });
    });

    test('performance toggle updates state, storage, and eventBus', async () => {
        const perfToggleInput = screen.getByText('Monitor rendimiento').closest('.debug-toggle').querySelector('input');
        ...
        await waitFor(() => {
          expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.DEV_PERFORMANCE_MONITOR_ENABLED, true);
        });

        fireEvent.click(perfToggleInput);
        await waitFor(() => expect(perfToggleInput).not.toBeChecked());
        await waitFor(() => {
            expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.DEV_PERFORMANCE_MONITOR_ENABLED, false);
        });
    });
    ...
    test('"Test Manual" button executes test sequence', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('test.manual', { mensaje: 'Test manual desde EventDebugger' });
    });
    ...
    test('"Info Sistema" button shows system info', () => {
      ...
      expect(mockConsoleGroup).toHaveBeenCalledWith('🖥️ INFORMACIÓN DEL SISTEMA');
      expect(mockConsoleTable).toHaveBeenCalledWith(expect.objectContaining({ version: '0.3.0' }));
    });

    test('"Stats" button shows event stats', () => {
      eventBus.getActiveEvents.mockReturnValue(['event1', 'event2']);
      eventBus.getSubscriberCount.mockImplementation(event => (event === 'event1' ? 2 : 1));
      const statsButton = screen.getByText('📊 Stats');
      fireEvent.click(statsButton);
      expect(mockConsoleLog).toHaveBeenCalledWith('📋 Eventos activos:', ['event1', 'event2']);
    ...
    test('subscribes to configuration change events and updates UI', async () => {
        ...
        act(() => {
          if (capturedEventCallbacks['developer.consoleLogsToggled']) {
            capturedEventCallbacks['developer.consoleLogsToggled']({ enabled: true });
          } else {
            // This case should ideally not happen if beforeEach mock is correct
            console.warn("Callback for 'developer.consoleLogsToggled' was not captured by the global mock.");
        ...
        act(() => {
           if (capturedEventCallbacks['developer.performanceMonitorToggled']) {
            capturedEventCallbacks['developer.performanceMonitorToggled']({ enabled: true });
          } else {
            console.warn("Callback for 'developer.performanceMonitorToggled' was not captured by the global mock.");
            eventBus.publish('developer.performanceMonitorToggled', { enabled: true });
          }
        });
        await waitFor(() => expect(perfToggleInput).toBeChecked());
    });
    ...
    test('subscribes to system events and updates counts', async () => {
        await act(async () => { jest.runAllTimers(); }); // For verifySystemStatus setTimeout

        // Use the captured callbacks from the beforeEach setup
        expect(capturedEventCallbacks[CalendarEvents.EVENT_CREATED]).toBeDefined();
        act(() => capturedEventCallbacks[CalendarEvents.EVENT_CREATED]({ id: 'test1' }));
        expect(screen.getByText('📅 Calendario').nextElementSibling).toHaveTextContent('1');

        expect(capturedEventCallbacks[AppEvents.INITIALIZED]).toBeDefined();
        act(() => capturedEventCallbacks[AppEvents.INITIALIZED]({ success: true }));
        expect(screen.getByText('🔧 Aplicación').nextElementSibling).toHaveTextContent('1');
        
        expect(capturedEventCallbacks[StorageEvents.DATA_CHANGED]).toBeDefined();
        act(() => capturedEventCallbacks[StorageEvents.DATA_CHANGED]({ key: 'myKey' }));
        expect(screen.getByText('💾 Storage').nextElementSibling).toHaveTextContent('1');
    });
    
    test('useEffect for event monitoring calls verifySystemStatus and logs', async () => {
        await act(async () => { jest.runAllTimers(); }); // For verifySystemStatus setTimeout
        expect(mockConsoleLog).toHaveBeenCalledWith('🔍 EventDebugger: Iniciando monitoreo completo del sistema...\n');
        expect(eventBus.hasSubscribers).toHaveBeenCalledWith('calendar.eventCreated');
    });
    ...
    test('unsubscribes from all eventBus subscriptions on unmount', async () => {
        ...
        eventBus.subscribe = jest.fn((eventName) => {
            if (eventName === 'developer.eventDebuggerToggled' ||
