...
const UpdateManager =
  require("../../../../../src/components/settings/update-manager").default;
...
describe("UpdateManager Component", () => {
  ...
  test('debe mostrar "Cargando..." si loading es true', async () => {
    ...
    render(<UpdateManager onBack={mockOnBack} />);
    expect(
      screen.getByText("Cargando datos de actualizaciones...")
    ).toBeInTheDocument();
    // Esperar a que el contenido principal aparezca después de la carga
    await waitFor(() =>
      expect(
        screen.queryByText("Cargando datos de actualizaciones...")
  ...
  test("debe llamar a checkForUpdates al hacer clic en el botón", async () => {
    ...
    expect(mockPluginUpdateManager.checkForUpdates).toHaveBeenCalledWith({
      fullCheck: true,
    });
  });
  ...
  test("debe abrir diálogo de confirmación y aplicar actualización específica", async () => {
    ...
    const updateButton = within(updateItem.closest(".update-item")).getByText(
      "Actualizar"
    );
    ...
    const confirmButtonInDialog = within(
      screen.getByTestId("mock-dialog")
    ).getByText("Actualizar");
  ...
  test("debe abrir diálogo de configuración y guardar cambios", async () => {
    ...
    const autoCheckCheckbox = screen.getByLabelText(
      /Verificar actualizaciones automáticamente/i
    );
    fireEvent.click(autoCheckCheckbox); // Desmarcar

    const saveButton = within(screen.getByTestId("mock-dialog")).getByText(
      "Guardar"
    );
    ...
    expect(
      mockPluginUpdateManager.configureUpdateSettings
  ...
  test("debe mostrar diálogo de historial de plugin", async () => {
    ...
    const viewDetailsButton = within(
      historyItem.closest(".history-item")
    ).getByText("Ver Detalles");

    fireEvent.click(viewDetailsButton);
    expect(await screen.findByTestId("mock-dialog")).toBeInTheDocument();
    expect(screen.getByTestId("dialog-title")).toHaveTextContent(
      "Historial de Actualizaciones: pluginY"
    );
