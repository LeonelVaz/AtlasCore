...
const ThemeConfig =
  require("../../../../../src/components/settings/theme-config").default;
...
describe("ThemeConfig Component", () => {
  ...
  test("debe renderizar el título y la descripción", () => {
    ...
    expect(
      screen.getByText(/Elige el tema que más te guste/i)
    ).toBeInTheDocument();
  });
  ...
  test("debe renderizar secciones de placeholders para funcionalidades futuras", () => {
    render(<ThemeConfig />);
    expect(
      screen.getByText("Estilo de Encabezados de Días")
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Visualización de Hora en Eventos")
    ).toBeInTheDocument();
  });
});
