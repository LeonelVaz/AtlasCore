...
const DeveloperPanel =
  require("../../../../../src/components/settings/developer-panel").default;
...
describe("DeveloperPanel Component", () => {
  ...
  beforeEach(() => {
    ...
    mockStorageService.get.mockImplementation(
      async (key, defaultValue) => defaultValue
    );
  ...
  test("debe renderizar el panel y cargar la configuración inicial (todos desactivados por defecto)", async () => {
    ...
    const eventDebuggerSection = screen
      .getByText("Event Debugger")
      .closest(".settings-section");
    const eventDebuggerInput = eventDebuggerSection.querySelector(
      'input[type="checkbox"]'
    );
    expect(eventDebuggerInput).not.toBeChecked();

    const consoleLogsSection = screen
      .getByText("Logs Detallados")
      .closest(".settings-section");
    const consoleLogsInput = consoleLogsSection.querySelector(
      'input[type="checkbox"]'
    );
    expect(consoleLogsInput).not.toBeChecked();

    const perfMonitorSection = screen
      .getByText("Monitor de Rendimiento")
      .closest(".settings-section");
    const perfMonitorInput = perfMonitorSection.querySelector(
      'input[type="checkbox"]'
    );
  ...
  test("debe cargar la configuración con valores guardados", async () => {
    mockStorageService.get.mockImplementation(async (key) => {
    ...
    const eventDebuggerSection = screen
      ...
      .closest(".settings-section");
    expect(
      eventDebuggerSection.querySelector('input[type="checkbox"]')
    ).toBeChecked();
    ...
    const consoleLogsSection = screen
      ...
      .closest(".settings-section");
    expect(
      consoleLogsSection.querySelector('input[type="checkbox"]')
    ).toBeChecked();
    ...
    const perfMonitorSection = screen
      ...
      .closest(".settings-section");
    expect(
      perfMonitorSection.querySelector('input[type="checkbox"]')
    ).not.toBeChecked();
  });
  ...
  test("debe activar/desactivar el Event Debugger y guardar el cambio", async () => {
    ...
    expect(
      screen.getByText(
        /El Event Debugger aparecerá en la esquina inferior derecha/i
      )
    ).toBeInTheDocument();
  ...
  test("debe activar/desactivar el Monitor de Rendimiento y guardar el cambio", async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        "developer.performanceMonitorToggled",
        { enabled: true }
      );
    });
  ...
  test('debe limpiar la consola al hacer clic en "Limpiar Consola"', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "🧹 Logs de consola limpiados por el usuario"
    );
  });
