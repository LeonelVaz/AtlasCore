...
const TimeScaleConfig = require('../../../../../src/components/settings/time-scale-config').default;
...
describe('TimeScaleConfig Component', () => {
  ...
  test('debe renderizar el título y la descripción', () => {
    ...
    expect(screen.getByText(/Configura la densidad visual de la rejilla temporal/i)).toBeInTheDocument();
  });
  ...
  test('debe resaltar la escala de tiempo actual', () => {
    render(<TimeScaleConfig />);
    const standardOption = screen.getByText('Estándar').closest('.time-scale-option');
  ...
  test('debe llamar a changeTimeScale al hacer clic en una opción de escala predefinida', () => {
    render(<TimeScaleConfig />);
    const compactOption = screen.getByText('Compacta').closest('.time-scale-option');
  ...
  test('debe mostrar el input para altura personalizada al hacer clic en "Personalizada"', () => {
    render(<TimeScaleConfig />);
    const customOption = screen.getByText('Personalizada').closest('.time-scale-option');
    fireEvent.click(customOption);

    expect(screen.getByLabelText('Altura (en píxeles por hora):')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Aplicar' })).toBeInTheDocument();
  });

  test('debe actualizar el valor de altura personalizada y validarlo', () => {
    render(<TimeScaleConfig />);
    fireEvent.click(screen.getByText('Personalizada').closest('.time-scale-option'));
  ...
  test('debe llamar a createCustomTimeScale al enviar el formulario de altura personalizada', () => {
    ...
    expect(screen.queryByLabelText('Altura (en píxeles por hora):')).not.toBeInTheDocument(); // Formulario se oculta
  });
  ...
  test('debe ocultar el input personalizado al hacer clic en Cancelar', () => {
    ...
    expect(screen.queryByLabelText('Altura (en píxeles por hora):')).not.toBeInTheDocument();
  });
