...
const mockEventBus = {
  ...
  subscribe: jest.fn((eventName, callback) => {
    ...
    if (!mockEventBus.subscriptions[eventName])
      mockEventBus.subscriptions[eventName] = [];
    mockEventBus.subscriptions[eventName].push(callback);
    return jest.fn(() => {
      if (mockEventBus.subscriptions && mockEventBus.subscriptions[eventName]) {
        // Chequeo extra
        mockEventBus.subscriptions[eventName] = mockEventBus.subscriptions[
          eventName
...
const RepositoryManager =
  require("../../../../../src/components/settings/repository-manager").default;

// --- Test Data ---
// Usar una copia profunda de los datos para cada test para evitar mutaciones accidentales
const getMockRepositoriesData = () =>
  JSON.parse(
    JSON.stringify({
      "repo-official": {
        id: "repo-official",
        name: "Official Atlas Repo",
        url: "https://atlas.official.repo",
        apiEndpoint: "https://atlas.official.api",
...
describe("RepositoryManager Component", () => {
  ...
  beforeEach(() => {
    ...
    mockPluginRepositoryManager.getRepositories.mockReturnValue(
      currentMockRepositoriesData
    );
  ...
  test("debe añadir un nuevo repositorio", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Nombre"), {
      target: { value: "New Awesome Repo" },
    });
    fireEvent.change(screen.getByLabelText("URL"), {
      target: { value: "https://new.repo.com" },
    });

    const addButtonInDialog = within(
      screen.getByTestId("mock-dialog")
    ).getByText("Añadir");
    ...
    await waitFor(() => {
      expect(mockPluginRepositoryManager.addRepository).toHaveBeenCalledWith(
  ...
  test("debe mostrar error si faltan campos obligatorios al añadir", async () => {
    ...
    expect(
      await screen.findByText("ID, nombre y URL son campos obligatorios")
    ).toBeInTheDocument();
  ...
  test("debe abrir el diálogo para editar un repositorio", async () => {
    render(<RepositoryManager onBack={mockOnBack} />);
    const communityRepoItem = (
      await screen.findByText("Community Showcase")
    ).closest(".repository-item");
  ...
  test("debe editar un repositorio existente", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Nombre"), {
      target: { value: "Updated Community Repo" },
    });

    const saveButtonInDialog = within(
      screen.getByTestId("mock-dialog")
    ).getByText("Guardar");
    await act(async () => {
    ...
    await waitFor(() => {
      expect(mockPluginRepositoryManager.updateRepository).toHaveBeenCalledWith(
        "repo-community",
        expect.objectContaining({
  ...
  test("debe deshabilitar/habilitar un repositorio", async () => {
    ...
    await waitFor(() => {
      expect(mockPluginRepositoryManager.toggleRepository).toHaveBeenCalledWith(
        "repo-community",
        false
      );
    });
    ...
    act(() => {
      mockEventBus._simulateEvent("pluginSystem.repositoryToggled", {
        repositoryId: "repo-community",
        enabled: false,
      });
    });
    ...
    expect(
      await within(communityRepoItem).findByText("Habilitar")
    ).toBeInTheDocument();
  });
  ...
  test("debe eliminar un repositorio (no oficial)", async () => {
    ...
    await waitFor(() => {
      expect(mockPluginRepositoryManager.removeRepository).toHaveBeenCalledWith(
        "repo-community"
      );
    });
    expect(screen.queryByTestId("mock-dialog")).not.toBeInTheDocument();
  ...
  test("debe sincronizar un repositorio específico", async () => {
    ...
    await waitFor(() => {
      expect(mockPluginRepositoryManager.syncRepository).toHaveBeenCalledWith(
        "repo-community"
      );
    });
  });
  ...
  test("debe sincronizar todos los repositorios", async () => {
    ...
    await waitFor(() => {
      expect(
        mockPluginRepositoryManager.syncAllRepositories
      ).toHaveBeenCalled();
    });
  });
  ...
  test('debe mostrar "Cargando repositorios..." durante la carga', async () => {
    ...
    expect(
      await screen.findByText("Official Atlas Repo", {}, { timeout: 500 })
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Cargando repositorios...")
  ...
  test("debe manejar error al cargar repositorios", async () => {
    ...
    mockPluginRepositoryManager.initialize.mockRejectedValueOnce(
      new Error("Init failed")
    );

    render(<RepositoryManager onBack={mockOnBack} />);
    expect(
      await screen.findByText("No se pudieron cargar los repositorios")
    ).toBeInTheDocument();
