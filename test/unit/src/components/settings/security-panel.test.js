...
jest.mock('../../../../../src/components/ui/dialog', () => {
  return jest.fn(({ isOpen, onClose, title, children }) => {
    ...
    return (
      <div data-testid="mock-dialog" aria-label={title}> {/* El título del diálogo se usa como aria-label */}
        <h2 data-testid="dialog-actual-title">{title}</h2> {/* Añadimos un testid al título real */}
        {children}
...
const mockThreatsDashboard = jest.fn(({ onPluginClick }) => <div data-testid="mock-threats-dashboard" onClick={() => onPluginClick('pluginX')}>ThreatsDashboardMock</div>);
const mockPermissionsManager = jest.fn(({ onPluginClick }) => <div data-testid="mock-permissions-manager" onClick={() => onPluginClick('pluginY')}>PermissionsManagerMock</div>);
const mockAuditDashboard = jest.fn(({ onPluginClick }) => <div data-testid="mock-audit-dashboard" onClick={() => onPluginClick('pluginZ')}>AuditDashboardMock</div>);

jest.mock('../../../../../src/components/security/threats-dashboard', () => mockThreatsDashboard);
jest.mock('../../../../../src/components/security/permissions-manager', () => mockPermissionsManager);
jest.mock('../../../../../src/components/security/audit-dashboard', () => mockAuditDashboard);
...
jest.mock('../../../../../src/core/plugins/plugin-security-manager', () => mockPluginSecurityManager);

const mockPluginSecurityAudit = {
  getAuditLog: jest.fn(() => []),
  clearAllAuditLogs: jest.fn().mockResolvedValue(true),
};
jest.mock('../../../../../src/core/plugins/plugin-security-audit', () => mockPluginSecurityAudit);
...
const SecurityPanel = require('../../../../../src/components/settings/security-panel').default;
...
describe('SecurityPanel Component', () => {
  const mockSecurityStatsData = {
    ...
    threats: { recent: [{ pluginId: 'pluginX', timestamp: Date.now(), severity: 'high', type: 'test-threat', actionTaken: 'blocked' }] },
  };
  const mockPluginSecurityInfoData = {
      ...
      resourceUsage: { operationCounts: { cpuTime: 10, totalOperations: 100 }, resources: { storage: 1024 } },
      auditHistory: [{ timestamp: Date.now(), auditType: 'test', eventType: 'testEvent' }],
  ...
  beforeEach(() => {
    ...
    mockPluginManager.getPendingPermissionRequests.mockReturnValue([{ pluginId: 'pluginY', permissions: ['network'], timestamp: Date.now() }]);
    mockPluginManager.getPluginSecurityInfo.mockImplementation((pluginId) => {
        ...
        return { // Devolver un objeto por defecto si no es pluginX
            id: pluginId,
            securityScore: 50,
    ...
    mockPluginSecurityAudit.getAuditLog.mockReturnValue([{ timestamp: Date.now(), pluginId: 'pluginZ', auditType: 'testAudit', eventType: 'logEvent', details: {} }]);
  ...
  test('debe renderizar el panel de seguridad y cargar datos iniciales', async () => {
    ...
    expect(mockPluginSecurityAudit.getAuditLog).toHaveBeenCalledWith(expect.objectContaining({ limit: 50 }));
  ...
  test('debe mostrar mensaje si el sistema de seguridad está desactivado', async () => {
    mockPluginManager.getSecurityStats.mockReturnValueOnce({ securityEnabled: false });
    render(<SecurityPanel />);
    expect(await screen.findByText('Sistema de Seguridad Desactivado')).toBeInTheDocument();
  });
  ...
  test('debe cambiar el nivel de seguridad', async () => {
    ...
    const highLevelRadio = screen.getByLabelText(/Alto/i, { selector: 'input[type="radio"]' });
    await act(async () => {
      fireEvent.click(highLevelRadio);
    });

    await waitFor(() => {
      expect(mockPluginManager.setSecurityLevel).toHaveBeenCalledWith(PLUGIN_CONSTANTS.SECURITY.LEVEL.HIGH);
    });
  ...
  test('debe activar/desactivar una verificación de seguridad', async () => {
    ...
    const resourceUsageCheckbox = screen.getByLabelText(/Monitoreo de Recursos/i);
    ...
    await waitFor(() => {
      expect(mockPluginSecurityManager.toggleSecurityCheck).toHaveBeenCalledWith('resourceUsage', false);
    });
    ...
    await waitFor(() => {
      expect(mockPluginSecurityManager.toggleSecurityCheck).toHaveBeenCalledWith('resourceUsage', true);
    });
    expect(resourceUsageCheckbox).toBeChecked();
  });
  ...
  test('debe mostrar detalles de seguridad de un plugin desde ThreatsDashboard', async () => {
    ...
    expect(screen.getByTestId('dialog-actual-title')).toHaveTextContent('Seguridad de Plugin: pluginX');
  ...
  test('debe abrir el diálogo de detalles de seguridad y mostrar información del plugin (prueba directa de showPluginSecurity)', async () => {
    ...
    expect(await screen.findByTestId('mock-dialog')).toBeInTheDocument();
    expect(screen.getByTestId('dialog-actual-title')).toHaveTextContent('Seguridad de Plugin: pluginY');
  });
