...
jest.mock(
  ...
  () => {
    ...
    const {
      PLUGIN_CONSTANTS: Mocked_PLUGIN_CONSTANTS,
    } = require("../../../../../src/core/config/constants");
    return jest.fn(({ zoneId, render, fallback }) => {
      if (
        render &&
        zoneId === Mocked_PLUGIN_CONSTANTS.UI_EXTENSION_ZONES.MAIN_NAVIGATION
      ) {
...
const MockNavItemComponent = ({
  ...
  <button
    data-testid={`nav-${pluginId}-${extensionId}`}
    onClick={() => onNavigate({ pluginId, pageId: label.toLowerCase() })}
  >
    {label} - {customProp}
  </button>
);
...
describe("NavigationExtensions Component", () => {
    ...
    mockNavExtensionsProvided = [
      {
        id: "nav1",
        pluginId: "pluginX",
        component: MockNavItemComponent,
        props: { label: "DashboardX", customProp: "PropX" },
      },
      {
        id: "nav2",
        pluginId: "pluginY",
        component: MockNavItemComponent,
        props: { label: "SettingsY", customProp: "PropY" },
      },
    ];
    ...
    mockNavExtensionsProvided = [
      {
        ...
        pluginId: "pluginX",
        component: MockNavItemComponent,
        props: { label: "TestNav", customProp: "TestProp" },
      },
    ];
  ...
  test("renders nothing meaningful (empty fragment from render prop) if ExtensionPoint provides no extensions", () => {
    ...
    const { container } = render(
      <NavigationExtensions onNavigate={mockOnNavigate} />
    );
