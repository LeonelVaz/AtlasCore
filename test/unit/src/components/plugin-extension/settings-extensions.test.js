...
describe("SettingsExtensions Component", () => {
  ...
  test("renders a title and ExtensionPoint with correct zoneId", () => {
    ...
    expect(screen.getByTestId("mock-extension-point")).toHaveTextContent(
      `Mocked ExtensionPoint for ${zoneId}`
    );
  });
  ...
  test("passes the correct fallback content to ExtensionPoint", () => {
    ...
    expect(
      getByText("No hay extensiones de configuración disponibles.")
    ).toBeInTheDocument();
    expect(
      getByText(
        "Las extensiones aparecerán aquí cuando instales plugins que añadan opciones de configuración."
      )
    ).toBeInTheDocument();
    expect(
      document.querySelector(".settings-extensions-empty")
    ).toBeInTheDocument();
    expect(
      document.querySelector(".settings-extensions-note")
    ).toBeInTheDocument();
  });

  test("applies className prop to the root div", () => {
    const { container } = render(
      <SettingsExtensions className="custom-settings-class" />
    );
