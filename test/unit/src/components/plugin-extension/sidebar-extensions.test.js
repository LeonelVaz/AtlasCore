...
jest.mock(
  ...
  () => {
    return jest.fn(
      ({ zoneId, fallback, "data-extension-zone": dataExtensionZone }) => (
        <div
          data-testid="mock-extension-point"
          data-zoneid={zoneId}
          data-fallback={fallback !== null ? "has-fallback" : "no-fallback"}
        >
...
describe("SidebarExtensions Component", () => {
  ...
  test("renders ExtensionPoint with correct zoneId and null fallback", () => {
    ...
    expect(screen.getByTestId("mock-extension-point")).toHaveAttribute(
      "data-fallback",
      "no-fallback"
    );
  });
  ...
  test("applies className prop to the root div", () => {
    const { container } = render(
      <SidebarExtensions className="custom-sidebar-class" />
    );
