...
const MockPage1Component = ({ pageId, pluginId, extensionId, title }) => (
  <div data-testid={`page-${pluginId}-${pageId}`}>
    <h1>{title}</h1>
    <p>
      Plugin: {pluginId}, Extension: {extensionId}
    </p>
  </div>
);
...
describe("PluginPages Component", () => {
  ...
  test("renders the correct plugin page if currentPluginPage matches an extension", () => {
    ...
    mockExtensionsProvidedToRender = [
      {
        id: "extA",
        pluginId: "pluginA",
        component: MockPage1Component,
        props: { pageId: "dashboard", title: "Plugin A Dashboard" },
      },
      {
        id: "extB",
        pluginId: "pluginB",
        component: MockPage1Component,
        props: { pageId: "settings", title: "Plugin B Settings" },
      },
    ];
    ...
    expect(
      withinRTL(pageContainer).getByText("Plugin: pluginA, Extension: extA")
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("page-pluginB-settings")
  ...
  test('renders "Página no encontrada" if currentPluginPage does not match any extension', () => {
    ...
    expect(screen.getAllByText("Página no encontrada").length).toBeGreaterThan(
      0
    );
    expect(
      screen.getAllByText("La página solicitada no está disponible.").length
    ).toBeGreaterThan(0);
    expect(
      screen.queryByTestId("page-pluginA-dashboard")
    ).not.toBeInTheDocument();
  });
  ...
  test("applies className prop to the root div", () => {
    ...
    const { container } = render(
      <PluginPages currentPluginPage={currentPage} className="custom-class" />
    );
  ...
  test("ExtensionPoint receives the correct fallback message", () => {
    ...
    render(
      <PluginPages currentPluginPage={{ pluginId: "p1", pageId: "pg1" }} />
    );
    ...
    expect(
      withinRTL(fallbackContainer).getByText("Página no encontrada")
    ).toBeInTheDocument();
    expect(
      withinRTL(fallbackContainer).getByText(
