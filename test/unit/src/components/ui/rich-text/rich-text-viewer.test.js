...
describe('RichTextViewer Component', () => {
  const getViewerContainer = () => screen.getByTestId('rich-text-viewer-container');
  ...
  test('debe renderizar contenido HTML simple', () => {
    ...
    expect(screen.getByText((content, element) => element.tagName.toLowerCase() === 'p' && content.startsWith('Hola'))).toBeInTheDocument();
  ...
  describe('Sanitización de HTML', () => {
    test('debe sanitizar scripts por defecto', () => {
      const maliciousContent = '<p>Contenido <script>alert("XSS")</script> seguro.</p>';
      ...
      expect(viewer.innerHTML).toContain('Contenido <span>alert("XSS")</span> seguro.'); 
      expect(screen.getByText((content, el) => el.tagName.toLowerCase() === 'p' && content.includes('Contenido') && content.includes('seguro'))).toBeInTheDocument();
    });

    test('debe remover atributos de evento peligrosos (onclick)', () => {
      const maliciousContent = '<p><a href="#" onclick="alert(\'XSS\')">Click Me</a></p>';
      render(<RichTextViewer content={maliciousContent} />);
    ...
    test('debe remover href con javascript: URL', () => {
      const maliciousContent = '<p><a href="javascript:alert(\'XSS\')">Evil Link</a></p>';
      render(<RichTextViewer content={maliciousContent} />);
    ...
    test('debe mantener atributos permitidos como href y src válidos', () => {
        const validContent = '<p><a href="https://example.com">Link</a> <img src="image.png" alt="alt text"></p>';
    ...
    test('debe remover etiquetas no permitidas, reemplazándolas por span y manteniendo el contenido', () => {
      const contentWithForbiddenTag = '<p>Texto <customtag>interno</customtag></p>';
    ...
    test('no debe sanitizar si la prop sanitize es false', () => {
      const maliciousContent = '<p>Contenido <script>alert("XSS")</script> inseguro.</p>';
