...
describe("WindowControls Component", () => {
  ...
  test("cambia entre los iconos de maximizar y restaurar según el estado", async () => {
    ...
    expect(
      initialMaximizeButton.querySelector(".max-icon")
    ).toBeInTheDocument();
    expect(
      initialMaximizeButton.querySelector(".restore-icon")
  ...
  test("aplica la clase window-focused cuando la ventana está enfocada", async () => {
    ...
    await waitFor(() => {
      ...
      expect(container.querySelector(".window-controls")).toHaveClass(
        "window-focused"
      );
    });
    ...
    await waitFor(() => {
      ...
      expect(container2.querySelector(".window-controls")).toHaveClass(
        "window-blurred"
      );
    });
  });
