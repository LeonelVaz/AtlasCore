...
describe('SidebarItem Component', () => {
  ...
  test('debe renderizar con etiqueta y sin icono', () => {
    ...
    expect(sidebarItemDiv.querySelector('.sidebar-item-icon')).not.toBeInTheDocument();
  });
  ...
  test('debe renderizar con etiqueta y un nodo de icono (ej. SVG)', () => {
    ...
    render(<SidebarItem label="Custom" icon={<CustomIcon />} onClick={mockOnClick} />);
    expect(screen.getByText('Custom')).toBeInTheDocument();
    const customIconElement = screen.getByTestId('custom-icon');
    expect(customIconElement.closest('span.sidebar-item-icon')).toBeInTheDocument();
  });
  ...
  test('debe renderizar con etiqueta y un emoji como icono', () => {
    ...
    expect(within(sidebarItemDiv).queryByText((content, node) => node.classList.contains('material-icons'))).not.toBeInTheDocument();
  });

  test('debe tener la clase "active" si la prop active es true', () => {
    render(<SidebarItem label="Active Item" active={true} onClick={mockOnClick} />);
    expect(screen.getByText('Active Item').closest('.sidebar-item')).toHaveClass('active');
  });
  ...
  test('no debe tener la clase "active" si la prop active es false o no se proporciona', () => {
    ...
    expect(screen.getByText('Inactive Item').closest('.sidebar-item')).not.toHaveClass('active');
  });
