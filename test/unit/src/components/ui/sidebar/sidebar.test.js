...
jest.mock(
  "../../../../../../src/components/plugin-extension/navigation-extensions",
  () => {
    ...
    return function MockNavigationExtensions({ onNavigate }) {
      return (
        <div
          ...
          onClick={() => onNavigate && onNavigate("test-plugin-nav")}
        >
          Nav Extensions
        </div>
      );
...
jest.mock(
  "../../../../../../src/components/plugin-extension/sidebar-extensions",
  () => {
    // eslint-disable-next-line react/prop-types
...
describe("Sidebar Component", () => {
  ...
  test("debe renderizar correctamente con children pero sin la prop onPluginNavigate (undefined)", () => {
    ...
    expect(screen.getByText("Atlas Core")).toBeInTheDocument();
    expect(
      screen.getByText("Child Content For No Nav Prop")
    ).toBeInTheDocument();
