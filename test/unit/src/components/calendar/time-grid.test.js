...
jest.mock("../../../../../src/components/calendar/event-item", () => {
  return jest.fn(
    ({
      ...
      >
        {event.title}
        <button onClick={() => onUpdate({ ...event, title: "Updated" })}>
          Update
        </button>
      </div>
    )
...
const mockUseTimeGrid = {
  ...
  shouldShowEventStart: jest.fn((event, day, hour, minutes, duration) => {
    ...
    return (
      ...
      eventStart.getMinutes() < minutes + duration
    );
  }),
  ...
  formatTimeSlot: jest.fn(
    (hour, minutes = 0) => `${hour}:${minutes.toString().padStart(2, "0")}`
  ),
...
jest.mock("../../../../../src/hooks/use-time-grid", () =>
  jest.fn(() => mockUseTimeGrid)
);

// Mock ExtensionPoint
const MockExtensionComponent = jest.fn(
  ({ date, hour, minutes, pluginId, extensionId }) => (
...
jest.mock(
  "../../../../../src/components/plugin-extension/extension-point",
  () => {
    const {
      PLUGIN_CONSTANTS,
    ...
    return jest.fn(({ zoneId, render, fallback }) => {
      if (
        ...
        const mockExtensions = [
          {
            id: "ext-dh-1",
            pluginId: "pluginDayHeader",
            component: MockExtensionComponent,
            props: {},
          },
        ];
...
describe("TimeGrid Component", () => {
  ...
  const mockEvents = [
    {
      id: "ev1",
      title: "Event 1",
      start: new Date(2023, 0, 2, 10, 0).toISOString(),
      end: new Date(2023, 0, 2, 11, 0).toISOString(),
    },
    {
      id: "ev2",
      title: "Event Cont",
      start: new Date(2023, 0, 1, 22, 0).toISOString(),
      end: new Date(2023, 0, 2, 2, 0).toISOString(),
    },
  ];
  ...
  beforeEach(() => {
    ...
    mockUseTimeGrid.shouldShowEventStart.mockImplementation(
      (event, day, hour, minutes, duration) => {
        const eventStart = new Date(event.start);
        ...
        return (
          eventDay.getTime() === compareDay.getTime() &&
          eventStart.getHours() === hour &&
          eventStart.getMinutes() >= minutes &&
          eventStart.getMinutes() < minutes + duration
        );
    ...
    mockUseTimeGrid.isEventActiveAtStartOfDay.mockImplementation(
      (event, day) => {
        const eventStart = new Date(event.start);
        const eventEnd = new Date(event.end);
  ...
  test("renders grid structure with headers and time slots", () => {
    ...
    expect(screen.getAllByTestId("calendar-day-header")).toHaveLength(
      mockDays.length
    );
    mockDays.forEach((day) => {
      expect(mockRenderDayHeader).toHaveBeenCalledWith(day);
      expect(
        screen.getByText(`Header for ${day.getDate()}`)
      ).toBeInTheDocument();
    });
    ...
    expect(screen.getAllByTestId("calendar-time-slot")).toHaveLength(
      mockUseTimeGrid.hours.length * mockDays.length
    );
  });
  ...
  test("handles cell clicks", () => {
    ...
    const nineAmCells = screen
      .getAllByTestId("calendar-time-slot")
      .filter((cell) => cell.dataset.hour === "9");
  ...
  test("renders custom time slots and handles add/remove", () => {
    ...
    expect(screen.getByTestId("calendar-time-slot-custom")).toHaveAttribute(
      "data-minutes",
      "30"
    );
  ...
  test("applies compact class when time scale is compact", () => {
    ...
    expect(
      screen
        .getByText(`Header for ${mockDays[0].getDate()}`)
        .closest(".calendar-grid")
    ).toHaveClass("time-scale-compact");
  });
  ...
  test("renders ExtensionPoints for day headers and hour cells", () => {
    ...
    const allExtensions = screen.getAllByTestId(
      "extension-pluginDayHeader-ext-dh-1"
    );
    ...
    const dayHeaderExtension = allExtensions.find((el) => {
      ...
      return (
        text.includes(mockDays[0].toISOString().split("T")[0]) &&
        !text.includes(" H:") &&
        !text.includes(" M:")
      );
    });
    ...
    if (dayHeaderExtension) {
      expect(dayHeaderExtension).toHaveTextContent(
        mockDays[0].toISOString().split("T")[0]
      );
    }
    ...
    if (hourCellExtension) {
      expect(hourCellExtension).toHaveTextContent(
        mockDays[0].toISOString().split("T")[0]
      );
    }
  });
  ...
  test("calculates event positioning for simultaneous events", () => {
    const simultaneousEvents = [
      ...
      },
      {
        id: "s2",
        title: "Sim Event 2",
        start: new Date(2023, 0, 2, 10, 15).toISOString(),
        end: new Date(2023, 0, 2, 11, 15).toISOString(),
      },
    ];
    mockUseTimeGrid.shouldShowEventStart.mockImplementation(
      (event, day, hour, minutes, duration) => {
    ...
    mockUseTimeGrid.eventsOverlapInTimeSlot.mockImplementation(
      (eventA, eventB) => {
