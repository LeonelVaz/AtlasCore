...
describe('EventItem Component', () => {
  ...
  beforeEach(() => {
    ...
    const { useEventResize } = require('../../../../../src/hooks/use-event-resize');
  ...
  test('renders event title and time', () => {
    render(<EventItem event={mockEvent} onClick={mockOnClick} onUpdate={mockOnUpdate} />);
  ...
  test('applies background color from event', () => {
    render(<EventItem event={mockEvent} onClick={mockOnClick} onUpdate={mockOnUpdate} />);
    const eventElement = screen.getByText(mockEvent.title).closest('.calendar-event');
  ...
  test('calls handleResizeStart on mousedown on resize handle', () => {
    render(<EventItem event={mockEvent} onClick={mockOnClick} onUpdate={mockOnUpdate} />);
    const resizeHandle = screen.getByText(mockEvent.title).closest('.calendar-event').querySelector('.event-resize-handle');
  ...
  test('does not call onClick if blockClicks is true (simulated by hook)', (done) => {
    ...
    const { useEventDrag } = require('../../../../../src/hooks/use-event-drag');
    useEventDrag.mockImplementationOnce(({ eventRef, event, onUpdate, gridSize, snapValue, setBlockClicks }) => {
        ...
        return {
            ...mockUseEventDrag,
            handleDragStart: (e) => {
                if (eventRef.current) eventRef.current.dataset.recentlyResized = 'true'; // Simulate resize
  ...
  test('applies conditional classes: dragging, resizing, continues', () => {
    ...
    let eventElement = screen.getByText(mockEvent.title).closest('.calendar-event');
  ...
  test('does not render resize handle if continuesNextDay is true', () => {
    render(<EventItem event={mockEvent} onClick={mockOnClick} onUpdate={mockOnUpdate} continuesNextDay={true} />);
    const eventElement = screen.getByText(mockEvent.title).closest('.calendar-event');
    expect(eventElement.querySelector('.event-resize-handle')).not.toBeInTheDocument();
  });
  ...
  test('passes correct arguments to hooks', () => {
    ...
    const { useEventDrag } = require('../../../../../src/hooks/use-event-drag');
    expect(useEventDrag).toHaveBeenCalledWith(expect.objectContaining({
      event: mockEvent,
      onUpdate: mockOnUpdate,
      gridSize: 50,
      snapValue: 10,
    ...
    const { useEventResize } = require('../../../../../src/hooks/use-event-resize');
    expect(useEventResize).toHaveBeenCalledWith(expect.objectContaining({
      event: mockEvent,
      onUpdate: mockOnUpdate,
      gridSize: 50,
      snapValue: 10,
