...
jest.mock("../../../../../src/components/ui/dialog", () => {
  return jest.fn(
    ({ children, isOpen, onClose, title, confirmText, onConfirm }) =>
      isOpen ? (
...
const MockExtensionComponent = jest.fn(
  ({ event, isEditing, onChange, pluginId, extensionId, ...props }) => (
    <div data-testid={`extension-${pluginId}-${extensionId}`}>
      ...
      <input
        type="text"
        data-testid={`ext-input-${extensionId}`}
        name={`extInput_${extensionId}`}
        onChange={onChange}
      />
      <button
        onClick={() =>
          onChange({
            target: {
              name: `customFieldExt_${extensionId}`,
              value: `extValueFromButton_${extensionId}`,
            },
          })
        }
      >
        ...
        const mockExtensions = [
          {
            ...
            props: { customProp: "valA" },
          },
          {
            id: "ext2",
            pluginId: "pluginB",
            component: MockExtensionComponent,
            props: { customProp: "valB" },
          },
        ];
...
describe("EventForm Component", () => {
  ...
  test("calls onChange prop and updates input value when user types", async () => {
    ...
    const simulateParentOnChange = (e) => {
      ...
      if (name === "start" || name === "end") {
        ...
        // Para otros campos como 'title', 'color'
        currentEventInTest = {
          ...currentEventInTest,
          [name]: value,
        };
    ...
    fireEvent.change(startInput, {
      target: { name: "start", value: newStartTimeRaw },
    });
  ...
  test("renderiza extensiones y pasa las props correctamente", async () => {
    ...
    let currentEventInTestExt = JSON.parse(
      JSON.stringify({
        title: "For Extension",
        startFormatted: "",
        endFormatted: "",
    ...
    const formExtensionsContainer = screen
      .getByLabelText("Título:")
      .closest(".event-form-content")
      .querySelector(".event-form-extensions");
    const extensionPluginA_form = within(formExtensionsContainer).getByTestId(
      "extension-pluginA-ext1"
    );

    const formExtensionACall = MockExtensionComponent.mock.calls.find(
      (call) => {
        const props = call[0];
        return (
          props.pluginId === "pluginA" &&
    ...
    const changeButtonInFormExtension = within(extensionPluginA_form).getByText(
      "Change Custom Button"
    );
    await user.click(changeButtonInFormExtension);
    expect(currentEventInTestExt["customFieldExt_ext1"]).toBe(
      "extValueFromButton_ext1"
    );

    const extInput = within(extensionPluginA_form).getByTestId(
      "ext-input-ext1"
    );
