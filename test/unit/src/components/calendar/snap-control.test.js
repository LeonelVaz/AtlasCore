...
describe('SnapControl Component', () => {
  ...
  test('renders toggle button and value indicator', () => {
    render(<SnapControl snapValue={SNAP_VALUES.NONE} onSnapChange={mockOnSnapChange} />);
    expect(screen.getByTitle('Activar/Desactivar imán')).toBeInTheDocument();
    expect(screen.getByTitle('Configurar precisión de imán')).toBeInTheDocument();
    expect(screen.getByText('Off')).toBeInTheDocument(); // Label for NONE
  });

  test('toggle button activates/deactivates snap', () => {
    const { rerender } = render(<SnapControl snapValue={SNAP_VALUES.NONE} onSnapChange={mockOnSnapChange} />);
    ...
    rerender(<SnapControl snapValue={SNAP_VALUES.PRECISE} onSnapChange={mockOnSnapChange} />);
    fireEvent.click(toggleButton);
  ...
  test('value indicator shows correct label for different snap values', () => {
    ...
    rerender(<SnapControl snapValue={SNAP_VALUES.BASIC} onSnapChange={mockOnSnapChange} />);
    expect(screen.getByText('1h')).toBeInTheDocument();

    rerender(<SnapControl snapValue={SNAP_VALUES.MEDIUM} onSnapChange={mockOnSnapChange} />);
  ...
  test('clicking outside the menu closes it', () => {
    render(
      <div>
        <SnapControl snapValue={SNAP_VALUES.NONE} onSnapChange={mockOnSnapChange} />
  ...
  test('selected option has "selected" class', () => {
    render(<SnapControl snapValue={SNAP_VALUES.MEDIUM} onSnapChange={mockOnSnapChange} />);
    fireEvent.click(screen.getByTitle('Configurar precisión de imán'));
