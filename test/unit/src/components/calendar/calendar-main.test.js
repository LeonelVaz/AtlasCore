  ...
  jest.fn(
    ({
      ...
      <div data-testid="week-view-mock">
        Week View - MaxSim: {maxSimultaneousEvents} - Snap: {snapValue} -
        Events: {events.length} - Date: {currentDate.toISOString()}
        <button
          data-testid="week-event-click"
          onClick={() =>
            onEventClick({
              id: "event1",
              title: "Test Event Week",
              start: new Date().toISOString(),
              end: new Date(Date.now() + 3600000).toISOString(),
            })
          }
        >
          Simulate Week Event Click
        </button>
        <button
          data-testid="week-cell-click"
          onClick={() => onCellClick(new Date(), 9, 0, 60)}
        >
          Simulate Week Cell Click
        </button>
        <button
          data-testid="week-update-event"
          onClick={() =>
            onUpdateEvent("event1-id", { title: "Updated Week Event" })
          }
        >
          Simulate Week Update Event
        </button>
      </div>
    )
  )
  ...
  jest.fn(
    ({
      ...
      <div data-testid="day-view-mock">
        Day View - MaxSim: {maxSimultaneousEvents} - Snap: {snapValue} - Events:{" "}
        {events.length} - Date: {date.toISOString()}
        <button
        ...
        >
          Simulate Day Event Click
        </button>
        <button
          data-testid="day-cell-click"
          onClick={() => onTimeSlotClick(new Date(), 10, 30, 30)}
        >
          Simulate Day Cell Click
        </button>
        <button
          data-testid="day-update-event"
          onClick={() => onUpdate("event2-id", { title: "Updated Day Event" })}
        >
          Simulate Day Update Event
        </button>
      </div>
    )
  )
...
jest.mock("../../../../../src/components/calendar/event-form", () =>
  jest.fn(
    ({ onSave, onClose, onDelete, onChange, event, isEditing, error }) => (
      <div data-testid="event-form-mock">
        Event Form: {isEditing ? "Editing" : "New"} - Title: {event?.title}{" "}
        {error ? `- Error: ${error}` : ""}
        <button data-testid="form-save" onClick={onSave}>
          Save
        </button>
        ...
        />
        <input
          data-testid="form-start-input"
          type="datetime-local"
          name="start"
          defaultValue={event?.startFormatted || ""}
          onChange={onChange}
        />
        <input
          data-testid="form-end-input"
          type="datetime-local"
          name="end"
          defaultValue={event?.endFormatted || ""}
          onChange={onChange}
        />
      </div>
    )
...
jest.mock("../../../../../src/components/calendar/snap-control", () => {
  const {
    SNAP_VALUES: ImportedSnapValues,
  } = require("../../../../../src/core/config/constants");
  return jest.fn(({ snapValue, onSnapChange }) => (
    <div data-testid="snap-control-mock">
      Snap: {snapValue}
      <button
        data-testid="snap-change-button"
        onClick={() => onSnapChange(ImportedSnapValues.MEDIUM)}
      >
        Set Snap Medium
      </button>
    </div>
...
jest.mock("../../../../../src/hooks/use-calendar-events", () => {
  const internalInitialMockEvents = [
    {
      id: "ev1",
      title: "Initial Event Factory",
      start: new Date().toISOString(),
      end: new Date(Date.now() + 3600000).toISOString(),
    },
  ];
...
let mockNewEventState = {
  id: "",
  title: "Nuevo evento mock",
  start: new Date().toISOString(),
  end: new Date(Date.now() + 3600000).toISOString(),
  startFormatted: "2023-01-01T10:00",
  endFormatted: "2023-01-01T11:00",
  color: "#2d4b94",
};
...
jest.mock("../../../../../src/hooks/use-event-form", () =>
  jest.fn(
    (createEvent, updateEvent, deleteEvent, events, maxSimultaneousEvents) => ({
...
import {
  registerModule,
  unregisterModule,
} from "../../../../../src/core/modules/module-registry";
...
describe("Componente CalendarMain", () => {
  ...
  test("cambia a la vista de día, actualiza el título y la navegación", () => {
    ...
    const dayViewButton = screen
      .getAllByText("Día")
      .find((btn) => btn.closest(".calendar-view-toggle"));
    ...
    const todayButtonDay = screen
      .getAllByText("Hoy")
      .find((btn) => btn.closest(".calendar-navigation"));
  ...
  describe("Interacciones del Formulario de Eventos", () => {
    ...
    test("muestra el formulario para editar un evento, maneja guardado y eliminación", () => {
      ...
      mockNewEventState = {
        // El estado del formulario se actualiza para reflejar el evento seleccionado
        id: "event123",
        title: "Evento Existente",
        start: new Date().toISOString(),
        end: new Date(Date.now() + 3600000).toISOString(),
        startFormatted: "2023-01-02T10:00",
        endFormatted: "2023-01-02T11:00",
        color: "#ff0000",
      };
      render(<CalendarMain />);
      expect(screen.getByTestId("event-form-mock")).toBeInTheDocument();
    ...
    test("maneja el cambio de input del formulario y el cierre", () => {
      ...
      fireEvent.change(titleInput, {
        target: { name: "title", value: "Nuevo Título Ingresado" },
      });
      expect(mockHandleEventFormChange).toHaveBeenCalledWith(
        expect.objectContaining({
          target: expect.objectContaining({
            name: "title",
            value: "Nuevo Título Ingresado",
          }),
        })
      );
    ...
    test("muestra el error del formulario si está presente", () => {
      ...
      render(<CalendarMain />);
      expect(
        screen.getByText(/Error: Este es un error de prueba/)
      ).toBeInTheDocument();
    });
  });
  ...
  test("maneja el clic en celda desde WeekView y DayView", () => {
    ...
    expect(mockHandleCellClick).toHaveBeenLastCalledWith(
      expect.any(Date),
      10,
      30,
      30
  ...
  test("maneja el clic en evento desde WeekView y DayView", () => {
    ...
    expect(mockHandleEventClick).toHaveBeenCalledWith(
      expect.objectContaining({ title: "Test Event Week" })
    );
    ...
    expect(mockHandleEventClick).toHaveBeenLastCalledWith(
      expect.objectContaining({ title: "Test Event Day" })
    );
  });
  ...
  test("maneja la actualización de evento desde WeekView y DayView (prop de actualización directa)", () => {
    ...
    expect(mockUpdateEvent).toHaveBeenCalledWith("event1-id", {
      title: "Updated Week Event",
    });
    ...
    expect(mockUpdateEvent).toHaveBeenCalledWith("event2-id", {
      title: "Updated Day Event",
    });
  });
  ...
  test("la suscripción de eventBus para maxSimultaneousEventsChanged funciona", async () => {
    ...
    const busSubscriptionCall = eventBus.subscribe.mock.calls.find(
      (call) => call[0] === "calendar.maxSimultaneousEventsChanged"
    );
