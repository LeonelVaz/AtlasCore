  ...
  return jest.fn(
    ({
      ...
      <div data-testid="time-grid-mock">
        ...
        <button onClick={() => onEventClick({ id: "test-event" })}>
          ClicEnEvento
        </button>
        <button
          onClick={() =>
            onCellClick(new Date(days[0]).setHours(10, 30, 0, 0), 10, 30, 30)
          }
        >
          ClicEnCelda
        </button>
        <button onClick={() => onUpdateEvent({ id: "updated-event" })}>
          ActualizarEvento
        </button>
      </div>
    )
...
describe("Componente DayView", () => {
  ...
  const mockEventos = [
    {
      ...
      title: "Evento 1",
      start: mockFecha,
      end: new Date(mockFecha.getTime() + 3600000),
    },
  ];
  ...
  test("renderiza TimeGrid con las props correctas", () => {
    ...
    expect(
      screen.getByText(`Fecha: ${mockFecha.toISOString()}`)
    ).toBeInTheDocument();
  ...
  test("renderDayHeader formatea y muestra la información del día correctamente", () => {
    ...
    expect(
      screen.getByText(mockFecha.getDate().toString())
    ).toBeInTheDocument();
