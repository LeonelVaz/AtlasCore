...
const MOCK_PERMISSION_CHECKER_PATH = '../../../../../src/core/plugins/plugin-permission-checker';
const mockPluginAPermissions = { approved: ['readData', 'writeData'], pending: ['sendNotifications'], revoked: ['accessCamera'] };
const mockPluginBPermissions = { approved: ['accessLocation'], pending: [], revoked: [] };
...
const MOCK_PLUGIN_MANAGER_PATH = '../../../../../src/core/plugins/plugin-manager';
const mockAllPlugins = [
  { id: 'pluginA', name: 'Plugin A' }, { id: 'pluginB', name: 'Plugin B' },
  { id: 'pluginC', name: 'Plugin C Vacío' }, { id: 'pluginD', name: 'Plugin D Sin Permisos Registrados' },
];
...
const mockPermissionTypes = {
  READDATA: 'Leer Datos', WRITEDATA: 'Escribir Datos', SENDNOTIFICATIONS: 'Enviar Notificaciones',
  ACCESSCAMERA: 'Acceder a Cámara', ACCESSLOCATION: 'Acceder a Ubicación',
};
...
const PermissionsManager = require('../../../../../src/components/security/permissions-manager').default; 
...
describe('PermissionsManager Component', () => {
  ...
  afterEach(() => {
    ...
    createElementSpy = null; appendChildSpy = null; removeChildSpy = null;
    clickSpy = null; createObjectURLSpy = null; revokeObjectURLSpy = null;
  ...
  test('debe renderizar el gestor completo y cargar datos iniciales', async () => {
    ...
    expect(await screen.findByText('Gestor de Permisos', {}, {timeout:3000})).toBeInTheDocument();
    
    expect(pluginManager.getAllPlugins).toHaveBeenCalledTimes(1);
    expect(pluginPermissionChecker.getPluginPermissions).toHaveBeenCalledWith('pluginA');
    expect(pluginPermissionChecker.getPluginPermissions).toHaveBeenCalledWith('pluginB');
    
    const pendingTab = screen.getByText(/Pendientes \(\d+\)/);
    expect(pendingTab).toHaveClass('active');
    
    expect(await screen.findByText('pluginA', {}, {timeout:1000})).toBeInTheDocument(); 
  ...
  test('debe renderizar la vista compacta y mostrar permisos pendientes', async () => {
    ...
    render(<PermissionsManager compact={true} onPluginClick={mockOnPluginClick} />);
    expect(await screen.findByText('Permisos Pendientes', {}, {timeout:3000})).toBeInTheDocument();
  ...
  test('debe cambiar a la pestaña "Todos los Permisos" y mostrar todos los permisos', async () => {
    ...
    const pluginAEntries = await screen.findAllByText('pluginA', {}, {timeout: 2000});
  ...
  test('debe filtrar permisos en la pestaña "Pendientes"', async () => {
    ...
    const filterInput = screen.getByPlaceholderText(/Filtrar por plugin o permiso.../i);
    
    await act(async () => {
        fireEvent.change(filterInput, { target: { value: 'pluginB' } });
    });
    
    expect(await screen.findByText('No hay permisos pendientes de aprobación', {}, { timeout: 2000 })).toBeInTheDocument();
  ...
  test('debe aprobar un permiso pendiente', async () => {
    ...
    const pluginARow = (await screen.findAllByText((content, node) => {
        const parent = node.closest('.permission-row');
        return parent && parent.textContent.includes('pluginA') && parent.textContent.includes('sendNotifications');
    ...
    expect(pluginManager.approvePluginPermissions).toHaveBeenCalledWith('pluginA', ['sendNotifications']);
    // Esperar a que los datos se recarguen debido al cambio en refreshKey
    await waitFor(() => expect(pluginManager.getAllPlugins).toHaveBeenCalledTimes(1)); 
  ...
  test('debe refrescar los datos al hacer clic en el botón Refrescar', async () => {
    ...
    await waitFor(() => expect(pluginPermissionChecker.getPluginPermissions).toHaveBeenCalledWith('pluginA')); 
  });
});