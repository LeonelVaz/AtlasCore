...
jest.mock('../../../../src/core/config/constants', () => ({
  TIME_SCALES: {
    ...
    COMFORTABLE: { id: 'comfortable', name: 'Confortable', height: 80, pixelsPerMinute: 80 / 60 },
    SPACIOUS: { id: 'spacious', name: 'Espaciosa', height: 100, pixelsPerMinute: 100 / 60 },
...
const { TIME_SCALES, STORAGE_KEYS } = require('../../../../src/core/config/constants');
...
describe('TimeScaleService', () => {
  ...
  describe('getCurrentTimeScale', () => {
    ...
    test('debe devolver una escala personalizada si su ID y altura están en el almacenamiento', async () => {
      ...
      const scale = await timeScaleService.getCurrentTimeScale();
      expect(storageService.get).toHaveBeenCalledWith(STORAGE_KEYS.TIME_SCALE);
      expect(storageService.get).toHaveBeenCalledWith(`${STORAGE_KEYS.TIME_SCALE}_${customId}`);
    ...
    test('debe devolver la escala por defecto en caso de error al leer del storage', async () => {
      ...
      const scale = await timeScaleService.getCurrentTimeScale();
      expect(scale).toEqual(TIME_SCALES.STANDARD);
      expect(console.error).toHaveBeenCalledWith('Error al obtener la escala de tiempo actual:', expect.any(Error));
    });
  });
  ...
  describe('setTimeScale', () => {
    test('debe establecer una escala predefinida por ID y publicar evento', async () => {
      ...
      const result = await timeScaleService.setTimeScale(TIME_SCALES.COMFORTABLE.id);
      expect(result).toBe(true);
      expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.TIME_SCALE, TIME_SCALES.COMFORTABLE.id);
      expect(eventBus.publish).toHaveBeenCalledWith('app.timeScaleChanged', { timeScaleId: TIME_SCALES.COMFORTABLE.id });
    });
    ...
    test('debe establecer una escala personalizada y guardar su altura', async () => {
      ...
      const result = await timeScaleService.setTimeScale({ height: customHeight });
      
      const expectedCustomId = `custom_1234567890`;
      expect(result).toBe(true);
      expect(storageService.set).toHaveBeenCalledWith(`${STORAGE_KEYS.TIME_SCALE}_${expectedCustomId}`, customHeight);
      expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.TIME_SCALE, expectedCustomId);
      expect(eventBus.publish).toHaveBeenCalledWith('app.timeScaleChanged', { timeScaleId: expectedCustomId });
    ...
    test('debe limitar la altura de la escala personalizada entre 20 y 200', async () => {
      ...
      expect(storageService.set).toHaveBeenCalledWith(`${STORAGE_KEYS.TIME_SCALE}_custom_111`, 20);
      ...
      expect(storageService.set).toHaveBeenCalledWith(`${STORAGE_KEYS.TIME_SCALE}_custom_222`, 200);
    ...
    test('debe devolver false si el ID de escala predefinida es inválido', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Escala de tiempo no válida: invalid-id');
    });
    ...
    test('debe devolver false si el objeto de escala personalizada es inválido', async () => {
      ...
      expect(result).toBe(false);
      expect(console.error).toHaveBeenCalledWith('Formato de escala de tiempo no válido:', { invalid: 'scale' });
    });
    ...
    test('debe devolver false en caso de error al guardar en storage', async () => {
      ...
      const result = await timeScaleService.setTimeScale(TIME_SCALES.STANDARD.id);
      expect(result).toBe(false);
  ...
  describe('createCustomTimeScale', () => {
    ...
    test('debe devolver false si la altura es inválida', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Altura de escala no válida. Debe ser un número entero entre 20 y 200 píxeles');
      ...
      console.error.mockClear();
      const result3 = await timeScaleService.createCustomTimeScale("not a number");
  ...
  test('initialize debe llamar a getCurrentTimeScale', async () => {
    const getCurrentTimeScaleSpy = jest.spyOn(timeScaleService, 'getCurrentTimeScale').mockResolvedValue(TIME_SCALES.STANDARD);
