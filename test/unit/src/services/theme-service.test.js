...
describe("ThemeService", () => {
  ...
  describe("getCurrentTheme", () => {
    ...
    test("debe devolver el tema por defecto en caso de error al leer del storage", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error al obtener el tema actual:",
        expect.any(Error)
      );
    });
  });
  ...
  describe("setTheme", () => {
    test("debe establecer un tema válido, guardarlo y aplicarlo", async () => {
      ...
      expect(
        document.documentElement.classList.contains(`theme-${THEMES.DARK}`)
      ).toBe(true);
    ...
    test("debe devolver false si el ID del tema es inválido", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Tema no válido: invalid-theme-id"
      );
    });
  ...
  describe("applyTheme", () => {
    test("debe añadir la clase correcta y el atributo data-theme al html", () => {
      ...
      expect(
        document.documentElement.classList.contains(
          `theme-${THEMES.ATLAS_DARK_BLUE}`
        )
      ).toBe(true);
      expect(document.documentElement.getAttribute("data-theme")).toBe(
        THEMES.ATLAS_DARK_BLUE
      );
    });
    ...
    test("debe eliminar clases de temas anteriores antes de aplicar uno nuevo", () => {
      ...
      expect(
        document.documentElement.classList.contains(`theme-${THEMES.LIGHT}`)
      ).toBe(false);
      expect(
        document.documentElement.classList.contains(
          `theme-${THEMES.DEEP_OCEAN}`
        )
      ).toBe(true);
      expect(document.documentElement.getAttribute("data-theme")).toBe(
        THEMES.DEEP_OCEAN
  ...
  test("initialize debe aplicar el tema por defecto si hay un error al obtener el tema", async () => {
    ...
    expect(console.error).not.toHaveBeenCalledWith(
      "Error al inicializar el servicio de temas:",
      expect.any(Error)
    );
