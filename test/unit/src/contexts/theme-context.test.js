...
const TestConsumer = () => {
  ...
  return (
    <div>
      ...
      <p>
        Available Themes:{" "}
        {context.availableThemes.map((t) => t.name).join(", ")}
      </p>
      <button onClick={async () => await context.changeTheme(THEMES.DARK)}>
        Change to Dark
      </button>
    </div>
  );
...
describe("ThemeProvider and ThemeContext", () => {
  ...
  test("debe inicializar y proveer el tema actual, temas disponibles y estado de carga", async () => {
    ...
    expect(
      screen.getByText(`Current Theme: ${THEMES.LIGHT}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        `Available Themes: ${mockAvailableThemesData
          .map((t) => t.name)
          .join(", ")}`
      )
    ).toBeInTheDocument();
  ...
  test("changeTheme debe llamar a themeService.setTheme y actualizar currentTheme si tiene éxito", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(`Current Theme: ${THEMES.DARK}`)
      ).toBeInTheDocument()
    );
  });
