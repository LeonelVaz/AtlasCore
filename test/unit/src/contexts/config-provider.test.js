...
const mockThemeProvider = jest.fn(({ children }) => <div data-testid="mock-theme-provider">{children}</div>);
const mockTimeScaleProvider = jest.fn(({ children }) => <div data-testid="mock-timescale-provider">{children}</div>);
...
const ConfigProvider = require('../../../../src/contexts/config-provider').default;
...
describe('ConfigProvider Component', () => {
  ...
  test('debe envolver a los hijos con ThemeProvider y TimeScaleProvider', () => {
    ...
    expect(themeProviderNode).toContainElement(screen.getByTestId('mock-timescale-provider'));
