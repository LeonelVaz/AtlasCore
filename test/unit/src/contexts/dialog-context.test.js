...
import { DialogProvider, useDialog } from '../../../../src/contexts/dialog-context';
...
jest.mock('../../../../src/components/ui/dialog-system/custom-dialog', () => {
  ...
  return function MockedCustomDialog(props) {
    ...
    return <div data-testid="mocked-custom-dialog" data-type={props.type}>{props.message}</div>;
  };
});
...
const TestComponent = () => {
  ...
  return (
    <div>
      <button onClick={() => dialog.showAlert('Alert message!')}>Show Alert</button>
      <button onClick={() => dialog.showConfirm('Confirm message?')}>Show Confirm</button>
      <button onClick={() => dialog.showPrompt('Prompt message', 'default')}>Show Prompt</button>
      <button onClick={() => dialog.showCustomDialog({ type: 'alert', message: 'Custom' })}>Show Custom</button>
...
describe('DialogProvider and useDialog Hook', () => {
  ...
  test('useDialog debe lanzar error si no está dentro de un DialogProvider', () => {
    ...
    expect(() => render(<TestComponent />)).toThrow('useDialog debe ser usado dentro de un DialogProvider');
  ...
  describe('Dialog Actions', () => {
    test('showAlert debe mostrar CustomDialog con tipo alert y resolver la promesa', async () => {
      ...
      await waitFor(() => {
        expect(mockCustomDialogComponent).toHaveBeenCalledWith( // CORREGIDO: Sin segundo argumento {}
      ...
      const customDialogProps = mockCustomDialogComponent.mock.calls[lastCallIndex][0];
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('mocked-custom-dialog')).not.toBeInTheDocument();
      });
    });
    ...
    test('showConfirm debe mostrar CustomDialog con tipo confirm y resolver con true/false', async () => {
        ...
        await waitFor(() => expect(mockCustomDialogComponent).toHaveBeenCalledWith( // CORREGIDO
          expect.objectContaining({ 
            isOpen: true, 
            type: 'confirm', 
        ...
        let customDialogProps = mockCustomDialogComponent.mock.calls[lastCallIndex][0];
        await act(async () => { customDialogProps.onConfirm(); });
        let result = await confirmPromise;
        expect(result).toBe(true);
        await waitFor(() => expect(screen.queryByTestId('mocked-custom-dialog')).not.toBeInTheDocument());
        ...
        customDialogProps = mockCustomDialogComponent.mock.calls[lastCallIndex][0];
        await act(async () => { customDialogProps.onCancel(); });
    ...
    test('showPrompt debe mostrar CustomDialog con tipo prompt y resolver con valor o null', async () => {
        ...
        let lastCallIndex = mockCustomDialogComponent.mock.calls.length - 1;
        let customDialogProps = mockCustomDialogComponent.mock.calls[lastCallIndex][0];
        await act(async () => { customDialogProps.onConfirm('user input'); });
    ...
    test('showCustomDialog debe mostrar CustomDialog con opciones personalizadas', async () => {
        ...
        await act(async () => {
            dialogFromHook.showCustomDialog({ type: 'alert', message: 'Custom', title: 'Custom Title' });
        });
    ...
    test('closeDialog debe cerrar un diálogo abierto', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('mocked-custom-dialog')).toBeInTheDocument());
        expect(mockCustomDialogComponent.mock.calls[mockCustomDialogComponent.mock.calls.length - 1][0].isOpen).toBe(true);
