...
import { TimeScaleProvider, TimeScaleContext } from '../../../../src/contexts/time-scale-context';
...
const TestConsumer = () => {
  ...
  return (
    <div>
      ...
      <p>Available Scales: {context.availableTimeScales.map(s => s.name).join(', ')}</p>
      <button onClick={async () => await context.changeTimeScale(TIME_SCALES.COMPACT.id)}>Change to Compact</button>
      <button onClick={async () => await context.createCustomTimeScale(90)}>Create Custom 90px</button>
    </div>
  );
};

describe('TimeScaleProvider and TimeScaleContext', () => {
  const mockAvailableScalesData = [
  ...
  beforeEach(() => {
    ...
    timeScaleService.getAvailableTimeScales.mockReturnValue(mockAvailableScalesData);
    timeScaleService.getCurrentTimeScale.mockResolvedValue(TIME_SCALES.STANDARD); // Default para la primera carga del evento
  ...
  test('debe inicializar y proveer la escala actual, escalas disponibles y estado de carga', async () => {
    ...
    expect(eventBus.subscribe).toHaveBeenCalledWith('app.timeScaleChanged', expect.any(Function));
    ...
    expect(screen.getByText(`Current Scale ID: ${TIME_SCALES.STANDARD.id}`)).toBeInTheDocument();
    expect(screen.getByText(`Available Scales: ${mockAvailableScalesData.map(s => s.name).join(', ')}`)).toBeInTheDocument();
  });
  ...
  test('debe manejar errores durante la inicialización', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Loading: false')).toBeInTheDocument());
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error al inicializar la escala de tiempo:', expect.any(Error));
    expect(screen.getByText(`Current Scale ID: ${TIME_SCALES.STANDARD.id}`)).toBeInTheDocument(); 
  ...
  test('changeTimeScale debe llamar al servicio y actualizar la escala actual si tiene éxito', async () => {
    ...
    const changeButton = screen.getByRole('button', { name: 'Change to Compact' });
    await act(async () => {
      fireEvent.click(changeButton);
    });

    expect(timeScaleService.setTimeScale).toHaveBeenCalledWith(TIME_SCALES.COMPACT.id);
    ...
    await waitFor(() => expect(screen.getByText(`Current Scale ID: ${TIME_SCALES.COMPACT.id}`)).toBeInTheDocument());
  });

  test('createCustomTimeScale debe llamar al servicio y actualizar la escala actual si tiene éxito', async () => {
    const customScale = { id: 'custom', name: 'Personalizada 90px', height: 90, pixelsPerMinute: 90/60 };
    // Simular el estado después de la inicialización
    timeScaleService.initialize.mockResolvedValue(TIME_SCALES.STANDARD);
    ...
    const createButton = screen.getByRole('button', { name: 'Create Custom 90px' });
    await act(async () => {
    ...
    expect(timeScaleService.getCurrentTimeScale).toHaveBeenCalledTimes(1);
    await waitFor(() => expect(screen.getByText(`Current Scale ID: ${customScale.id}`)).toBeInTheDocument());
  });
  ...
  test('debe actualizar la escala si el evento "app.timeScaleChanged" del eventBus se dispara', async () => {
    ...
    timeScaleService.getCurrentTimeScale.mockResolvedValueOnce(TIME_SCALES.COMPACT);
