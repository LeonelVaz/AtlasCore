...
describe('App Component', () => {
  ...
  describe('AppContent Component', () => {
    ...
    test('renders sidebar items correctly', () => {
      ...
      expect(screen.getByTestId('sidebar-item-configuración')).toBeInTheDocument();
      
      // Calendar should be active by default
      expect(screen.getByTestId('sidebar-item-calendario')).toHaveAttribute('data-active', 'true');
      expect(screen.getByTestId('sidebar-item-configuración')).toHaveAttribute('data-active', 'false');
    });
    ...
    test('maintains active state in sidebar when switching sections', () => {
      ...
      expect(screen.getByTestId('sidebar-item-configuración')).toHaveAttribute('data-active', 'true');
      expect(screen.getByTestId('sidebar-item-calendario')).toHaveAttribute('data-active', 'false');
    });
  ...
  describe('Plugin System Integration', () => {
    ...
    test('renders plugin pages when plugin section is active with valid plugin page', () => {
      ...
      expect(screen.getByText('No se ha seleccionado ninguna página de plugin.')).toBeInTheDocument();
      
      // Limpiar el render anterior
      unmount();
      
      // Test con plugin page válida
      render(<PluginPages currentPluginPage={{ pluginId: 'test-plugin', pageId: 'test-page' }} />);
      expect(screen.getByTestId('plugin-pages')).toBeInTheDocument();
      expect(screen.getByText('Plugin Page: test-plugin/test-page')).toBeInTheDocument();
    });
  });
  ...
  describe('Dialog System Integration', () => {
    test('initializes dialog interceptor with dialog context', async () => {
      ...
      await waitFor(() => {
        expect(initializeDialogInterceptor).toHaveBeenCalledWith(mockDialogContext);
      });
    });
    ...
    test('logs success message when dialog interceptor initializes', async () => {
      ...
      await waitFor(() => {
        expect(console.log).toHaveBeenCalledWith('Interceptor de diálogos inicializado');
      });
    });
  ...
  describe('Event Debugger Integration', () => {
    test('loads debugger configuration on mount', async () => {
      ...
      await waitFor(() => {
        expect(storageService.get).toHaveBeenCalledWith('dev.event_debugger_enabled', false);
        expect(storageService.get).toHaveBeenCalledWith('dev.console_logs_enabled', false);
      });
    });
    ...
    test('enables debug mode when console logs are enabled', async () => {
      ...
      await waitFor(() => {
        ...
        expect(console.log).toHaveBeenCalledWith('🔧 Modo debug activado para el sistema de eventos');
      });
    });
  ...
  describe('Layout and Structure', () => {
    test('has correct CSS classes and structure', () => {
      ...
      const appContainer = screen.getByTestId('calendar-main').closest('.app-container');
  ...
  describe('Constants and Configuration', () => {
    test('APP_SECTIONS constants are used correctly', () => {
      ...
      expect(calendarItem.querySelector('[data-testid="icon"]')).toHaveTextContent('calendar_today');
      
      const settingsItem = screen.getByTestId('sidebar-item-configuración');
      expect(settingsItem.querySelector('[data-testid="icon"]')).toHaveTextContent('settings');
    });
  });
