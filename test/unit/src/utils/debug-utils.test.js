...
describe('Debug Utils', () => {
  ...
  beforeEach(() => {
    mockEvents = [
      { id: '1', title: 'Event 1', start: new Date().toISOString(), end: new Date().toISOString() }
    ];
    mockCreateEvent = jest.fn(eventData => ({ ...eventData, id: 'new-' + Date.now() }));
  ...
  test('debe configurar window.debugAtlas con las funciones correctas', () => {
    if (typeof window === 'undefined') {
      ...
      const cleanup = setupDebugTools(mockEvents, mockCreateEvent, mockUpdateEvent, mockSaveEvents);
      expect(typeof cleanup).toBe('function');
      cleanup(); // No debería fallar
      return;
    }

    setupDebugTools(mockEvents, mockCreateEvent, mockUpdateEvent, mockSaveEvents);
  ...
  test('window.debugAtlas.forceUpdate debe llamar a updateEvent con fechas modificadas', () => {
    ...
    expect(new Date(calledData.start).getHours()).toBe(expectedStartDate.getHours());
  });
  ...
  test('no debe fallar si window no está definido al ejecutar', () => {
    ...
    let cleanupFunction;
    expect(() => {
        cleanupFunction = setupDebugTools(mockEvents, mockCreateEvent, mockUpdateEvent, mockSaveEvents);
    }).not.toThrow();
