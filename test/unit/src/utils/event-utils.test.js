...
describe("Event Utils", () => {
  ...
  describe("initializeGridInfo", () => {
    ...
    test("debe inicializar correctamente para la vista diaria", () => {
      ...
      if (mockEventRefCurrent.parentElement) {
        // Asegurarse de que tiene padre antes de mover
  ...
  describe("findTargetSlot", () => {
    ...
    beforeEach(() => {
      ...
      document
        .querySelectorAll(".calendar-time-slot, .day-view-hour-slot")
        .forEach((el) => {
          const baseTop = el.dataset.hour ? parseInt(el.dataset.hour) * 60 : 0;
          let baseLeft = 0;
          if (
            el.parentElement &&
            el.parentElement.classList.contains("calendar-row")
          ) {
            const children = Array.from(el.parentElement.children);
            const index = children.indexOf(el);
            if (index > 0 && children[0].classList.contains("calendar-time")) {
              // Si la primera es la celda de tiempo
              baseLeft = (index - 1) * 100; // Asumiendo que 100 es el ancho de día
            } else if (index > 0) {
              // Si no hay celda de tiempo, o es una fila diferente
              baseLeft = index * 100;
            }
          }
    ...
    test("debe encontrar el slot correcto usando elementsFromPoint si está directamente bajo el cursor", () => {
      ...
      document.elementsFromPoint = jest
        .fn()
        .mockReturnValue([targetSlotElement, document.body]);
    ...
    test("debe actualizar data-events-count si el conteo visual es mayor", () => {
      ...
      document.elementsFromPoint = jest
        .fn()
        .mockReturnValue([targetSlotElement]);
