...
describe("Module Utils", () => {
  ...
  describe("checkTimeConflict", () => {
    ...
    test("debe detectar conflicto cuando los eventos se solapan", () => {
      const eventB = {
        start: new Date(baseTime + 30 * 60000).toISOString(),
        end: new Date(baseTime + 90 * 60000).toISOString(),
      };
    ...
    test("no debe detectar conflicto si los eventos son adyacentes", () => {
      const eventD = {
        start: new Date(baseTime + 60 * 60000).toISOString(),
        end: new Date(baseTime + 120 * 60000).toISOString(),
      };
    ...
    test("no debe detectar conflicto si los eventos están separados", () => {
      const eventE = {
        start: new Date(baseTime + 120 * 60000).toISOString(),
    ...
    test("debe devolver false y loguear si las cadenas de fecha son inválidas (resultan en NaN)", () => {
      const eventWithInvalidStartString = {
        start: "esto no es una fecha",
        end: eventA.end,
      };
      const eventWithInvalidEndString = {
        start: eventA.start,
        end: "tampoco esto",
      };

      expect(checkTimeConflict(eventWithInvalidStartString, eventA)).toBe(
        false
      );
      expect(console.error).toHaveBeenCalledWith(
        "Fechas inválidas al verificar conflicto"
      );
    ...
    test("debe loguear error genérico si new Date() lanza una excepción inesperada", () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error al verificar conflicto de tiempo:",
        expect.any(Error)
      );
    });
  });
  ...
  describe("convertEventFormat", () => {
    ...
    test("debe devolver copia si los formatos son iguales", () => {
      const converted = convertEventFormat(
        calendarEvent,
        "calendar",
        "calendar"
      );
    ...
    test("debe devolver null y advertir si la conversión no está implementada", () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        "Conversión de calendar a unknownFormat no implementada"
      );
    });
  ...
  describe("getRegisteredModules y executeAcrossModules", () => {
    ...
    test("getRegisteredModules debe devolver los nombres de los módulos registrados", () => {
      ...
      expect(modules).toEqual(
        expect.arrayContaining(["moduleA", "moduleB", "moduleC"])
      );
    ...
    test("executeAcrossModules debe ejecutar el método en los módulos que lo implementan", () => {
      ...
      expect(window.__appModules.moduleA.commonFunc).toHaveBeenCalledWith(
        1,
        "arg2"
      );
      expect(window.__appModules.moduleB.commonFunc).toHaveBeenCalledWith(
        1,
        "arg2"
      );
    ...
    test("executeAcrossModules debe manejar errores en la ejecución de un método", () => {
      window.__appModules.moduleA.commonFunc.mockImplementationOnce(() => {
        throw new Error("Exec error A");
      });
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error al ejecutar commonFunc en el módulo moduleA:",
        expect.any(Error)
      );
    });
  });
  ...
  describe("checkModuleDependencies", () => {
    ...
    test("debe devolver fallo y listar dependencias faltantes", () => {
      const result = checkModuleDependencies("myModule", [
        "depA",
        "depC",
        "depD",
      ]);
