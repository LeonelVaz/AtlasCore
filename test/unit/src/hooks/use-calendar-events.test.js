...
describe('useCalendarEvents Hook', () => {
  const mockEvents = [
    { id: '1', title: 'Evento 1', start: new Date(2023, 0, 1, 10, 0).toISOString(), end: new Date(2023, 0, 1, 11, 0).toISOString() },
    { id: '2', title: 'Evento 2', start: new Date(2023, 0, 2, 14, 0).toISOString(), end: new Date(2023, 0, 2, 15, 0).toISOString() },
  ];
  ...
  test('debe cargar eventos iniciales desde storageService y publicar EVENTS_LOADED', async () => {
    ...
    expect(eventBus.subscribe).toHaveBeenCalledWith('storage.eventsUpdated', expect.any(Function));
    expect(eventBus.publish).toHaveBeenCalledWith(CalendarEvents.EVENTS_LOADED, {
      events: mockEvents,
  ...
  test('debe manejar un array vacío o datos inválidos al cargar eventos', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith('Los datos cargados no son un array válido');

    storageService.get.mockResolvedValueOnce([{ id: '3', title: 'Invalid Date', start: 'no-es-fecha', end: 'tampoco' }]);
  ...
  test('debe recargar eventos cuando eventBus publica "storage.eventsUpdated"', async () => {
    ...
    const newMockEvents = [{ id: '3', title: 'Nuevo Evento', start: new Date().toISOString(), end: new Date().toISOString() }];
  ...
  test('createEvent debe añadir un evento, guardarlo y publicarlo', async () => {
    ...
    const newEventData = { title: 'Evento Creado', start: new Date(2023, 0, 3, 10, 0).toISOString(), end: new Date(2023, 0, 3, 11, 0).toISOString() };
    let createdEvent;
    await act(async () => {
    ...
    expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.EVENTS, expect.arrayContaining([expect.objectContaining({ title: 'Evento Creado' })]));
    expect(eventBus.publish).toHaveBeenCalledWith(CalendarEvents.EVENT_CREATED, { event: createdEvent });
  });
  ...
  test('updateEvent debe modificar un evento, guardarlo y publicarlo', async () => {
    ...
    await act(async () => {
      updatedEventResult = result.current.updateEvent(eventToUpdateId, updatedEventData);
    });
     act(() => { jest.advanceTimersByTime(100); });

    expect(result.current.events.find(e => e.id === eventToUpdateId).title).toBe('Evento Actualizado');
    expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.EVENTS, expect.any(Array));
    expect(eventBus.publish).toHaveBeenCalledWith(CalendarEvents.EVENT_UPDATED, { oldEvent, newEvent: updatedEventResult });
  });
  ...
  test('deleteEvent debe eliminar un evento, guardarlo y publicarlo', async () => {
    ...
    expect(storageService.set).toHaveBeenCalledWith(STORAGE_KEYS.EVENTS, expect.any(Array));
    expect(eventBus.publish).toHaveBeenCalledWith(CalendarEvents.EVENT_DELETED, { event: eventToDelete });
  });
