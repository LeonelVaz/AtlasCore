...
describe("useTimeGrid Hook", () => {
  ...
  test("debe inicializar con horas por defecto y cargar slots vacíos", async () => {
    ...
    expect(storageService.get).toHaveBeenCalledWith(
      STORAGE_KEYS.CUSTOM_TIME_SLOTS,
      {}
    );
  });
  ...
  describe("addCustomTimeSlot", () => {
    test("debe añadir un slot, recalcular duraciones y guardar", async () => {
      ...
      expect(storageService.set).toHaveBeenCalledWith(
        STORAGE_KEYS.CUSTOM_TIME_SLOTS,
        { 9: [{ minutes: 30, duration: 30 }] }
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
        "calendar.timeSlotsChanged",
        expect.any(Object)
      );
    ...
    test("no debe añadir si el slot ya existe", async () => {
      ...
      let success;
      act(() => {
        success = result.current.addCustomTimeSlot(9, 30);
      });
    ...
    test("debe ajustar minutos a múltiplo de 15 (y añadir si no existe)", async () => {
      ...
      let success;
      act(() => {
        success = result.current.addCustomTimeSlot(10, 20);
      });
      expect(success).toBe(true);
      expect(result.current.customSlots[10]).toEqual([
        { minutes: 15, duration: 45 },
      ]);
    });
    ...
    test("no debe añadir 00:45 si 00:30 no existe (subdivision order)", async () => {
      ...
      let success;
      act(() => {
        success = result.current.addCustomTimeSlot(0, 45);
      });
      expect(success).toBe(false);
  ...
  describe("shouldShowEventStart", () => {
    ...
    test("debe devolver true si el evento empieza dentro del slot", () => {
      ...
      expect(result.current.shouldShowEventStart(event, day, 10, 0, 60)).toBe(
        true
      );
    });
    test("debe devolver false si el evento empieza fuera del slot o en otro día", () => {
      ...
      const eventOtherDay = {
        start: new Date(2023, 0, 16, 10, 0).toISOString(),
      };
      expect(
        result.current.shouldShowEventStart(eventOtherDay, day, 10, 0, 60)
      ).toBe(false);
    });
    test("debe devolver false si el evento o su fecha de inicio son inválidos", () => {
      expect(result.current.shouldShowEventStart(null, day, 10, 0, 60)).toBe(
        false
      );
      expect(
        result.current.shouldShowEventStart(
          { start: "invalid-date" },
          day,
          10,
          0,
  ...
  test("isEventActiveAtStartOfDay debe funcionar correctamente", () => {
    ...
    expect(
      result.current.isEventActiveAtStartOfDay(eventSpanningMidnight, day)
    ).toBe(true);
    ...
    expect(
      result.current.isEventActiveAtStartOfDay(eventNotSpanning, day)
    ).toBe(false);
  });
  ...
  test("getEventPositionInSlot debe calcular el offset correctamente", () => {
    ...
    const position = result.current.getEventPositionInSlot(
      event,
      10,
      0,
      60,
