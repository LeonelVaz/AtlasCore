...
describe('useEventDrag Hook', () => {
  ...
  beforeEach(() => {
    ...
    eventUtils.calculatePreciseTimeChange.mockImplementation((deltaY, isResize, gridSize, snapValue) => {
  ...
  test('debe inicializar el estado y limpiar listeners al desmontar', () => {
    const { unmount } = renderHook(() => useEventDrag({
    ...
    expect(document.body.classList.remove).toHaveBeenCalledWith('dragging-active', 'snap-active');
  });

  test('handleDragStart debe iniciar el arrastre y añadir listeners', () => {
    const { result } = renderHook(() => useEventDrag({
      eventRef: mockEventRef, event: mockEvent, onUpdate: mockOnUpdate,
      setBlockClicks: mockSetBlockClicks,
    }));
    ...
    act(() => { result.current.handleDragStart(mockMouseDownEvent); });
  ...
  describe('Simulación de Arrastre', () => {
    ...
    test('handleMouseMove debe actualizar el estilo y estado si se mueve significativamente', () => {
        ...
        act(() => {
          result.current.handleDragStart({ preventDefault: jest.fn(), stopPropagation: jest.fn(), clientX: startX, clientY: startY, target: { classList: { contains: () => false } } });
        });
  
        act(() => { handleMouseMoveFn({ clientX: startX + 20, clientY: startY + 70 }); });
  
        expect(result.current.dragging).toBe(true);
        expect(mockEventRef.current.style.transform).toBe('translate(20px, 70px)');
        expect(mockEventRef.current.classList.add).toHaveBeenCalledWith('dragging');
    ...
    test('handleMouseUp debe llamar a onUpdate con las nuevas fechas si el arrastre fue significativo (sin snap, acoplando a hora completa)', () => {
      ...
      const startX = 100, startY = 200;
      act(() => { result.current.handleDragStart({ preventDefault: jest.fn(), stopPropagation: jest.fn(), clientX: startX, clientY: startY, target: { classList: { contains: () => false } } }); });
      
      const deltaY = 90; // Mover 90px Y (1.5 horas según píxeles)
            
      act(() => { handleMouseMoveFn({ clientX: startX, clientY: startY + deltaY }); });
      ...
      const expectedNewEndDate = new Date(expectedNewStartDate.getTime() + 2 * 60 * 60 * 1000); // Mantener duración de 2h
    ...
    test('handleMouseUp con snapValue debe llamar a onUpdate con fechas ajustadas por calculatePreciseTimeChange', () => {
        ...
        expect(eventUtils.calculatePreciseTimeChange).toHaveBeenCalledWith(deltaY, false, gridSize, snapValue);
        
        const expectedNewStartDate = new Date(initialEventTimestamp);
        expectedNewStartDate.setMinutes(expectedNewStartDate.getMinutes() + 15);
        const expectedNewEndDate = new Date(expectedNewStartDate.getTime() + 2 * 60 * 60 * 1000);
    ...
    test('handleMouseUp no debe llamar a onUpdate si el arrastre no fue significativo', () => {
        ...
        const startX = 100, startY = 200;
        act(() => { result.current.handleDragStart({ preventDefault: jest.fn(), stopPropagation: jest.fn(), clientX: startX, clientY: startY, target: { classList: { contains: () => false } } }); });
        act(() => { handleMouseMoveFn({ clientX: startX + 2, clientY: startY + 2 }); });
        act(() => { handleMouseUpFn({ preventDefault: jest.fn(), stopPropagation: jest.fn() }); });
    ...
    test('handleMouseUp no debe llamar a onUpdate si excede el límite de eventos', () => {
        const { result } = renderHook(() => useEventDrag({
          eventRef: mockEventRef, event: mockEvent, onUpdate: mockOnUpdate,
          setBlockClicks: mockSetBlockClicks, maxSimultaneousEvents: 1
        }));
  
        const startX = 100, startY = 200;
        const mockTargetSlotExceed = { 
            classList: { 
                contains: jest.fn(cn => cn === 'calendar-time-slot' || cn === 'day-view-hour-slot'), 
        ...
        expect(mockEventRef.current.classList.add).toHaveBeenCalledWith('cannot-place');
        expect(mockOnUpdate).not.toHaveBeenCalled();
        expect(mockSetBlockClicks).toHaveBeenCalledWith(true);
        act(() => { jest.runAllTimers(); });
        expect(mockSetBlockClicks).toHaveBeenLastCalledWith(false);
