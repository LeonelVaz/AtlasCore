...
describe("useEventResize Hook", () => {
  ...
  const initialEventTimestampStart = new Date(
    2023,
    0,
    15,
    10,
  ...
  const initialEventTimestampEnd =
    initialEventTimestampStart + 1 * 60 * 60 * 1000; // 11:00:00 LOCAL (1 hora duración)
  ...
  test("handleResizeStart debe iniciar el redimensionamiento y añadir listeners", () => {
    ...
    act(() => {
      result.current.handleResizeStart(mockMouseDownEvent);
    });
  ...
  describe("Simulación de Redimensionamiento", () => {
    ...
    test("handleMouseMove debe actualizar la altura del elemento (sin snap)", () => {
      ...
      if (!handleMouseMoveFn)
        throw new Error("handleMouseMoveFn no fue capturado");
      act(() => {
        handleMouseMoveFn({ clientY: startY + 30 });
      });
      expect(mockEventRef.current.style.height).toBe(
        `${mockEventRef.current.offsetHeight + 30}px`
      );
    });
    ...
    test("handleMouseMove debe actualizar la altura del elemento (con snap)", () => {
      ...
      if (!handleMouseMoveFn)
        throw new Error("handleMouseMoveFn no fue capturado");
      act(() => {
        handleMouseMoveFn({ clientY: startY + 20 });
      });
      // El deltaY de 20px, con snap a 15px, resultará en un adjustedDeltaY de 15px.
      expect(mockEventRef.current.style.height).toBe(
        `${mockEventRef.current.offsetHeight + snapPixels}px`
      );
    });
    ...
    test("handleMouseUp (snapValue=0) NO debe llamar a onUpdate si el delta NO es suficiente para cambiar la hora completa", () => {
      ...
      if (!handleMouseMoveFn || !handleMouseUpFn)
        throw new Error("Listeners no capturados");
    ...
    test("handleMouseUp (snapValue=0) SÍ debe llamar a onUpdate si el delta es suficiente para cambiar la hora completa", () => {
      ...
      act(() => {
        handleMouseMoveFn({ clientY: startY + 75 });
      });
      act(() => {
        handleMouseUpFn({
          preventDefault: jest.fn(),
      ...
      expectedNewEndDate.setHours(
        originalEndDateObject.getHours() + 1,
        0,
        0,
        0
      ); // 12:00 AM LOCAL
    ...
    test("handleMouseUp no debe llamar a onUpdate si no hubo redimensionamiento real", () => {
      ...
      if (!handleMouseMoveFn || !handleMouseUpFn)
        throw new Error("Listeners no capturados");
      act(() => {
        handleMouseMoveFn({ clientY: startY + 1 });
      });
      act(() => {
        handleMouseUpFn({
          preventDefault: jest.fn(),
    ...
    test("handleMouseUp debe asegurar que la fecha de fin no sea anterior o igual a la de inicio (con snap)", () => {
      ...
      if (!handleMouseMoveFn || !handleMouseUpFn)
        throw new Error("Listeners no capturados");
      act(() => {
        handleMouseMoveFn({ clientY: startY - 60 });
      }); // Mover para reducir más allá del inicio
      act(() => {
        handleMouseUpFn({
          preventDefault: jest.fn(),
          stopPropagation: jest.fn(),
      ...
      const expectedMinEndDate = new Date(
        eventStartDate.getTime() + snapValue * 60 * 1000
      ); // Inicio + snapValue
