...
describe("useTimeScale Hook", () => {
  test("debe devolver el contexto de escala de tiempo cuando se usa dentro de un TimeScaleProvider", () => {
    ...
    expect(result.current.setCurrentTimeScaleById).toBe(
      mockTimeScaleContextValue.setCurrentTimeScaleById
    );
  ...
  test("debe lanzar un error si se usa fuera de un TimeScaleProvider", () => {
    ...
    expect(() => renderHook(() => useTimeScale())).toThrow(
      "useTimeScale debe usarse dentro de un TimeScaleProvider"
    );
