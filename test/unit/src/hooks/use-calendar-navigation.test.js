...
describe('useCalendarNavigation Hook', () => {
  ...
  test('goToPreviousWeek debe restar 7 días y publicar evento', () => {
    ...
    const previousCurrentDate = new OriginalDate(result.current.currentDate.getTime()); // Crear una copia usando OriginalDate
  ...
  test('goToNextWeek debe sumar 7 días y publicar evento', () => {
    const { result } = renderHook(() => useCalendarNavigation());
    const previousCurrentDate = new OriginalDate(result.current.currentDate.getTime());
  ...
  test('goToPreviousDay debe restar 1 día a selectedDay y publicar evento', () => {
    const { result } = renderHook(() => useCalendarNavigation());
    const previousSelectedDay = new OriginalDate(result.current.selectedDay.getTime());
