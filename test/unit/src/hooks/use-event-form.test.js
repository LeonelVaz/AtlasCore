...
describe("useEventForm Hook", () => {
  ...
  const mockAllEventsData = [
    {
      ...
      start: new Date(2023, 0, 15, 10, 0).toISOString(),
      end: new Date(2023, 0, 15, 11, 0).toISOString(),
      color: "#ff0000",
    },
    {
      id: "event2",
      title: "Existing Event 2",
      start: new Date(2023, 0, 15, 12, 0).toISOString(),
      end: new Date(2023, 0, 15, 13, 0).toISOString(),
      color: "#00ff00",
    },
  ];
  ...
  describe("handleCellClick", () => {
    test("debe preparar un nuevo evento y mostrar el formulario (con fecha y hora)", () => {
      ...
      act(() => {
        result.current.handleCellClick(day, hour, 0, 60);
      });
      ...
      expect(dateUtils.formatDateForInput).toHaveBeenCalledWith(
        expectedStartDate
      );
      expect(dateUtils.formatDateForInput).toHaveBeenCalledWith(
        expectedEndDate
    ...
    test("debe usar la fecha completa si el argumento day ya incluye hora/minutos", () => {
      ...
      act(() => {
        result.current.handleCellClick(specificDateTime, 0, 0, 30);
      });

      const expectedEndDate = new Date(
        specificDateTime.getTime() + 30 * 60 * 1000
      );
  ...
  describe("handleEventClick", () => {
    test("debe cargar un evento existente y mostrar el formulario", () => {
      ...
      act(() => {
        result.current.handleEventClick(eventToLoad);
      });
      ...
      expect(dateUtils.formatDateForInput).toHaveBeenCalledWith(
        new Date(eventToLoad.start)
      );
      expect(dateUtils.formatDateForInput).toHaveBeenCalledWith(
        new Date(eventToLoad.end)
      );
    });
  });
  ...
  describe("handleEventFormChange", () => {
    test("debe actualizar el estado de newEvent para título y color", () => {
      ...
      act(() => {
        result.current.handleCellClick(new Date(), 9);
      });

      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Nuevo Título" },
        });
      });
      ...
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "color", value: "#0000ff" },
        });
      });
    ...
    test("debe actualizar el estado de newEvent para fechas", () => {
      ...
      const expectedISOForHookStart = new Date(
        newStartTimeString
      ).toISOString();
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "start", value: newStartTimeString },
        });
      });
      ...
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "end", value: newEndTimeString },
        });
      });
  ...
  describe("handleSaveEvent", () => {
    test("debe llamar a createEvent para un nuevo evento válido", () => {
      ...
      act(() => {
        result.current.handleCellClick(new Date(2023, 0, 16), 10);
      });
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Evento Guardado" },
        });
      });
      ...
      expect(mockCreateEvent).toHaveBeenCalledWith(
        expect.objectContaining({ title: "Evento Guardado" })
      );
      expect(result.current.showEventForm).toBe(false);
      expect(result.current.formError).toBe("");
    });
    ...
    test("debe llamar a updateEvent para un evento existente válido", () => {
      ...
      act(() => {
        result.current.handleEventClick(mockAllEventsData[0]);
      });
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Evento Modificado" },
        });
      });
      ...
      expect(mockUpdateEvent).toHaveBeenCalledWith(
        mockAllEventsData[0].id,
        expect.objectContaining({ title: "Evento Modificado" })
      );
      expect(result.current.showEventForm).toBe(false);
      expect(result.current.formError).toBe("");
    });
    ...
    test("debe mostrar error y no guardar si la fecha de fin es anterior a la de inicio", () => {
      ...
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Test Fechas" },
        });
      });
      ...
      act(() => {
        result.current.handleEventFormChange({
          target: {
            name: "start",
            value: dateUtils.formatDateForInput(new Date(2023, 0, 15, 11, 0)),
          },
        });
      });
      act(() => {
        result.current.handleEventFormChange({
          target: {
            name: "end",
            value: dateUtils.formatDateForInput(new Date(2023, 0, 15, 10, 0)),
          },
        });
      });
    ...
    test("debe mostrar error si se excede el límite de eventos simultáneos al crear", () => {
      ...
      const threeEvents = [
        {
          id: "e1",
          start: localTime1000.toISOString(),
          end: new Date(localTime1000.getTime() + 60 * 60000).toISOString(),
        },
        {
          id: "e2",
          start: localTime1015.toISOString(),
          end: new Date(localTime1015.getTime() + 60 * 60000).toISOString(),
        },
        {
          id: "e3",
          start: localTime1030.toISOString(),
          end: new Date(localTime1030.getTime() + 60 * 60000).toISOString(),
        },
      ];
      const { result } = renderHook(() =>
        useEventForm(
      ...
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Conflicting Event" },
        });
        result.current.handleEventFormChange({
          target: {
            name: "start",
            value: dateUtils.formatDateForInput(localTime1045),
          },
        });
        result.current.handleEventFormChange({
          target: {
            name: "end",
            value: dateUtils.formatDateForInput(
              new Date(localTime1045.getTime() + 60 * 60000)
            ),
          },
        });
        result.current.handleEventFormChange({
          target: { name: "color", value: "#2d4b94" },
        });
      });
    ...
    test("NO debe mostrar error de límite si se edita un evento sin cambiar su tiempo", () => {
      ...
      const { result } = renderHook(() =>
        useEventForm(
          ...
          mockUpdateEvent,
          mockDeleteEvent,
          threeEventsIncludingSelected,
          3
        )
      );

      act(() => {
        result.current.handleEventClick(threeEventsIncludingSelected[0]);
      }); // Carga event1 para editar
      act(() => {
        result.current.handleEventFormChange({
          target: { name: "title", value: "Título Cambiado" },
        });
      ...
      expect(result.current.formError).toBe("");
      expect(mockUpdateEvent).toHaveBeenCalledWith(
        threeEventsIncludingSelected[0].id,
        expect.objectContaining({ title: "Título Cambiado" })
      );
    });
  });
