...
export default {
  ...
  description:
    "Plugin simple para gestionar notas personales con editor de texto enriquecido y vinculación con eventos",
    ...
    return new Promise(function (resolve) {
      try {
        ...
        const hasRichText = !!(
          ...
          core?.ui?.components?.RichTextViewer
        );
        ...
        console.error(
          "[Notas Simples] Error durante la inicialización:",
          error
        );
        resolve(false);
      }
  ...
  _loadNotes: function () {
    ...
    return this._core.storage
      .getItem(this.id, "notes", [])
      .then(function (notes) {
        self._notes = notes || [];
        console.log(
          "[Notas Simples] Cargadas " + self._notes.length + " notas"
        );
        return self._notes;
      });
  },
  ...
  _saveNotes: function () {
    if (this._core) {
      return this._core.storage
        .setItem(this.id, "notes", this._notes)
        .then(function () {
  ...
  _handleEventDelete: function (data) {
    ...
    if (updated) {
      this._saveNotes();
      console.log(
        "[Notas Simples] Removidas vinculaciones de evento eliminado:",
        data.event.id
      );
    }
  },
  ...
  _registerNavigation: function () {
    ...
    console.log(
      "[Notas Simples] Navegación registrada con ID:",
      this._navigationExtensionId
    );
  },
  ...
  _registerPage: function () {
    ...
    console.log(
      "[Notas Simples] Página registrada con ID:",
      this._pageExtensionId
    );
  },
  ...
  createNoteFromEvent: function (event) {
    ...
    if (!navigated) {
      ...
      console.error(
        "[Notas Simples] No se pudo abrir el formulario de creación"
      );
  ...
  createNote: function (title, content, linkedEventId, linkedEventTitle) {
    ...
    console.log(
      "[Notas Simples] Nota creada:",
      newNote.id,
      linkedEventId ? "(vinculada a evento)" : ""
    );
    return newNote;
  },
  ...
  updateNote: function (noteId, updates) {
    ...
    if (noteIndex !== -1) {
      ...
      // Publicar evento para actualizar UI
      if (this._core && this._core.events) {
        this._core.events.publish(this.id, "noteUpdated", {
          note: this._notes[noteIndex],
        });
      }
  ...
  unlinkNoteFromEvent: function (noteId) {
    ...
    if (noteIndex !== -1) {
      ...
      if (this._notes[noteIndex].tags) {
        this._notes[noteIndex].tags = this._notes[noteIndex].tags.filter(
          (tag) => tag !== "calendario"
        );
      }
  ...
  getNotesStats: function () {
    ...
    return {
      ...
      createdToday: this._notes.filter(
        (note) => new Date(note.createdAt) >= today
      ).length,
      createdThisWeek: this._notes.filter(
        (note) => new Date(note.createdAt) >= thisWeek
      ).length,
      withRichContent: this._notes.filter(
  ...
  searchNotes: function (query) {
    ...
    return this._notes.filter((note) => {
      try {
        ...
        const eventMatch =
          note.linkedEventTitle &&
          note.linkedEventTitle.toLowerCase().includes(searchTerm);
